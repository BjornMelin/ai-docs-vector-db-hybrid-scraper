...
def process_file(filepath: Path, fix_types: set[str]) -> list[str]:
    ...
    if 'asyncio_marks' in fix_types and fix_missing_asyncio_marks(filepath):
        fixes_applied.append('Added @pytest.mark.asyncio')

    if 'httpx_mocks' in fix_types and convert_httpx_mock_to_respx(filepath):
        fixes_applied.append('Converted httpx mocks to respx')

    if 'context_managers' in fix_types and fix_async_context_managers(filepath):
