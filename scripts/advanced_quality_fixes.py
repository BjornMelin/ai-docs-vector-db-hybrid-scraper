...
class AdvancedQualityFixer:
    ...
    def fix_undefined_names(self, violations: list[AdvancedViolation]) -> int:
        ...
        for filepath, file_violations in files_to_fix.items():
            try:
                ...
                logger.exception(f"Failed to fix undefined names in {filepath}: {e}")
                self.fixes_failed += 1
    ...
    def fix_blind_except_clauses(self, violations: list[AdvancedViolation]) -> int:
        ...
        for filepath, file_violations in files_to_fix.items():
            try:
                ...
                logger.exception(f"Failed to fix blind except in {filepath}: {e}")
                self.fixes_failed += 1
    ...
    def _infer_exception_type(self, context: str) -> str:
        ...
        if any(word in context.lower() for word in ['file', 'open', 'read', 'write']):
            return "OSError"
        if any(word in context.lower() for word in ['json', 'parse', 'decode']):
            return "(ValueError, TypeError)"
        if any(word in context.lower() for word in ['http', 'request', 'api', 'client']):
            return "Exception"  # Often custom exceptions
        if any(word in context.lower() for word in ['import', 'module']):
            return "ImportError"
        if any(word in context.lower() for word in ['key', 'index', 'dict', 'list']):
    ...
    def fix_logging_format_strings(self, violations: list[AdvancedViolation]) -> int:
        ...
        for filepath, file_violations in files_to_fix.items():
            try:
                ...
                logger.exception(f"Failed to fix logging format in {filepath}: {e}")
                self.fixes_failed += 1
    ...
    def run_advanced_fixes(self) -> dict[str, int]:
        ...
        for fix_name, fix_func in fixes:
            try:
                ...
                logger.exception(f"❌ {fix_name}: Failed - {e}")
                results[fix_name] = 0
