...
class TestMarkerAnalyzer(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        if node.name.startswith('test_'):
            ...
            if any(keyword in source.lower() for keyword in ['sleep', 'time.sleep', 'asyncio.sleep']):
    ...
    def _analyze_import(self, import_name: str):
        ...
        # Browser automation
        if any(browser in import_name for browser in ['playwright', 'browser_use', 'crawl4ai', 'selenium']):
            self.has_browser_imports = True
            self.markers_needed.add('browser')
        
        # Network dependencies
        if any(net in import_name for net in ['requests', 'httpx', 'aiohttp', 'urllib', 'firecrawl']):
            self.has_network_imports = True
            self.markers_needed.add('network')
        
        # Database dependencies
        if any(db in import_name for db in ['qdrant', 'redis', 'sqlalchemy', 'sqlite', 'database']):
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if line.strip().startswith('class Test') and 'Test' in line:
                ...
                marker_lines = [f'@pytest.mark.{marker}' for marker in sorted(new_markers)]
