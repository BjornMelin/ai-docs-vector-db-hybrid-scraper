...
class ConfigDeploymentValidator:
    ...
    def validate_yaml_files(self) -> ValidationResult:
        ...
        yaml_files = list(self.config_dir.rglob("*.yml")) + list(
            ...
            try:
                result = subprocess.run(
                    [
                        ...
                        "--rm",
                        "-v",
                        f"{prometheus_config.parent}:/etc/prometheus",
                ...
                if result.returncode != 0:
                    errors.append(
                        f"Prometheus config validation failed: {result.stderr}"
                    )
            except Exception:
        ...
        if grafana_dashboards.exists():
            for dashboard_file in grafana_dashboards.glob("*.json"):
                try:
                    ...
                    errors.append(
                        f"Failed to validate Grafana dashboard {dashboard_file}: {e}"
                    )
    ...
    def validate_security_compliance(self) -> ValidationResult:
        ...
        for config_file in self.config_dir.rglob("*.json"):
            try:
                ...
                if "_template_placeholder_" not in content and "example" not in str(
                    config_file
                ):
                    if any(
...
def main():
    ...
    parser.add_argument(
        "--report-file", type=Path, help="Path to save validation report"
    )
