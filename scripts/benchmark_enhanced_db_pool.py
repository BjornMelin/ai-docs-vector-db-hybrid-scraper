...
class DatabasePoolBenchmark:
    ...
    async def run_benchmark_scenario(
        self, manager, num_operations: int, concurrency: int, scenario_name: str
    ) -> BenchmarkMetrics:
        """Run a benchmark scenario with specified parameters."""

        logger.info(
            f"Running {scenario_name} benchmark: {num_operations} ops, concurrency {concurrency}"
        )
            ...
            for i in range(0, num_operations // concurrency, batch_size):
                ...
                if hasattr(manager, "adaptive_sizing"):
                    ...
                    for j in range(min(batch_size, num_operations // concurrency - i)):
                        ...
                        sub_batch_tasks.append(
                            self._execute_single_query(manager, query)
                        )

                    sub_batch_results = await asyncio.gather(
                        *sub_batch_tasks, return_exceptions=True
                    )
    ...
    async def run_comprehensive_benchmark(
        self,
    ) -> dict[str, tuple[BenchmarkMetrics, BenchmarkMetrics]]:
        ...
        for scenario_name, num_ops, concurrency in scenarios:
            ...
            throughput_improvement = (
                (
                    enhanced_metrics.throughput_ops_per_sec
    ...
    def print_detailed_results(
        self, results: dict[str, tuple[BenchmarkMetrics, BenchmarkMetrics]]
    ):
        ...
        for scenario_name, (baseline, enhanced) in results.items():
            ...
            print(
                f"{'Metric':<25} {'Baseline':<15} {'Enhanced':<15} {'Improvement':<12}"
            )
            print("-" * 67)

            # Latency metrics
            p95_improvement = (
                (baseline.p95_latency_ms - enhanced.p95_latency_ms)
            ...
            print(
                f"{'P95 Latency (ms)':<25} {baseline.p95_latency_ms:<15.2f} {enhanced.p95_latency_ms:<15.2f} {p95_improvement:<12.1f}%"
            )
            print(
                f"{'Avg Latency (ms)':<25} {baseline.avg_latency_ms:<15.2f} {enhanced.avg_latency_ms:<15.2f} {((baseline.avg_latency_ms - enhanced.avg_latency_ms) / baseline.avg_latency_ms * 100):<12.1f}%"
            )

            # Throughput metrics
            throughput_improvement = (
                (enhanced.throughput_ops_per_sec - baseline.throughput_ops_per_sec)
            ...
            print(
                f"{'Throughput (ops/sec)':<25} {baseline.throughput_ops_per_sec:<15.1f} {enhanced.throughput_ops_per_sec:<15.1f} {throughput_improvement:<12.1f}%"
            )

            # Efficiency metrics
            print(
                f"{'Error Rate (%)':<25} {baseline.error_rate * 100:<15.2f} {enhanced.error_rate * 100:<15.2f} {((baseline.error_rate - enhanced.error_rate) / max(baseline.error_rate, 0.001) * 100):<12.1f}%"
            )
        ...
        throughput_status = (
            "✅ ACHIEVED" if avg_throughput_improvement >= 40.0 else "❌ NOT ACHIEVED"
        )
