...
class TestPerformanceProfiler:
    ...
    def _analyze_performance(self, timings: List[Dict]) -> Dict:
        ...
        return {
            ...
            "categories": {
                "fast": {"count": len(fast_tests), "percentage": len(fast_tests) / len(timings) * 100},
                "medium": {"count": len(medium_tests), "percentage": len(medium_tests) / len(timings) * 100},
                "slow": {"count": len(slow_tests), "percentage": len(slow_tests) / len(timings) * 100},
            },
            "bottlenecks": {
    ...
    def _identify_optimizations(self, timings: List[Dict], file_stats: Dict) -> List[Dict]:
        ...
        slow_unit_files = [f for f in unit_test_files if file_stats[f]["avg_time"] > 0.5]
    ...
    def generate_report(self, output_file: str = None) -> str:
        ...
        report.append(f"  Fast Tests (<0.1s): {summary['fast_tests']} ({summary['fast_tests']/summary['total_tests']*100:.1f}%)")
        report.append(f"  Medium Tests (0.1-2s): {summary['medium_tests']} ({summary['medium_tests']/summary['total_tests']*100:.1f}%)")
        report.append(f"  Slow Tests (>2s): {summary['slow_tests']} ({summary['slow_tests']/summary['total_tests']*100:.1f}%)")
        report.append("")
        ...
        for i, (file_path, stats) in enumerate(bottlenecks["slowest_files"][:5], 1):
            report.append(f"  {i}. {file_path} - {stats['total_time']:.2f}s ({stats['test_count']} tests)")
        report.append("")
        ...
        if optimizations:
            ...
            for opt in optimizations:
                report.append(f"  🔧 {opt['type'].title()} ({opt['priority']} priority)")
        ...
        if summary['slow_tests'] > summary['total_tests'] * 0.1:
            ...
            report.append("     Consider mocking external dependencies and optimizing test data")
        ...
        report.append("  ✅ Consider implementing test markers for fast/slow categorization")
        report.append("  ✅ Use session-scoped fixtures for expensive setup")
        report.append("  ✅ Implement test data factories for consistent, minimal test data")
...
def main():
    ...
    parser.add_argument(
        "--pattern",
        default="",
        help="Test pattern to profile (default: all tests)"
    )
    parser.add_argument(
        "--no-parallel",
        action="store_true",
    ...
    parser.add_argument(
        "--output",
        default="test_performance_report.txt",
