...
class QualityFixer:
    ...
    def fix_unused_imports(self, violations: list[Violation]) -> int:
        ...
        for filepath, file_violations in files_to_fix.items():
            try:
                ...
                logger.exception(f"Failed to fix unused imports in {filepath}: {e}")
                self.fixes_failed += 1
    ...
    def fix_typing_imports(self, violations: list[Violation]) -> int:
        ...
        for filepath, file_violations in files_to_fix.items():
            try:
                ...
                logger.exception(f"Failed to fix typing imports in {filepath}: {e}")
                self.fixes_failed += 1
    ...
    def fix_datetime_timezone(self, violations: list[Violation]) -> int:
        ...
        for filepath, file_violations in files_to_fix.items():
            try:
                ...
                logger.exception(f"Failed to fix timezone issues in {filepath}: {e}")
                self.fixes_failed += 1
    ...
    def fix_whitespace_issues(self, violations: list[Violation]) -> int:
        ...
        try:
            ...
            logger.exception(f"Failed to run ruff auto-fix: {e}")
            return 0
