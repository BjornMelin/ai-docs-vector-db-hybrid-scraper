...
class ConfigurationRollback:
    ...
    def restore_snapshot(
        self, snapshot: DeploymentSnapshot, create_backup: bool = True
    ) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Configuration restored from snapshot: {snapshot.snapshot_id}"
            )
    ...
    def rollback_with_git(self, snapshot: DeploymentSnapshot) -> bool:
        ...
        self.logger.info(
            f"Performing Git-based rollback to commit: {snapshot.commit_sha}"
        )
        ...
        try:
            ...
            if result.returncode != 0:
                self.logger.error(
                    "Not in a Git repository - cannot perform Git rollback"
                )
                return False
            ...
            for file_path in snapshot.changed_files:
                if file_path.startswith("config/"):
                    ...
                    if result.returncode != 0:
                        self.logger.warning(
                            f"Failed to rollback file {file_path}: {result.stderr}"
                        )
                    else:
            ...
            # Commit the rollback
            commit_message = (
                f"Rollback configuration to snapshot {snapshot.snapshot_id}"
            )

            subprocess.run(
                ["git", "add", "config/"], cwd=self.config_dir.parent, check=True
            )
    ...
    def validate_post_rollback(self, environment: str) -> bool:
        ...
        if not validate_script.exists():
            self.logger.warning(
                "Validation script not found - skipping post-rollback validation"
            )
            return True
...
def main():
    ...
    list_parser.add_argument(
        "--limit", type=int, default=10, help="Limit number of snapshots to show"
    )
    ...
    rollback_parser.add_argument(
        "--no-backup", action="store_true", help="Skip creating pre-rollback backup"
    )
    rollback_parser.add_argument(
        "--use-git", action="store_true", help="Use Git-based rollback"
    )
    rollback_parser.add_argument(
        "--skip-validation", action="store_true", help="Skip post-rollback validation"
    )
    rollback_parser.add_argument(
        "--report-file", type=Path, help="Path to save rollback report"
    )

    # Validate command
    validate_parser = subparsers.add_parser(
        "validate", help="Validate a snapshot for rollback"
    )
    ...
    if args.command == "list":
        ...
        print(
            f"📋 Available deployment snapshots (showing {min(len(snapshots), args.limit)}):"
        )
        print()
        ...
        if snapshot.environment == "production":
            confirm = input(
                "⚠️  This is a PRODUCTION rollback. Type 'CONFIRM' to proceed: "
            )
            if confirm != "CONFIRM":
        ...
        if success and not args.skip_validation:
            validation_success = rollback_manager.validate_post_rollback(
