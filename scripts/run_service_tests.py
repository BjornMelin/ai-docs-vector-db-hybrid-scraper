    ...
    def print_summary(self):
        ...
        for suite_name, result in self.results.items():
            ...
            if status == "passed":
                ...
                print(
                    f"  💥 {suite_name}: {description} - {result.get('error', 'Unknown error')}"
                )
            else:
...
"""
        ...
        for suite_name, result in self.results.items():
            ...
            status_emoji = {"passed": "✅", "failed": "❌", "crashed": "💥"}.get(
                status, "❓"
            )
        ...
        if summary["failed_suites"]:
            ...
            for suite in summary["failed_suites"]:
                result = self.results.get(suite, {})
                report += (
                    f"- **{suite}**: {result.get('description', 'No description')}\n"
                )
            report += "\n"
        ...
        if summary["overall_status"] == "passed":
            ...
            report += (
                "- Consider implementing mutation testing for additional validation\n"
            )
...
Examples:
  ...
  python scripts/run_service_tests.py --all --no-benchmarks   # Run all except benchmarks
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Run dependency injection tests",
    )
    parser.add_argument(
        "--circuit-breaker", action="store_true", help="Run circuit breaker tests"
    )
    parser.add_argument(
        "--database-pooling", action="store_true", help="Run database pooling tests"
    )
    parser.add_argument(
        "--browser-monitoring", action="store_true", help="Run browser monitoring tests"
    )
    parser.add_argument(
        "--integration", action="store_true", help="Run service integration tests"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-benchmarks", action="store_true", help="Exclude performance benchmarks"
    )
    parser.add_argument(
        "--no-mutation", action="store_true", help="Exclude mutation testing"
    )
    ...
    if args.all:
        ...
        if args.dependency_injection:
            results["dependency_injection"] = runner.run_dependency_injection_tests(
                args.verbose
            )
        ...
        if args.database_pooling:
            results["database_pooling"] = runner.run_database_pooling_tests(
                args.verbose
            )

        if args.browser_monitoring:
            results["browser_monitoring"] = runner.run_browser_monitoring_tests(
                args.verbose
            )

        if args.integration:
            results["service_integration"] = runner.run_service_integration_tests(
                args.verbose
            )

        if args.performance:
            results["performance_benchmarks"] = runner.run_performance_benchmarks(
                args.verbose
            )
        ...
        if not results:
            print(
                "❌ No test suites specified. Use --all or specify individual suites."
            )
            parser.print_help()
            return 1
        ...
        failed_suites = [
            name for name, result in results.items() if result["status"] != "passed"
        ]
