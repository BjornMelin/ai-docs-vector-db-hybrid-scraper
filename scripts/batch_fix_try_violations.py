    ...
    for filepath, file_violations in by_file.items():
        try:
            ...
            patterns = [
                ...
                (
                    r"(logger\.(?:error|warning|info|debug|critical)\([^)]+),\s*exception=\w+\)",
                    r"\1)",
                ),
                (
                    r"(logger\.(?:error|warning|info|debug|critical)\([^)]+),\s*exc=\w+\)",
                    r"\1)",
                ),
...
def main():
    ...
    if final_count < 200:
        ...
        print(
            f"⚠️  Target not yet achieved. {final_count - 200} more violations to fix."
        )
