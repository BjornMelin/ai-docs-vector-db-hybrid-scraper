...
class TestAriaAttributeCompliance:
    ...
    def test_aria_roles_validation(self, wcag_validator):
        ...
        <div role="application">Discouraged role</div>
        <div role="directory">Deprecated role</div>
        <div role="document">Rarely appropriate role</div>
        <div role="">Empty role</div>
        <div role="multiple roles here">Multiple roles in one attribute</div>
        """
        
        # Test valid roles - should pass validation
        result = wcag_validator.validate_aria_attributes(valid_roles_html)
    ...
    def test_aria_properties_and_states(self):
        ...
        aria_attributes = [
            ...
            "aria-rowindex", "aria-colindex", "aria-sort",
        ]
    ...
    def test_aria_label_and_description_hierarchy(self):
        ...
        <!-- Self-referencing -->
        <div id="self-ref" aria-labelledby="self-ref">
            Self-referencing element
        </div>
        
        <!-- Circular references -->
        <div id="elem1" aria-labelledby="elem2">Element 1</div>
        <div id="elem2" aria-labelledby="elem1">Element 2</div>
        
        <!-- Invalid ID references -->
        <input type="text" aria-labelledby="nonexistent-id">
        <button aria-describedby="also-missing">Button</button>
        """
        ...
        assert 'aria-labelledby="first-name-label required-indicator"' in correct_labeling_html
        assert 'aria-describedby="password-requirements password-strength"' in correct_labeling_html
    ...
    def test_aria_interactive_widgets(self):
        ...
        widget_roles = [
            "dialog", "menu", "menuitem", "combobox", "listbox", "option",
            "grid", "gridcell", "columnheader", "slider", "tree", "treeitem"
        ]
        
        for role in widget_roles:
            assert f'role="{role}"' in interactive_widgets_html, f"Should include {role} widget"
    ...
    def test_aria_state_management(self):
        ...
        state_attributes = [
            ...
            'aria-invalid', 'aria-disabled', 'aria-required', 'aria-busy',
            'aria-sort', 'aria-multiselectable'
        ]
    ...
    @pytest.mark.parametrize("role,required_attrs,optional_attrs", [
        ("button", [], ["aria-pressed", "aria-expanded"]),
        ("checkbox", ["aria-checked"], ["aria-required", "aria-invalid"]),
        ("combobox", ["aria-expanded"], ["aria-autocomplete", "aria-owns", "aria-activedescendant"]),
        ...
        ("slider", ["aria-valuenow", "aria-valuemin", "aria-valuemax"], ["aria-valuetext", "aria-orientation"]),
    ...
    def test_role_attribute_requirements(self, role, required_attrs, optional_attrs):
        ...
        for attr in required_attrs:
            ...
            assert attr.startswith("aria-"), f"Required attribute {attr} should be ARIA attribute"
        
        # Optional attributes are commonly used
        for attr in optional_attrs:
            # Test that we know this attribute is valid for the role
            assert attr.startswith("aria-"), f"Optional attribute {attr} should be ARIA attribute"
    ...
    def test_aria_validation_comprehensive_report(self, wcag_validator):
        ...
        aria_report = {
            ...
            "role_analysis": {
                ...
                "widget_roles": ["combobox", "listbox", "option", "grid", "gridcell", "columnheader"],
            ...
            "attribute_coverage": {
                ...
                "state": ["aria-expanded", "aria-selected", "aria-checked", "aria-invalid"],
                "properties": ["aria-required", "aria-owns", "aria-controls", "aria-live"],
                "relationships": ["aria-owns", "aria-controls", "aria-describedby", "aria-labelledby"]
            },
        ...
        if result["compliant"]:
            ...
            error_issues = len([i for i in result["issues"] if i.get("severity") == "error"])
            aria_report["compliance_score"] = max(0.0, 1.0 - (error_issues / max(total_issues, 1)))
