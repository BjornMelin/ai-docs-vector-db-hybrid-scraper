...
class NetworkPartitionSimulator:
    ...
    def get_partition_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "largest_partition": max(partitions.values(), key=len) if partitions else [],
            "isolated_nodes": [node for partition in partitions.values() if len(partition) == 1 for node in partition]
        }
...
class TestNetworkPartitions:
    ...
    def network_simulator(self):
        ...
        simulator.add_node("node_1", NetworkZone.ZONE_A, {"role": "leader", "data": {"key1": "value1"}})
        simulator.add_node("node_2", NetworkZone.ZONE_A, {"role": "follower", "data": {"key2": "value2"}})
        simulator.add_node("node_3", NetworkZone.ZONE_B, {"role": "follower", "data": {"key3": "value3"}})
        simulator.add_node("node_4", NetworkZone.ZONE_B, {"role": "follower", "data": {"key4": "value4"}})
        simulator.add_node("node_5", NetworkZone.ZONE_C, {"role": "follower", "data": {"key5": "value5"}})
        
        return simulator

    async def test_simple_network_partition(
        self,
        network_simulator,
        fault_injector
    ):
        ...
        assert await network_simulator.can_communicate("node_1", "node_2")  # Same partition
        assert not await network_simulator.can_communicate("node_1", "node_5")  # Different partition
        assert not await network_simulator.can_communicate("node_3", "node_5")  # Different partition
        
        # Test message sending
        assert await network_simulator.send_message("node_1", "node_2", {"test": "message"})
        
        with pytest.raises(ConnectionError):
            await network_simulator.send_message("node_1", "node_5", {"test": "message"})
    ...
    async def test_split_brain_prevention(
        ...
        class DistributedConsensus:
            ...
            async def elect_leader(self, candidate_node: str) -> dict[str, Any]:
                ...
                for node_id in reachable_nodes:
                    ...
                    try:
                        ...
                        await self.network.send_message(candidate_node, node_id, {
                            "type": "vote_request",
                            "term": self.term + 1,
            ...
            async def check_split_brain(self) -> dict[str, Any]:
                ...
                split_brain_risk = len([p for p in potential_leaders if p["can_elect_leader"]]) > 1
        ...
        assert not minority_election["leader_elected"]  # Should fail due to insufficient votes

    async def test_quorum_based_operations(
        self,
        network_simulator
    ):
        ...
        class QuorumBasedStorage:
            ...
            async def write_with_quorum(self, key: str, value: Any, initiator_node: str) -> dict[str, Any]:
                ...
                for node_id in reachable_nodes:
                    try:
                        await self.network.send_message(initiator_node, node_id, {
                            "type": "write_request",
                            "key": key,
                            "value": value
                        })
            ...
            async def read_with_quorum(self, key: str, initiator_node: str) -> dict[str, Any]:
                ...
                if quorum_achieved and read_values:
                    ...
                    consensus_value = max(read_values.keys(), key=lambda v: read_values[v])
        ...
        write_result = await storage.write_with_quorum("test_key", "test_value", "node_1")
        ...
        majority_write = await storage.write_with_quorum("partition_key", "partition_value", "node_1")
        assert majority_write["write_successful"]
        
        # Write from minority partition - should fail
        minority_write = await storage.write_with_quorum("minority_key", "minority_value", "node_5")
    ...
    async def test_network_partition_detection(
        ...
        class PartitionDetector:
            ...
            async def send_heartbeat(self, from_node: str) -> dict[str, Any]:
                ...
                for target_node in self.network.nodes:
                    ...
                    try:
                        await self.network.send_message(from_node, target_node, {
                            "type": "heartbeat",
                            "timestamp": time.time(),
                            "from": from_node
                        })
            ...
            async def detect_partitions(self, timeout_threshold: float = 5.0) -> dict[str, Any]:
                ...
                return {
                    ...
                    "partition_info": partition_info,
                    "detection_confidence": len(suspected_down_nodes) / len(self.network.nodes) if self.network.nodes else 0
                }
            
            async def validate_partition_suspicion(self, suspected_node: str, validator_node: str) -> bool:
                """Validate partition suspicion by attempting direct communication."""
                try:
                    await self.network.send_message(validator_node, suspected_node, {
                        "type": "ping",
                        "timestamp": time.time()
                    })
        ...
        # Validate suspicion
        partition_confirmed = await detector.validate_partition_suspicion("node_5", "node_1")
        assert partition_confirmed

    async def test_partition_recovery_mechanisms(
        self,
        network_simulator
    ):
        ...
        class PartitionRecoveryManager:
            ...
            async def repair_inconsistencies(self) -> dict[str, Any]:
                ...
                reference_node = list(self.network.nodes.keys())[0]
        ...
        await recovery_manager.record_operation("node_1", {"type": "write", "key": "a", "value": 1})
        await recovery_manager.record_operation("node_2", {"type": "write", "key": "b", "value": 2})
        
        # Create partition
        network_simulator.create_partition([NetworkZone.ZONE_C])
        
        # Record operations in different partitions
        await recovery_manager.record_operation("node_1", {"type": "write", "key": "c", "value": 3})  # Majority partition
        await recovery_manager.record_operation("node_5", {"type": "write", "key": "d", "value": 4})  # Minority partition
    ...
    async def test_jepsen_style_consistency_checking(
        self,
        network_simulator
    ):
                ...
                try:
                    ...
                    if len(reachable_nodes) >= len(self.network.nodes) // 2:  # Majority
                        ...
                        for target_node in reachable_nodes:
                            try:
                                await self.network.send_message(node_id, target_node, {
            ...
            def check_consistency(self) -> dict[str, Any]:
                ...
                for read_op in self.reads_log:
                    ...
                    for write_op in reversed(self.operations_log):
                        if (write_op["type"] == "write" and 
                            ...
                            write_op["timestamp"] < read_op["timestamp"] and
                            write_op["success"]):
                            last_write = write_op
                            break
                ...
                for key, reads in key_read_history.items():
                    ...
                    for i in range(1, len(reads)):
                        ...
                        if (current_read["value"] is not None and 
                            previous_read["value"] is not None and
                            current_read["value"] != previous_read["value"]):
