...
class TestChaosIntegration:
    ...
    def integrated_system(self):
        ...
        class IntegratedSystemSimulator:
            def __init__(self):
                # System components
                self.services = {
                    "api_gateway": {"status": "healthy", "connections": 0, "response_time": 0.05},
                    "search_service": {"status": "healthy", "connections": 0, "response_time": 0.1},
                    "vector_db": {"status": "healthy", "connections": 0, "response_time": 0.03},
                    "embedding_service": {"status": "healthy", "connections": 0, "response_time": 0.08},
                    "cache": {"status": "healthy", "connections": 0, "response_time": 0.01},
                    "auth_service": {"status": "healthy", "connections": 0, "response_time": 0.02}
                }
            ...
            async def inject_failure(self, failure_type: FailureType, target_service: str):
                ...
                if target_service in self.services:
                    if failure_type == FailureType.SERVICE_UNAVAILABLE:
                        ...
                        self.services[target_service]["response_time"] *= 10  # 10x slower
                    elif failure_type == FailureType.MEMORY_EXHAUSTION:
                        self.resources["memory"]["used"] = min(95, self.resources["memory"]["total"])
                        self.services[target_service]["status"] = "degraded"
                
                # Trigger cascading effects
                await self._handle_failure_cascade(failure_type, target_service)
                
            async def _handle_failure_cascade(self, failure_type: FailureType, failed_service: str):
            ...
            def _update_system_metrics(self):
                ...
                # Update error rate
                failed_services = len([s for s in self.services.values() if s["status"] == "failed"])
                ...
                throughput_factor = max(0.1, 1.0 - (failed_services * 0.3) - (degraded_services * 0.1))
                ...
                self.metrics["latency_p95"] = avg_response_time * 5  # P95 is ~5x average
            ...
            async def _recover_service(self, service_name: str):
                ...
                self.services[service_name]["response_time"] = baseline_times.get(service_name, 0.05) * recovery_factor
                
            async def health_check(self):
                """Perform system health check."""
                failed_services = [name for name, service in self.services.items() if service["status"] == "failed"]
                
                if failed_services:
                    raise Exception(f"Health check failed - services down: {failed_services}")
    ...
    async def test_comprehensive_chaos_scenario(
        ...
        for result in results:
            assert result.failure_injected, f"Failure not injected in {result.experiment_name}"
            assert result.system_recovered, f"System did not recover in {result.experiment_name}"
    ...
    async def test_real_world_failure_simulation(
        ...
        await integrated_system.inject_failure(FailureType.MEMORY_EXHAUSTION, "embedding_service")
        
        # Monitor system adaptation
        phase1_metrics = await integrated_system.get_metrics()
        assert phase1_metrics["resources"]["memory"]["used"] > baseline_metrics["resources"]["memory"]["used"]
        
        # Phase 2: Add network latency
        await integrated_system.inject_failure(FailureType.NETWORK_TIMEOUT, "search_service")
        ...
        assert degraded_metrics["system_metrics"]["error_rate"] > baseline_metrics["system_metrics"]["error_rate"]
        ...
        assert recovery_metrics["system_metrics"]["throughput"] > degraded_metrics["system_metrics"]["throughput"]
        assert recovery_metrics["system_metrics"]["error_rate"] < degraded_metrics["system_metrics"]["error_rate"]
    ...
    async def test_circuit_breaker_integration(
        ...
        await integrated_system.inject_failure(FailureType.SERVICE_UNAVAILABLE, "vector_db")
        ...
        assert circuit_result["circuit_breaker_triggered"], "Circuit breaker should be triggered"
    ...
    async def test_system_recovery_validation(
        ...
        await integrated_system.inject_failure(FailureType.SERVICE_UNAVAILABLE, "auth_service")
        await integrated_system.inject_failure(FailureType.MEMORY_EXHAUSTION, "embedding_service")
        ...
        try:
            ...
            assert False, "Health check should fail with multiple failures"
        except Exception:
            # Expected failure
            pass
    ...
    async def test_data_consistency_during_chaos(
        self,
        integrated_system
    ):
        ...
        assert during_failure_integrity, "Data integrity should be maintained during failure"
        ...
        assert post_recovery_integrity, "Data integrity should be maintained after recovery"
    ...
    async def test_performance_impact_measurement(
        ...
        await integrated_system.inject_failure(FailureType.NETWORK_TIMEOUT, "api_gateway")
        ...
        throughput_impact = (baseline_throughput - degraded_throughput) / baseline_throughput
        latency_impact = (degraded_latency - baseline_latency) / baseline_latency
        
        # Performance impact should be significant but not total
        assert 0.1 < throughput_impact < 0.9, f"Throughput impact should be 10-90%, was {throughput_impact:.2%}"
        assert latency_impact > 0.1, f"Latency impact should be >10%, was {latency_impact:.2%}"
        ...
        assert recovered_throughput > degraded_throughput, "Performance should improve after recovery"

    async def test_chaos_test_automation(
        self,
        integrated_system
    ):
        ...
        assert suite_result["successful_experiments"] >= 1  # At least some should succeed
    ...
    async def test_chaos_monitoring_integration(
        self,
        integrated_system
    ):
        ...
        async def monitoring_callback():
            ...
            event = {
                "timestamp": time.time(),
                "system_health": "healthy" if len(integrated_system.active_failures) == 0 else "degraded",
        ...
        failure_events = [e for e in monitoring_events if e["system_health"] == "degraded"]
        recovery_events = [e for e in monitoring_events if e["system_health"] == "healthy"]
        ...
        failure_event = next((e for e in monitoring_events if len(e["active_failures"]) > 0), None)
        recovery_event = monitoring_events[-1]
        
        if failure_event:
            assert failure_event["throughput"] < baseline_event["throughput"], "Throughput should decrease during failure"
            assert failure_event["error_rate"] > baseline_event["error_rate"], "Error rate should increase during failure"
