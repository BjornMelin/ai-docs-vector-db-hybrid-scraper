...
class ChaosTestRunner:
    ...
    async def execute_experiment(
        ...
        try:
            ...
            if execution.retry_count < execution.max_retries:
                ...
                return await self.execute_experiment(experiment, target_system, monitoring_callback)
        ...
        try:
            if suite.parallel_execution:
                ...
                experiment_executions = await self._execute_parallel(
                    suite.experiments, 
                    target_system,
                    suite.max_concurrent_experiments
                )
            else:
    ...
    async def _execute_sequential(
        ...
        for experiment in experiments:
            ...
            if execution.status == ExperimentStatus.FAILED:
                failure_rate = len([e for e in executions if e.status == ExperimentStatus.FAILED]) / len(executions)
    ...
    async def _run_experiment_implementation(
        ...
        try:
            ...
            result.system_recovered = await self._validate_system_recovery(target_system)
    ...
    async def _perform_safety_checks(self, experiment: ChaosExperiment) -> Dict[str, Any]:
        ...
        return {
            ...
            "reason": "; ".join([check["reason"] for check in unsafe_checks]) if unsafe_checks else None,
    ...
    async def _inject_experiment_failure(self, experiment: ChaosExperiment, target_system: Any):
        """Inject failure based on experiment configuration."""
        # Mock failure injection
        if hasattr(target_system, 'inject_failure'):
            await target_system.inject_failure(experiment.failure_type, experiment.target_service)
    
    async def _stop_experiment_failure(self, experiment: ChaosExperiment, target_system: Any):
    ...
    def generate_report(self, suite_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        successful_experiments = len([e for e in executions if e.status == ExperimentStatus.COMPLETED])
        failed_experiments = len([e for e in executions if e.status == ExperimentStatus.FAILED])
        ...
        recovery_times = [
            e.result.recovery_time for e in executions 
    ...
    def _generate_recommendations(self, executions: List[ExperimentExecution]) -> List[str]:
        ...
        if len(failed_experiments) > len(executions) * 0.3:  # More than 30% failures
            recommendations.append("High failure rate detected - review system resilience mechanisms")
        ...
        for execution in executions:
            if execution.result and execution.result.success_criteria_met:
                for i, met in enumerate(execution.result.success_criteria_met):
                    if not met:
                        criteria_failures.append(execution.experiment.success_criteria[i])
        
        if criteria_failures:
            most_common_failure = max(set(criteria_failures), key=criteria_failures.count)
            recommendations.append(f"Success criteria '{most_common_failure}' frequently fails - investigate and improve")
        
        if not recommendations:
            recommendations.append("System demonstrates good resilience - continue regular chaos testing")
...
class TestChaosRunner:
    ...
    async def test_experiment_retry_mechanism(
        self,
        chaos_runner,
        mock_target_system
    ):
