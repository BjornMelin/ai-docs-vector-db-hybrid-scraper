    ...
    class FaultInjector:
        ...
        async def inject_network_timeout(
            ...
            async def timeout_fault():
                if random.random() < failure_rate:
                    ...
                    raise asyncio.TimeoutError(f"Simulated timeout for {target}")
        ...
        async def inject_memory_pressure(
            ...
            async def memory_fault():
                if random.random() < failure_rate:
                    raise MemoryError(f"Simulated memory pressure ({pressure_level}) for {target}")
...
def resilience_validator():
    ...
    class ResilienceValidator:
        ...
        def calculate_resilience_score(
            ...
            if cb_results:
                cb_triggered = sum(1 for r in cb_results if r["circuit_breaker_triggered"])
                cb_recovered = sum(1 for r in cb_results if r.get("recovery_successful", False))
                scores["circuit_breaker"] = (cb_triggered + cb_recovered) / (len(cb_results) * 2) * 100
            
            # Retry behavior scoring
            retry_results = [r for r in test_results if "retry_attempts" in r]
            if retry_results:
                successful_retries = sum(1 for r in retry_results if r.get("success_on_retry", False))
                proper_backoff = sum(1 for r in retry_results if r.get("backoff_respected", False))
                scores["retry_behavior"] = (successful_retries + proper_backoff) / (len(retry_results) * 2) * 100
            
            # Graceful degradation scoring
            degradation_results = [r for r in test_results if "fallback_triggered" in r]
            if degradation_results:
                successful_fallbacks = sum(1 for r in degradation_results if r.get("fallback_successful", False))
                scores["graceful_degradation"] = successful_fallbacks / len(degradation_results) * 100
            
            # Recovery time scoring
            recovery_results = [r for r in test_results if "recovery_time" in r]
            if recovery_results:
                avg_recovery_time = sum(r["recovery_time"] or 60.0 for r in recovery_results) / len(recovery_results)
...
def chaos_experiment_runner():
    ...
    class ChaosExperimentRunner:
        ...
        async def run_experiment(
            ...
            experiment: ChaosExperiment,
            target_system: Any,
            monitoring_func: Optional[Callable] = None
        ) -> ExperimentResult:
            ...
            try:
                ...
                result.system_recovered = await self._check_system_recovery(target_system)
        ...
        async def _inject_failure(self, experiment: ChaosExperiment, target_system: Any):
            ...
            if experiment.failure_type == FailureType.NETWORK_TIMEOUT:
                ...
                if hasattr(target_system, 'simulate_network_timeout'):
                    await target_system.simulate_network_timeout(experiment.duration_seconds)
        ...
        async def _stop_failure_injection(self, experiment: ChaosExperiment, target_system: Any):
            """Stop failure injection."""
        ...
        def get_experiment_summary(self, results: List[ExperimentResult]) -> Dict[str, Any]:
            ...
            return {
                ...
                "average_recovery_time": sum(r.recovery_time for r in results) / len(results),
                "success_rate": sum(1 for r in results if all(r.success_criteria_met)) / len(results),
