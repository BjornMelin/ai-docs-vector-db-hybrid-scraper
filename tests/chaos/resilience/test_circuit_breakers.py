...
class TestCircuitBreakers:
    ...
    async def test_circuit_breaker_with_different_failure_rates(
        ...
        for i in range(50):
            try:
                result = await circuit_breaker.call(intermittent_service)
                successes += 1
    ...
    async def test_circuit_breaker_timeout_recovery(
        self,
        circuit_breaker_config
    ):
    ...
    async def test_circuit_breaker_with_different_exception_types(
        ...
        async def service_with_different_errors():
            ...
            if call_count == 1:
                ...
                raise ConnectionError("Another connection error - should trigger circuit breaker")
            elif call_count == 4:
                raise TimeoutError("Timeout error - should not count when circuit is open")
            else:
    ...
    async def test_circuit_breaker_metrics_and_monitoring(
        ...
        assert failure_stats["failure_count"] == circuit_breaker_config.failure_threshold
    ...
    async def test_multiple_circuit_breakers(
        ...
        db_circuit = CircuitBreaker(CircuitBreakerConfig(
            failure_threshold=2,
            timeout=0.1,
            expected_exception=ConnectionError
        ))
        
        api_circuit = CircuitBreaker(CircuitBreakerConfig(
            failure_threshold=3,
            timeout=0.15,
            expected_exception=Exception
        ))
        
        cache_circuit = CircuitBreaker(CircuitBreakerConfig(
            failure_threshold=5,
            timeout=0.05,
    ...
    async def test_circuit_breaker_with_retry_logic(
        self,
        circuit_breaker_config
    ):
    ...
    async def test_circuit_breaker_bulkhead_pattern(
        self
    ):
        """Test circuit breaker with bulkhead pattern for resource isolation."""
        # Different circuit breakers for different resource pools
        critical_pool_circuit = CircuitBreaker(CircuitBreakerConfig(
            failure_threshold=2,
            timeout=0.1,
            expected_exception=Exception
        ))
        
        normal_pool_circuit = CircuitBreaker(CircuitBreakerConfig(
            failure_threshold=5,
            timeout=0.2,
            expected_exception=Exception
        ))
    ...
    async def test_circuit_breaker_adaptive_thresholds(
        self
    ):
        ...
        class AdaptiveCircuitBreaker(CircuitBreaker):
            ...
            async def _on_failure(self):
                ...
                if (self.state in [CircuitState.CLOSED, CircuitState.HALF_OPEN] and
                    self.failure_count >= adaptive_threshold):
                    self.state = CircuitState.OPEN
        
        adaptive_circuit = AdaptiveCircuitBreaker(CircuitBreakerConfig(
