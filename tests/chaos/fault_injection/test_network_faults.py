...
class TestNetworkFaultInjection:
    ...
    async def test_partial_network_failure(
        ...
        assert 0.6 <= actual_success_rate <= 0.8, f"Success rate {actual_success_rate} outside expected range"
        
        # Clean up
        fault_injector.remove_fault(fault_id)

    async def test_latency_spike_injection(
        self, 
        fault_injector
    ):
    ...
    async def test_temporary_fault_context_manager(
        self, 
        fault_injector
    ):
        ...
        for fault_type, kwargs in fault_types:
            async with fault_injector.temporary_fault(
                ...
                **kwargs
                ...
                if fault_type in ["network_timeout", "connection_failure", "service_unavailable"]:
    ...
    async def test_concurrent_fault_injection(
        self, 
        fault_injector
    ):
        ...
        fault_ids.append(await fault_injector.inject_network_timeout(
            target="service_a", timeout_seconds=0.1
        ))
        
        # Connection failure on service B
        fault_ids.append(await fault_injector.inject_connection_failure(
            target="service_b"
        ))
        
        # Latency spike on service C
        fault_ids.append(await fault_injector.inject_latency_spike(
        ...
        for fault_id in fault_ids:
            ...
            if fault["type"] in [FailureType.NETWORK_TIMEOUT, FailureType.CONNECTION_REFUSED]:
                with pytest.raises(Exception):
    ...
    async def test_network_fault_with_circuit_breaker(
        ...
        assert result["circuit_breaker_triggered"], "Circuit breaker should have been triggered"
        assert result["failure_count"] >= 3, f"Expected >= 3 failures, got {result['failure_count']}"
        assert circuit_breaker_triggered, "Circuit breaker flag should be set"

    @pytest.mark.parametrize("failure_rate", [0.1, 0.5, 0.9])
    async def test_variable_failure_rates(
        self, 
        fault_injector,
        failure_rate: float
    ):
...
class TestNetworkChaosWithRealServices:
    ...
    async def test_vector_db_connection_failure_scenario(
        ...
        mock_vector_db_service.search.side_effect = ConnectionError("DB connection failed")
        mock_vector_db_service.health_check.side_effect = ConnectionError("Health check failed")
    ...
    async def test_embedding_service_timeout_with_retry(
        ...
        async def unreliable_embedding_service():
            ...
            if call_count <= 2:
                raise asyncio.TimeoutError("Embedding service timeout")
        ...
        assert result["retry_attempts"] == 3, f"Expected 3 attempts, got {result['retry_attempts']}"
    ...
    async def test_cascade_failure_prevention(
        ...
        assert embedding_result == [0.1, 0.2, 0.3], "Embedding service should remain functional"
        ...
        assert circuit_result["circuit_breaker_triggered"], "Circuit breaker should prevent cascade"
    ...
    async def test_bandwidth_limitation_simulation(
        self, 
        fault_injector
    ):
        ...
        assert elapsed_time >= 0.01, "Operation should be slowed by bandwidth limitation"
        assert result["transferred"] == "10MB"

    async def test_dns_failure_simulation(
        self, 
        fault_injector
    ):
