...
class TestServiceFaultInjection:
    ...
    async def test_memory_exhaustion_injection(
        self, 
        fault_injector
    ):
    ...
    async def test_database_connection_failure(
        ...
        tasks = []
        for i in range(5):  # Exceed max connections
    ...
    async def test_cache_service_failures(
        self, 
        fault_injector,
        resilience_validator
    ):
        ...
        assert all(result.startswith(("cached_value", "db_value")) for result in results)

    async def test_api_rate_limiting_failure(
        self, 
        fault_injector
    ):
        ...
        assert successes <= rate_limit, f"Too many successes: {successes} > {rate_limit}"
    ...
    async def test_data_corruption_simulation(
        self, 
        fault_injector
    ):
        ...
        for i in range(20):
            try:
                ...
                validated_data = await data_validator(processed_data)
    ...
    async def test_service_startup_failure(
        self, 
        fault_injector
    ):
...
class TestAdvancedServiceFaults:
    ...
    async def test_resource_starvation(
        self, 
        fault_injector
    ):
        ...
        async def resource_intensive_operation(cpu_required: int, memory_required: int):
            ...
            if (cpu_pool["used"] + cpu_required > cpu_pool["available"] or
                memory_pool["used"] + memory_required > memory_pool["available"]):
            ...
            try:
                # Simulate work
                await asyncio.sleep(0.01)
                return {"status": "completed", "resources_used": {
                    "cpu": cpu_required, "memory": memory_required
                }}
            finally:
                # Release resources
                cpu_pool["used"] -= cpu_required
    ...
    async def test_jitter_injection(
        self, 
        fault_injector
    ):
