...
class TestContractIntegration:
    ...
    async def test_full_contract_validation_pipeline(self, 
                                                   client_manager,
        ...
        schema_validation = json_schema_validator.validate_data(valid_search_result, "search_result")
        assert schema_validation["valid"], f"Schema validation failed: {schema_validation['errors']}"
        ...
        assert spec_validation["valid"], f"OpenAPI validation failed: {spec_validation['errors']}"
        ...
        pact_contract_builder.will_respond_with(
            status=200,
            body={"results": [valid_search_result]}
        )
    ...
    @pytest.mark.contract
    @pytest.mark.integration
    async def test_contract_violation_detection_pipeline(self,
                                                        api_contract_validator,
                                                        json_schema_validator,
                                                        contract_test_data):
        ...
        schema_validation = json_schema_validator.validate_data(invalid_result, "search_result")
        ...
        invalid_request_validation = api_contract_validator.validate_request(
            "/api/search", "GET", params={}  # Missing required 'q' parameter
        )
    ...
    @pytest.mark.contract
    @pytest.mark.integration
    async def test_cross_service_contract_validation(self,
                                                   api_contract_validator,
                                                   json_schema_validator):
        ...
        api_contract_validator.register_contract("/embeddings", embedding_service_contract)
        ...
        vector_request = {
            ...
            "vectors": [
                ...
                {
                    "id": "doc2",
                    "vector": embedding_response["embeddings"][1]
                }
            ]
        }
        ...
        v2_search_contract = {
            "GET": {
                "parameters": [
                    ...
                    {"name": "max_results", "type": "integer", "required": False, "deprecated": True},
    ...
    def test_contract_compatibility_matrix(self, api_contract_validator):
        ...
        for scenario in compatibility_scenarios:
            ...
            if scenario["expected_compatible"]:
                assert validation_result["valid"], f"Scenario {scenario['name']} should be compatible"
            else:
...
class TestContractPerformanceValidation:
    ...
    def test_contract_validation_performance(self, api_contract_validator, json_schema_validator):
        ...
        large_schema = {
            ...
            "required": [f"field_{i}" for i in range(0, 100, 10)]  # Every 10th field required
        }
        ...
        assert throughput > 100, f"Validation throughput too low: {throughput} validations/sec"
    ...
    def test_concurrent_contract_validation(self, api_contract_validator):
        ...
        assert concurrent_time < 5.0, f"Concurrent validation took too long: {concurrent_time}s"
