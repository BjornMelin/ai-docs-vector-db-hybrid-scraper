...
class TestAPIEndpointContracts:
    ...
    def test_search_endpoint_contract(self, client, api_contract_validator, contract_test_data):
        ...
        assert request_validation["valid"], f"Request validation failed: {request_validation['errors']}"
        ...
        assert response_validation["valid"], f"Response validation failed: {response_validation['errors']}"

    @pytest.mark.api_contract
    def test_search_endpoint_error_contract(self, client, api_contract_validator, contract_test_data):
        ...
        assert response_validation["valid"], f"Error response validation failed: {response_validation['errors']}"
    ...
    @pytest.mark.api_contract
    def test_documents_endpoint_contract(self, client, api_contract_validator, contract_test_data):
        """Test documents endpoint contract validation."""
        # Register contract
        api_contract_validator.register_contract(
            "/api/documents", 
            contract_test_data["api_contracts"]["/api/documents"]
        )
    ...
    def test_api_version_compatibility(self, api_contract_validator):
        ...
        v2_contract = {
            "GET": {
                "parameters": [
                    {"name": "q", "type": "string", "required": True},
                    {"name": "strategy", "type": "string", "required": False}  # New optional param
                ],
                "responses": {
                    "200": {
                        "schema": {
                            ...
                            "properties": {
                                ...
                                "results": {"type": "array"},
                                "total_count": {"type": "integer"},  # Renamed from 'total'
...
class TestCrossServiceContracts:
    ...
    def test_vector_db_service_contract(self, api_contract_validator):
        ...
        api_contract_validator.register_contract("/api/vectors/upsert", vector_db_contract)
...
class TestContractDocumentation:
    ...
    @pytest.mark.api_contract
    def test_generate_contract_documentation(self, openapi_contract_manager, contract_test_data):
        ...
        for endpoint in endpoints:
            ...
            success_responses = [
                status for status in responses.keys() 
                if status.startswith("2")
            ]
            assert len(success_responses) > 0

    @pytest.mark.api_contract
    def test_contract_test_generation(self, openapi_contract_manager, contract_test_data):
