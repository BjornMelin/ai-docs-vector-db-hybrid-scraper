    ...
    class JSONSchemaValidator:
        ...
        def _generate_valid_data(self, schema: Dict[str, Any]) -> Any:
            ...
            if schema_type == "object":
                ...
                return [self._generate_valid_data(items_schema) for _ in range(min_items)]
                ...
                if required:
                    ...
                    for prop in required[1:]:
                        if prop in properties:
                            incomplete_data[prop] = self._generate_valid_data(properties[prop])
    ...
    class APIContractValidator:
        ...
        def validate_response(
            ...
            if not method_contract:
                return {
                    "valid": False,
                    "errors": [f"Method {method} not in contract for endpoint {endpoint}"],
                }
        ...
        def _validate_request_body(self, body: Any, body_contract: Dict[str, Any]) -> List[str]:
            ...
            if "required" in body_contract and body_contract["required"] and body is None:
                errors.append("Request body is required but not provided")
            
            if body is not None and "schema" in body_contract:
                validator = JSONSchemaValidator()
                result = validator.validate_against_schema(body, body_contract["schema"])
                if not result["valid"]:
        ...
        def _validate_response_headers(self, headers: Dict[str, str], header_contracts: Dict[str, Any]) -> List[str]:
            ...
            for header_name, header_contract in header_contracts.items():
                ...
                if required and header_name.lower() not in [h.lower() for h in headers.keys()]:
                    errors.append(f"Required response header '{header_name}' is missing")
                ...
                for method, operation in path_item.items():
                    if method.lower() in ["get", "post", "put", "delete", "patch", "head", "options"]:
                        if not isinstance(operation, dict):
                            ...
                            errors.append(f"Operation {method} in path {path} must have responses")
    ...
    class PactContractBuilder:
        ...
        def verify_interaction(self, actual_request: Dict[str, Any], actual_response: Dict[str, Any]) -> Dict[str, Any]:
            ...
            for interaction in self.interactions:
                request_match = self._match_request(actual_request, interaction.get("request", {}))
                response_match = self._match_response(actual_response, interaction.get("response", {}))
        ...
        def _match_request(self, actual: Dict[str, Any], expected: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if "method" in expected and actual.get("method") != expected["method"]:
                errors.append(f"Method mismatch: expected {expected['method']}, got {actual.get('method')}")
            
            # Check path
            if "path" in expected and actual.get("path") != expected["path"]:
                errors.append(f"Path mismatch: expected {expected['path']}, got {actual.get('path')}")
        ...
        def _match_response(self, actual: Dict[str, Any], expected: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if "status" in expected and actual.get("status") != expected["status"]:
                errors.append(f"Status mismatch: expected {expected['status']}, got {actual.get('status')}")
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "pact: mark test as Pact contract test"
    )
    config.addinivalue_line(
        "markers", "openapi: mark test as OpenAPI contract test"
    )
    config.addinivalue_line(
