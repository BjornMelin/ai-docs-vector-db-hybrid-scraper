...
def mock_crawl4ai_service():
    """Mock Crawl4AI service with  features."""
    service = MagicMock()
    ...
    async def mock_crawl(url, **kwargs):
        return {
            ...
            "metadata": {"title": f"Page: {url}", "crawl_time": 0.5, "word_count": 100},
    ...
    service.batch_crawl = AsyncMock(
        side_effect=lambda urls, **kw: [mock_crawl(url, **kw) for url in urls]
    )
