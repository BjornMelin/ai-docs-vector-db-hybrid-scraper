...
class DataQualityRule:
    ...
    name: str
    description: str
    rule_type: str  # "completeness", "validity", "consistency", "accuracy", "uniqueness"
    ...
    class DataQualityValidator:
        ...
        def validate_completeness(self, data: List[Dict[str, Any]], rule: DataQualityRule) -> DataQualityResult:
            ...
            if rule.field_name:
                ...
                failed_records = sum(
                    ...
                    if record.get(rule.field_name) is None or record.get(rule.field_name) == ""
                )
            else:
                ...
                if rule.field_name and rule.field_name in record:
                    ...
                    if "expected_type" in rule.reference_data:
                        ...
                        if not isinstance(value, expected_type):
                            failed_records += 1
                            validation_errors.append(f"Type mismatch: expected {expected_type}, got {type(value)}")
                            continue
                    ...
                    if rule.pattern and isinstance(value, str):
                        ...
                        if not re.match(rule.pattern, value):
                            failed_records += 1
                            validation_errors.append(f"Pattern mismatch: {value} does not match {rule.pattern}")
                            continue
                    
                    # Range validation
                    if "min_value" in rule.reference_data or "max_value" in rule.reference_data:
                        if isinstance(value, (int, float)):
                            ...
                            if min_val is not None and value < min_val:
                                failed_records += 1
                                validation_errors.append(f"Value {value} below minimum {min_val}")
                                continue
                            
                            if max_val is not None and value > max_val:
                                failed_records += 1
                                validation_errors.append(f"Value {value} above maximum {max_val}")
                                continue
        ...
        def validate_uniqueness(self, data: List[Dict[str, Any]], rule: DataQualityRule) -> DataQualityResult:
            ...
            if rule.field_name:
                ...
                values = [record.get(rule.field_name) for record in data if rule.field_name in record]
                ...
                for record in data:
                    for consistency_rule in rule.reference_data["consistency_rules"]:
                        ...
                        if rule_type == "field_relationship":
                            ...
                            relationship = consistency_rule.get("relationship")  # "greater", "equal", "less"
                            ...
                            if field1 in record and field2 in record:
                                ...
                                if relationship == "greater" and val1 <= val2:
                                    failed_records += 1
                                    consistency_errors.append(f"{field1} ({val1}) should be greater than {field2} ({val2})")
                                elif relationship == "equal" and val1 != val2:
                                    failed_records += 1
                                    consistency_errors.append(f"{field1} ({val1}) should equal {field2} ({val2})")
                                elif relationship == "less" and val1 >= val2:
                                    failed_records += 1
                                    consistency_errors.append(f"{field1} ({val1}) should be less than {field2} ({val2})")
        ...
        def generate_data_profile(self, data: List[Dict[str, Any]], dataset_name: str) -> Dict[str, Any]:
            ...
            for field in all_fields:
                ...
                field_profile = {
                    ...
                    "null_rate": (len(field_values) - len(non_null_values)) / max(len(field_values), 1),
                    "unique_count": len(set(non_null_values)),
                    "uniqueness_rate": len(set(non_null_values)) / max(len(non_null_values), 1),
                }
                
                # Type analysis
                types = [type(v).__name__ for v in non_null_values]
                field_profile["data_types"] = {t: types.count(t) for t in set(types)}
                field_profile["dominant_type"] = max(field_profile["data_types"], key=field_profile["data_types"].get) if types else None
            ...
            profile = {
                ...
                "schema_consistency": {
                    "all_records_same_fields": all(set(record.keys()) == all_fields for record in data),
...
def data_integrity_checker():
    ...
    class DataIntegrityChecker:
        ...
        def check_referential_integrity(self, data: List[Dict[str, Any]], reference_data: List[Dict[str, Any]], check: DataIntegrityCheck) -> Dict[str, Any]:
            ...
            reference_values = {record.get(reference_key) for record in reference_data if reference_key in record}
            ...
            for record in data:
                if foreign_key in record:
                    ...
                    if foreign_value not in reference_values and foreign_value is not None:
                        violations.append({
                            "record": record,
        ...
        def run_all_checks(self, data: List[Dict[str, Any]], reference_datasets: Dict[str, List[Dict[str, Any]]] = None) -> List[Dict[str, Any]]:
            ...
            for check in self.checks.values():
                if check.check_type == "referential":
                    ...
                    result = self.check_referential_integrity(data, reference_data, check)
        ...
        def generate_document_records(self, count: int = 100, introduce_quality_issues: bool = False) -> List[Dict[str, Any]]:
            ...
            for i in range(count):
                record = {
                    ...
                    "title": f"Document {i}",
                    "content": f"This is the content of document {i}. " * random.randint(5, 20),
                    "created_at": datetime.now() - timedelta(days=random.randint(0, 365)),
                    "updated_at": datetime.now() - timedelta(days=random.randint(0, 30)),
                    "collection": random.choice(["tech", "science", "business", "general"]),
                    ...
                    "metadata": {
                        ...
                        "language": "en",
                        "tags": random.sample(["ai", "ml", "data", "tech", "science"], random.randint(1, 3)),
                    }
                }
                
                # Introduce quality issues if requested
                if introduce_quality_issues:
                    issue_type = random.choice(["missing_field", "invalid_format", "duplicate", "inconsistent"])
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "data_quality: mark test as data quality test"
    )
    config.addinivalue_line(
