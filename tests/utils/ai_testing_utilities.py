...
class RAGTestUtils:
    ...
    def calculate_contextual_precision(
        ...
        for retrieved in retrieved_contexts:
            for ground_truth in ground_truth_contexts:
                ...
                similarity = len(set(retrieved.lower().split()) & 
                              set(ground_truth.lower().split())) / \
                            max(len(set(retrieved.lower().split())), 
    ...
    def evaluate_rag_response_quality(
        ...
        if context_words:
            context_utilization = len(response_words & context_words) / len(context_words)
...
class AITestStrategies:
    ...
    def search_queries():
        ...
        question_queries = st.builds(
            lambda text: f"What is {text}?",
            st.text(min_size=3, max_size=50)
        )
        
        how_queries = st.builds(
            lambda text: f"How to {text}?",
            st.text(min_size=3, max_size=50)
        )
