...
def create_mock_embedding_service(
    ...
    async def mock_embed_batch(texts: list[str]) -> list[List[float]]:
        ...
        return [
            generator._generate_normalized_vector(dimension) 
            for _ in texts
        ]
...
def create_mock_web_scraper(
    ...
    async def mock_scrape_url(url: str, **kwargs) -> dict[str, Any]:
        ...
        return {
            ...
            "links": [
                generator.fake.url() for _ in range(random.randint(0, 10))
            ],
            "images": [
                generator.fake.image_url() for _ in range(random.randint(0, 5))
            ],
        }
...
def create_mock_cache_service(
    ...
    def mock_set(key: str, value: Any, ttl: Optional[int] = None) -> bool:
        """Mock cache set."""
...
def create_mock_api_client(
    ...
    async def mock_request(
        ...
        if endpoint.startswith("/search"):
            return {
                "results": [
                    {"id": str(uuid.uuid4()), "title": f"Result {i}", "score": 0.9 - i * 0.1}
