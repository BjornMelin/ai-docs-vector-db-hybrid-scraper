...
def assert_pagination_response(
    ...
    assert isinstance(response[per_page_key], int), f"'{per_page_key}' must be an integer"
...
class AssertionHelpers:
    ...
    def assert_search_result(result: Dict[str, Any], min_score: float = 0.0) -> None:
        ...
        assert not missing_fields, f"Search result missing required fields: {missing_fields}"
    ...
    def assert_configuration_valid(config: Dict[str, Any], required_sections: List[str]) -> None:
        ...
        assert not missing_sections, f"Configuration missing required sections: {missing_sections}"
    ...
    def assert_timestamp_recent(
        ...
        assert age_seconds >= -1.0, "Timestamp cannot be in the future (allowing 1s clock skew)"
...
def assert_successful_response(
    ...
    assert response.get("success") is True, f"Response should indicate success, got: {response}"
...
def assert_valid_vector_point(
    ...
    assert all(isinstance(x, (int, float)) for x in vector), "Vector must contain numeric values"
...
def assert_mock_called_with_pattern(
    ...
    for i, expected_call in enumerate(expected_calls):
        if i >= len(actual_calls):
            assert False, f"Missing expected call {i + 1}: {expected_call}"
...
async def assert_async_operation_completes(
    ...
    try:
        ...
        assert False, f"{operation_name} timed out after {timeout_seconds}s"
    except Exception as e:
        assert False, f"{operation_name} failed with error: {e}"
...
def assert_contract_compliance(
    ...
    try:
        ...
        assert False, f"Response doesn't match contract schema: {e.message}"
...
class assert_resource_cleanup:
    ...
    def __init__(self, resource_tracker: Callable[[], int], resource_name: str = "resource"):
