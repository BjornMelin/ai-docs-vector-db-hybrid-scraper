...
FORBIDDEN_PATTERNS = {
    ...
    "shared_state": re.compile(r"@pytest\.fixture.*scope=[\"']module[\"']|@pytest\.fixture.*scope=[\"']session[\"']"),
    "magic_values": re.compile(r"assert.*==\s*\d{4,}|assert.*==\s*[\"'][a-f0-9]{32,}[\"']"),
...
class TestQualityValidator:
            ...
            for pattern_name, pattern in FORBIDDEN_PATTERNS.items():
                ...
                if matches:
                    for match in matches:
                        ...
                        file_issues.append({
                            "type": "naming" if pattern_name in ["enhanced", "modern", "advanced"] else "patterns",
                            "severity": "error",
    ...
    def _check_mock_complexity(self, content: str, file_path: Path) -> List[Dict[str, any]]:
        """Check for excessive mocking complexity."""
        issues = []
        
        # Count mock depth (nested mocks)
        mock_pattern = re.compile(r"Mock\(|MagicMock\(|patch\(|mock\.")
        mock_lines = [i for i, line in enumerate(content.splitlines(), 1) if mock_pattern.search(line)]
    ...
    def _check_function_size(self, content: str, file_path: Path) -> List[Dict[str, any]]:
        ...
        for match in function_pattern.finditer(content):
            ...
            for i in range(start_line, len(lines)):
                line = lines[i]
                if line.strip() and not line.startswith(' ' * (indent + 1)) and i > start_line:
                    end_line = i
                    break
            else:
                end_line = len(lines)
