...
def system_monitor():
    ...
    class SystemMonitor:
        ...
        def get_metrics_delta(self) -> Optional[PerformanceMetrics]:
            ...
            return PerformanceMetrics(
                ...
                disk_io_read_mb=current.disk_io_read_mb - self.baseline_metrics.disk_io_read_mb,
                disk_io_write_mb=current.disk_io_write_mb - self.baseline_metrics.disk_io_write_mb,
                network_sent_mb=current.network_sent_mb - self.baseline_metrics.network_sent_mb,
                network_recv_mb=current.network_recv_mb - self.baseline_metrics.network_recv_mb,
        ...
        def get_average_metrics(self) -> Optional[PerformanceMetrics]:
            ...
            return PerformanceMetrics(
                ...
                memory_percent=sum(m.memory_percent for m in self.metrics_history) / count,
                disk_io_read_mb=sum(m.disk_io_read_mb for m in self.metrics_history) / count,
                disk_io_write_mb=sum(m.disk_io_write_mb for m in self.metrics_history) / count,
                network_sent_mb=sum(m.network_sent_mb for m in self.metrics_history) / count,
                network_recv_mb=sum(m.network_recv_mb for m in self.metrics_history) / count,
                response_time_ms=sum(m.response_time_ms for m in self.metrics_history) / count,
                throughput_ops_per_sec=sum(m.throughput_ops_per_sec for m in self.metrics_history) / count,
                timestamp=time.time(),
            )
...
def throughput_calculator():
    ...
    class ThroughputCalculator:
        ...
        def calculate_throughput(self, operation: str) -> Optional[float]:
            """Calculate operations per second."""
            if operation not in self.start_times or operation not in self.operation_counts:
                return None
        ...
        def check_resource_usage(self) -> Dict[str, Any]:
            ...
            return {
                "open_files": {
                    ...
                    "usage_percent": (open_files / self.initial_limits["max_open_files"]) * 100,
                },
                "memory": {
                    "current_mb": memory_info.rss / (1024 * 1024),
                    "limit_mb": self.initial_limits["max_memory"] / (1024 * 1024) if self.initial_limits["max_memory"] != -1 else "unlimited",
                },
                "cpu_time": {
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "cpu: mark test as CPU performance test"
    )
    config.addinivalue_line(
        "markers", "network: mark test as network performance test"
    )
    config.addinivalue_line(
        "markers", "database: mark test as database performance test"
    )
    config.addinivalue_line(
        "markers", "api_latency: mark test as API latency test"
    )
    config.addinivalue_line(
