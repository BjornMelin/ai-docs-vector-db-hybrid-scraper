...
from src.config.reload import ConfigReloader, ReloadTrigger, ReloadStatus
...
class TestConfigReloader:
    ...
    async def test_configuration_rollback(self, reloader: ConfigReloader):
        ...
        configs = [
            Config(app_name=f"App v{i}", version=f"1.0.{i}")
            for i in range(3)
        ]
...
class TestConfigurationAPI:
    ...
    def test_reload_endpoint(self, client: TestClient, mock_reloader: Mock):
        ...
        from src.config.reload import ReloadOperation, ReloadTrigger, ReloadStatus
