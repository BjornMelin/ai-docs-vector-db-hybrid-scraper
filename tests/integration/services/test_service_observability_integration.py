...
class TestDistributedTracing:
    ...
    async def test_end_to_end_request_tracing(self, tracing_infrastructure):
        ...
        embedding_span = trace_context.start_span("embedding_service", "generate_embeddings", api_span.span_id)
        await asyncio.sleep(0.02)  # Simulate embedding generation
        trace_context.finish_span(embedding_span, {
        ...
        vector_db_span = trace_context.start_span("vector_db_service", "hybrid_search", api_span.span_id)
        await asyncio.sleep(0.015)  # Simulate search
        trace_context.finish_span(vector_db_span, {
        ...
        content_span = trace_context.start_span("content_intelligence", "analyze_results", api_span.span_id)
        await asyncio.sleep(0.008)  # Simulate analysis
        trace_context.finish_span(content_span, {
        ...
        # Finish root span
        trace_context.finish_span(api_span, {
            "http.method": "POST",
        ...
        # Verify tags
        embedding_span_found = next(s for s in spans if s.service_name == "embedding_service")
        assert embedding_span_found.tags["embedding.model"] == "text-embedding-3-small"
        
        vector_span_found = next(s for s in spans if s.service_name == "vector_db_service")
    ...
    async def test_error_tracking_in_traces(self, tracing_infrastructure):
        ...
        class ErrorTrackingTracer:
            ...
            def record_error(self, span: TraceSpan, error: Exception, error_context: Dict = None):
                ...
                error_log = {
                    "timestamp": time.time(),
                    "level": "ERROR",
                    "message": f"Error in {span.operation_name}: {str(error)}",
        ...
        error_tracer.record_error(embedding_span, embedding_error, {
        ...
        error_tracer.record_error(vector_span, vector_error, {
...
class TestMetricsCollection:
    ...
    async def test_service_metrics_collection(self, metrics_infrastructure):
        ...
        api_collector.record_counter("http.requests.total", 1, {
            "service": "api_gateway",
            "method": "POST",
            "endpoint": "/search",
            "status_code": "200"
        })
        api_collector.record_histogram("http.request.duration", 0.125, {
            "service": "api_gateway",
            "method": "POST",
            "endpoint": "/search"
        })
        api_collector.record_gauge("http.active_connections", 15, {
            "service": "api_gateway"
        })
        
        # Simulate Embedding Service metrics
        embedding_collector.record_counter("embeddings.requests.total", 1, {
            "service": "embedding_service",
            "provider": "openai",
            "model": "text-embedding-3-small"
        })
        embedding_collector.record_histogram("embeddings.generation.duration", 0.089, {
            "service": "embedding_service",
            "provider": "openai"
        })
        embedding_collector.record_counter("embeddings.tokens.total", 150, {
            "service": "embedding_service",
            "provider": "openai"
        })
        embedding_collector.record_gauge("embeddings.cost.usd", 0.001, {
            "service": "embedding_service",
            "provider": "openai"
        })
        
        # Simulate Vector DB metrics
        vector_db_collector.record_counter("search.requests.total", 1, {
            "service": "vector_db_service",
            "collection": "documents",
            "search_type": "hybrid"
        })
        vector_db_collector.record_histogram("search.duration", 0.045, {
            "service": "vector_db_service",
            "collection": "documents"
        })
        vector_db_collector.record_gauge("search.results.count", 15, {
            "service": "vector_db_service",
            "collection": "documents"
        })
        vector_db_collector.record_gauge("vector_db.memory.usage", 0.67, {
            "service": "vector_db_service"
        })
        ...
        embedding_requests = [m for m in metrics_storage if m.name == "embeddings.requests.total"]
        assert len(embedding_requests) == 1
        assert embedding_requests[0].tags["provider"] == "openai"
        
        search_requests = [m for m in metrics_storage if m.name == "search.requests.total"]
    ...
    async def test_metrics_aggregation_and_alerting(self, metrics_infrastructure):
        ...
        sample_metrics = [
            Metric("http.requests.total", 100, time.time() - 60, {"service": "api_gateway"}, "counter"),
            Metric("http.requests.total", 150, time.time() - 30, {"service": "api_gateway"}, "counter"),
            Metric("http.requests.total", 200, time.time(), {"service": "api_gateway"}, "counter"),
            Metric("http.request.duration", 0.1, time.time() - 60, {"service": "api_gateway"}, "histogram"),
            Metric("http.request.duration", 0.5, time.time() - 30, {"service": "api_gateway"}, "histogram"),
            Metric("http.request.duration", 1.2, time.time(), {"service": "api_gateway"}, "histogram"),
            Metric("embeddings.errors.total", 0, time.time() - 60, {"service": "embedding_service"}, "counter"),
            Metric("embeddings.errors.total", 2, time.time() - 30, {"service": "embedding_service"}, "counter"),
            Metric("embeddings.errors.total", 5, time.time(), {"service": "embedding_service"}, "counter"),
        ]
        ...
        class MetricsAggregator:
            ...
            def calculate_rate(self, metric_name: str, time_window: float = 60) -> float:
                ...
                recent_metrics = [
                    ...
                    if m.name == metric_name and current_time - m.timestamp <= time_window
                ]
            ...
            def calculate_percentile(self, metric_name: str, percentile: float, time_window: float = 60) -> float:
                ...
                self.alert_rules = [
                    {
                        ...
                        "condition": lambda aggregator: aggregator.calculate_rate("http.requests.total") > 3.0,
                        "message": "High request rate detected"
                    },
                    {
                        "name": "high_latency",
                        "condition": lambda aggregator: aggregator.calculate_percentile("http.request.duration", 0.95) > 1.0,
                        "message": "High latency detected (P95 > 1s)"
                    },
                    {
                        "name": "embedding_errors",
                        "condition": lambda aggregator: aggregator.calculate_rate("embeddings.errors.total") > 0.1,
        ...
        assert "embedding_errors" in alert_names  # Should trigger due to error rate increase
    ...
    async def test_custom_business_metrics(self, metrics_infrastructure):
        ...
        class BusinessMetricsCollector:
            ...
            def record_search_quality(self, query: str, results_count: int, user_satisfaction: float):
                """Record search quality metrics."""
                self.business_metrics.extend([
                    Metric("search.quality.results_count", results_count, time.time(), 
                          {"query_type": self._classify_query(query)}, "gauge"),
                    Metric("search.quality.user_satisfaction", user_satisfaction, time.time(),
                          {"query_type": self._classify_query(query)}, "gauge")
                ])
            
            def record_content_processing(self, processing_time: float, quality_score: float, content_type: str):
                """Record content processing metrics."""
                self.business_metrics.extend([
                    Metric("content.processing.duration", processing_time, time.time(),
                          {"content_type": content_type}, "histogram"),
                    Metric("content.quality.score", quality_score, time.time(),
                          {"content_type": content_type}, "gauge")
                ])
            
            def record_user_engagement(self, session_duration: float, queries_per_session: int, bounce_rate: float):
                """Record user engagement metrics."""
                self.business_metrics.extend([
                    Metric("user.session.duration", session_duration, time.time(), {}, "histogram"),
                    Metric("user.queries.per_session", queries_per_session, time.time(), {}, "gauge"),
                    Metric("user.bounce_rate", bounce_rate, time.time(), {}, "gauge")
                ])
        ...
        business_collector.record_search_quality("machine learning algorithms", 15, 0.85)
        ...
        class BusinessMetricsAnalyzer:
            ...
            def analyze_search_quality_by_type(self) -> Dict:
                ...
                satisfaction_metrics = [m for m in self.metrics if m.name == "search.quality.user_satisfaction"]
            ...
            def analyze_content_quality_trends(self) -> Dict:
                ...
                quality_metrics = [m for m in self.metrics if m.name == "content.quality.score"]
...
class TestLogCorrelation:
    ...
    async def test_structured_logging_correlation(self):
        ...
        correlated_logs = [log for log in all_logs if log["correlation_id"] == correlation_id]
        ...
        embedding_logs = [log for log in all_logs if log["service"] == "embedding_service"]
        vector_db_logs = [log for log in all_logs if log["service"] == "vector_db_service"]
    ...
    async def test_error_correlation_and_debugging(self):
        ...
        class ErrorCorrelationManager:
            ...
            def record_error(self, error_id: str, service: str, error_type: str, 
                           message: str, correlation_id: str, context: Dict = None):
            ...
            def analyze_error_propagation(self, correlation_id: str) -> Dict:
                ...
                affected_services = list(set(error["service"] for error in error_chain))
        ...
        assert timeline[0]["context"]["api_endpoint"] == "https://api.openai.com/v1/embeddings"
...
class TestHealthMonitoring:
    ...
    async def test_service_health_monitoring(self):
        ...
        class ServiceHealthMonitor:
            ...
            def register_health_check(self, service: str, check_func, check_interval: float = 30):
            ...
            def get_overall_health(self) -> Dict:
                ...
                service_statuses = [config["status"] for config in self.health_checks.values()]
                ...
                return {
                    ...
                    "unhealthy_services": sum(1 for s in service_statuses if s == "error")
                }
        ...
        health_monitor.register_health_check("embedding_service", embedding_service_health, 30)
        ...
        assert overall_health["overall_status"] == "unhealthy"  # Due to cache service error
        ...
        assert health_results["embedding_service"]["details"]["openai_api_status"] == "slow"
        assert health_results["vector_db_service"]["details"]["index_status"] == "optimized"
    ...
    async def test_performance_monitoring_integration(self):
        ...
        class PerformanceMonitor:
            ...
            def record_performance(self, service: str, operation: str, 
            ...
            def establish_baseline(self, service: str, operation: str, 
                                 baseline_duration: float, tolerance: float = 0.2):
        ...
        # Establish baselines
        performance_monitor.establish_baseline("api_gateway", "search_request", 0.15, 0.3)
        performance_monitor.establish_baseline("embedding_service", "generate_embeddings", 0.1, 0.5)
        performance_monitor.establish_baseline("vector_db_service", "hybrid_search", 0.05, 0.4)
        
        # Record normal performance
        performance_monitor.record_performance("api_gateway", "search_request", 0.14)
        performance_monitor.record_performance("embedding_service", "generate_embeddings", 0.09)
        performance_monitor.record_performance("vector_db_service", "hybrid_search", 0.048)
        
        # Record degraded performance
        performance_monitor.record_performance("api_gateway", "search_request", 0.22)  # Warning
        performance_monitor.record_performance("embedding_service", "generate_embeddings", 0.18)  # Warning
        performance_monitor.record_performance("vector_db_service", "hybrid_search", 0.12)  # Critical
        ...
        embedding_degradation = next(d for d in degradations if d["service"] == "embedding_service")
        vector_db_degradation = next(d for d in degradations if d["service"] == "vector_db_service")
        ...
        assert embedding_degradation["degradation_percentage"] > 50  # Should be around 80%
        
        assert vector_db_degradation["severity"] == "critical"
        assert vector_db_degradation["degradation_percentage"] > 100  # Should be around 140%
