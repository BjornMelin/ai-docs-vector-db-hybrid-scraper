...
class TestMCPPerformanceBenchmarks:
    ...
    async def test_search_performance_benchmark(self, benchmark_server):
        ...
        for total_requests, concurrent in load_levels:
            ...
            assert metrics.success_rate >= 0.99, f"Success rate {metrics.success_rate} too low"
            assert metrics.requests_per_second > 100, f"Throughput {metrics.requests_per_second} req/s too low"
            assert metrics.p95_response_time < 0.1, f"P95 response time {metrics.p95_response_time}s too high"
    ...
    async def test_mixed_workload_performance(self, benchmark_server):
        ...
        tools = {}
        for tool in mcp_server._tools:
            if tool.name in ["search_documents", "generate_embeddings", "get_cache_stats"]:
    ...
    async def test_sustained_load_performance(self, benchmark_server):
        ...
        assert actual_rps > requests_per_second_target * 0.8, f"Could not sustain target RPS"
    ...
    async def test_spike_load_handling(self, benchmark_server):
        ...
        spike_degradation = (spike_metrics.avg_response_time - baseline_metrics.avg_response_time) / baseline_metrics.avg_response_time * 100
        recovery_degradation = (recovery_metrics.avg_response_time - baseline_metrics.avg_response_time) / baseline_metrics.avg_response_time * 100
    ...
    async def test_concurrent_tool_performance(self, benchmark_server):
        ...
        tools = {}
        for tool in mcp_server._tools:
            if tool.name in ["search_documents", "generate_embeddings", "list_collections", "get_cache_stats"]:
                tools[tool.name] = tool

        # Mock list_collections
        with patch.object(mock_client_manager.vector_service, "list_collections") as mock_list:
            mock_list.return_value = [{"name": f"col-{i}", "vectors_count": i*100} for i in range(5)]
            ...
            async def concurrent_operations():
                ...
                for i in range(25):
                    tasks.append(tools["search_documents"].handler(
                ...
                for i in range(25):
                    tasks.append(tools["generate_embeddings"].handler(
...
class TestMCPResourceOptimization:
    ...
    async def test_connection_pooling_efficiency(self, benchmark_server):
        ...
        assert avg_conn_time < 0.01, f"Average connection time too high: {avg_conn_time*1000:.2f}ms"
    ...
    async def test_batch_processing_optimization(self, benchmark_server):
        ...
        for i in range(0, num_texts, batch_size):
            batch_texts = [f"Batch text {j}" for j in range(i, min(i + batch_size, num_texts))]
