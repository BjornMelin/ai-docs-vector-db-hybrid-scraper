...
class TestAPIWorkflowValidation:
    ...
    async def test_authentication_workflow(self, mock_api_client, journey_data_manager):
        ...
        assert auth_result["success_rate"] == 1.0, "All authentication steps should succeed"
    ...
    async def test_document_management_workflow(self, mock_api_client, journey_data_manager):
        ...
        try:
            ...
            project_response = await mock_api_client.request(
                "POST",
                "/api/projects", 
                json={"name": "test-document-project", "description": "Test project for documents"}
            )
            ...
            assert len(added_documents) == len(test_urls), "All documents should be added"
            ...
            assert "documents" in documents_list_response, "Should return documents list"
            
            # Step 5: Get analytics/stats
            stats_response = await mock_api_client.request("GET", "/api/analytics/stats")
            doc_mgmt_steps.append({
                "step": "get_analytics",
                "success": True,
        ...
        # Store results
        journey_data_manager.store_artifact("api_document_management_workflow", doc_mgmt_result)
        
        # Validate document management workflow
        assert doc_mgmt_result["success_rate"] == 1.0, "All document management steps should succeed"
        assert len(successful_steps) == 5, "All five document management steps should complete"
        assert total_duration < 10.0, f"Document management took too long: {total_duration}s"
    ...
    async def test_search_workflow(self, mock_api_client, journey_data_manager):
        ...
        try:
            ...
            for i, search_params in enumerate(search_queries):
                ...
                assert search_response["query"] == search_params["query"], f"Query mismatch in search {i}"
            ...
            search_steps.append({
                ...
                "searches_with_results": len([sr for sr in search_results if sr["results_count"] > 0]),
            })
            
            assert total_results > 0, "At least some search results should be returned"
            assert avg_search_time < 1000, f"Average search time too high: {avg_search_time}ms"
            ...
            for search_result in search_results:
                ...
                if results:
                    ...
                    assert "content" in first_result, "Search results should have content"
        ...
        # Store results
        journey_data_manager.store_artifact("api_search_workflow", search_workflow_result)
        
        # Validate search workflow
        assert search_workflow_result["success_rate"] == 1.0, "All search steps should succeed"
    ...
    async def test_error_handling_workflow(self, mock_api_client, journey_data_manager):
        ...
        try:
            ...
            error_scenarios = [
                {
                    "name": "404_endpoint",
                    "setup": lambda: mock_api_client.set_error_response("/api/nonexistent", 404, "Not Found"),
                    "request": lambda: mock_api_client.request("GET", "/api/nonexistent"),
                    "expected_error": True,
                },
                {
                    "name": "500_server_error",
                    "setup": lambda: mock_api_client.set_error_response("/api/projects", 500, "Internal Server Error"),
                    "request": lambda: mock_api_client.request("GET", "/api/projects"),
                    "expected_error": True,
                },
                {
                    "name": "timeout_simulation",
                    "setup": lambda: mock_api_client.set_response_delay("/api/slow", 5000),  # 5 second delay
            ...
            assert recovery_response["status"] == "healthy", "System should recover after errors"
            
            # Test graceful degradation
            # Simulate partial service failure
            mock_api_client.set_error_response("/api/analytics/stats", 503, "Service Temporarily Unavailable")
            ...
            assert "projects" in core_response, "Core functionality should work during partial failures"
        ...
        if error_handling_steps:
            error_test_step = next((s for s in error_handling_steps if s.get("step") == "test_error_scenarios"), None)
            if error_test_step and error_test_step.get("error_test_results"):
                correct_predictions = [r for r in error_test_step["error_test_results"] if r.get("success", False)]
                error_handling_result["error_prediction_accuracy"] = len(correct_predictions) / len(error_test_step["error_test_results"])
        
        # Store results
        journey_data_manager.store_artifact("api_error_handling_workflow", error_handling_result)
        
        # Validate error handling workflow
        assert error_handling_result["success_rate"] >= 0.8, f"Error handling success rate too low: {error_handling_result['success_rate']:.2%}"
        assert len(successful_steps) >= 2, "At least basic error handling steps should succeed"
        
        # Validate error prediction accuracy if available
        if "error_prediction_accuracy" in error_handling_result:
            assert error_handling_result["error_prediction_accuracy"] >= 0.6, f"Error prediction accuracy too low: {error_handling_result['error_prediction_accuracy']:.2%}"

    @pytest.mark.performance
    async def test_api_performance_workflow(self, mock_api_client, journey_data_manager):
        ...
        try:
            ...
            for method, endpoint, *kwargs in sequential_requests:
                ...
                response = await mock_api_client.request(method, endpoint, **(kwargs[0] if kwargs else {}))
            ...
            performance_steps.append({
                ...
                "avg_request_duration_s": sequential_duration / len(sequential_requests),
            ...
            concurrent_requests = [
                mock_api_client.request("GET", "/api/search", params={"q": f"query_{i}", "limit": 5})
                for i in range(10)
            ]
            
            concurrent_responses = await asyncio.gather(*concurrent_requests, return_exceptions=True)
            concurrent_duration = time.perf_counter() - concurrent_start
            
            successful_concurrent = [r for r in concurrent_responses if not isinstance(r, Exception)]
            failed_concurrent = [r for r in concurrent_responses if isinstance(r, Exception)]
            ...
            performance_steps.append({
                ...
                "success_rate": len(successful_concurrent) / len(concurrent_requests),
            })
            ...
            for i in range(rate_test_requests):
                request_start = time.perf_counter()
                try:
                    response = await mock_api_client.request("GET", "/api/search", params={"q": f"rate_test_{i}", "limit": 3})
                    request_duration = time.perf_counter() - request_start
            ...
            successful_rate_tests = [r for r in rate_test_results if r.get("success", False)]
            ...
            performance_steps.append({
                ...
                "avg_request_duration_s": sum(r["duration_s"] for r in successful_rate_tests) / max(len(successful_rate_tests), 1),
            })
        ...
        # Store results
        journey_data_manager.store_artifact("api_performance_workflow", performance_result)
        
        # Validate API performance
        assert performance_result["success_rate"] == 1.0, "All performance test steps should succeed"
        
        # Validate specific performance metrics
        for step in performance_steps:
            if step["step"] == "sequential_requests":
                assert step["avg_request_duration_s"] < 1.0, f"Sequential requests too slow: {step['avg_request_duration_s']}s avg"
            elif step["step"] == "concurrent_requests":
                assert step["success_rate"] >= 0.9, f"Concurrent request success rate too low: {step['success_rate']:.2%}"
            elif step["step"] == "rate_testing":
                assert step["throughput_rps"] >= 5, f"Throughput too low: {step['throughput_rps']:.2f} RPS"

    async def test_complete_api_integration_workflow(self, mock_api_client, journey_data_manager):
        ...
        try:
            ...
            health_response = await mock_api_client.request("GET", "/health")
            integration_steps.append({"step": "health_check", "success": True, "response": health_response})
            ...
            integration_steps.append({"step": "authentication", "success": True, "response": auth_response})
            ...
            integration_steps.append({"step": "create_project", "success": True, "response": project_response})
            project_id = project_response["project"]["id"]
            ...
            integration_steps.append({"step": "create_collection", "success": True, "response": collection_response})
            collection_id = collection_response["collection"]["id"]
            ...
            for url in document_urls:
                doc_response = await mock_api_client.request(
                    "POST", "/api/documents",
                    json={"url": url, "collection_id": collection_id, "project_id": project_id},
                    headers={"Authorization": f"Bearer {token}"}
                )
            ...
            integration_steps.append({"step": "wait_processing", "success": True, "wait_time_s": 1.0})
            ...
            integration_steps.append({"step": "get_analytics", "success": True, "response": analytics_response})
            ...
            integration_steps.append({"step": "final_health_check", "success": True, "response": final_health_response})
        ...
        # Store results
        journey_data_manager.store_artifact("api_complete_integration_workflow", integration_result)
        
        # Validate complete integration workflow
        assert integration_result["success_rate"] == 1.0, "All integration steps should succeed"
        assert len(successful_steps) >= 10, "At least 10 integration steps should complete"
        assert total_duration < 15.0, f"Complete integration took too long: {total_duration}s"
        
        # Validate specific workflow aspects
        step_names = [step["step"] for step in integration_steps if step.get("success", False)]
        required_steps = ["health_check", "authentication", "create_project", "add_documents", "perform_searches"]
        
        for required_step in required_steps:
            assert required_step in step_names, f"Required step missing: {required_step}"
