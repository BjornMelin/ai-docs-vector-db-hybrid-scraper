...
class TestRAGPipelineIntegration:
    ...
    def mock_rag_components(self):
        ...
        vector_db.search = AsyncMock(
            return_value=ModernAITestingUtils.create_mock_qdrant_response([0.1] * 384, 5)
        )
    ...
    @integration_test
    async def test_rag_pipeline_error_handling(self, mock_services, mock_rag_components):
        ...
        embedding_manager.generate_single.side_effect = Exception("Embedding service unavailable")
        ...
        vector_db.search.return_value = ModernAITestingUtils.create_mock_qdrant_response([0.1] * 384, 3)
        text_generator.generate_response.side_effect = Exception("Text generation failed")
    ...
    async def test_rag_pipeline_performance_integration(
        ...
        search_results = await vector_db.search(query_vector=query_embedding, limit=5)
        contexts = [result["payload"]["text"] for result in search_results["result"]]
        response = await text_generator.generate_response(query=query, contexts=contexts)
        
        end_time = time.perf_counter()
        total_latency = end_time - start_time
        
        # Performance assertions (for mocked services, should be very fast)
        assert total_latency < 1.0, f"Pipeline latency {total_latency:.3f}s too high for mocked services"
    ...
    @integration_test
    async def test_concurrent_rag_requests(
        self, 
        mock_services, 
        mock_rag_components
    ):
        ...
        async def process_query(query: str) -> Dict[str, Any]:
            ...
            query_embedding = await embedding_manager.generate_single(query)
            search_results = await vector_db.search(query_vector=query_embedding, limit=3)
            contexts = [result["payload"]["text"] for result in search_results["result"]]
            response = await text_generator.generate_response(query=query, contexts=contexts)
            return {
                "query": query,
