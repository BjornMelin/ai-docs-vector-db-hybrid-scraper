...
""",
    ...
    def test_chunking_throughput_basic_strategy(self, sample_documents):
        ...
        assert throughput > 1000, f"Chunking throughput {throughput:.0f} docs/sec is below target of 1000 docs/sec"
    ...
    def test_chunking_throughput_enhanced_strategy(self, sample_documents):
        ...
        assert throughput > 500, f"Enhanced chunking throughput {throughput:.0f} docs/sec is too slow"
...
""" + ("\n    # More code...\n" * 100)
        ...
        for filename, content in source_files:
            ...
            for _ in range(3):
                ...
                has_code = any(chunk.get("chunk_type") == "code" for chunk in chunks if isinstance(chunk, dict))
        ...
        assert avg_parsing_time < 100, f"Average AST parsing time {avg_parsing_time:.1f}ms exceeds target of 100ms"
        assert max_parsing_time < 200, f"Maximum AST parsing time {max_parsing_time:.1f}ms is too slow"
    ...
    def test_chunking_memory_efficiency(self, sample_documents):
        ...
        all_chunks = []
        for doc in sample_documents:
            chunks = chunker.chunk_content(doc["content"], title=doc["title"], url=doc["url"])
        ...
        print(f"Processed {len(all_chunks)} chunks from {len(sample_documents)} documents")
        
        # Memory increase should be reasonable (less than 50MB for this test)
        assert memory_increase < 50, f"Memory increase of {memory_increase:.1f}MB is too high"
    ...
    def test_concurrent_chunking_performance(self, sample_documents):
        ...
        def chunk_document(doc):
            ...
            return chunker.chunk_content(doc["content"], title=doc["title"], url=doc["url"])
        
        start_time = time.time()
        
        # Use ThreadPoolExecutor for concurrent chunking
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(chunk_document, doc) for doc in sample_documents[:50]]
...
class TestChunkingStressTests:
    ...
    def test_very_large_document_chunking(self):
        ...
        assert elapsed < 10, f"Large document chunking took {elapsed:.2f}s, which is too slow"
        assert len(chunks) > 100, "Should generate many chunks for large document"
        
        # Check chunk integrity
        total_content_length = sum(len(chunk["content"]) for chunk in chunks)
        assert total_content_length > len(large_content) * 0.8, "Significant content loss in chunking"
    ...
    def test_many_small_documents_chunking(self):
        ...
        small_docs = []
        for i in range(2000):
            small_docs.append(f"Document {i}: This is a short document with minimal content.")
        ...
        assert throughput > 2000, f"Small document throughput {throughput:.0f} docs/sec is below target"
...
"""
        ...
        assert elapsed_ms < 500, f"Complex code chunking took {elapsed_ms:.1f}ms, which is too slow"
        ...
        if chunker.parsers:
            code_chunks = [c for c in chunks if isinstance(c, dict) and c.get("chunk_type") == "code"]
            assert len(code_chunks) > 10, "Should create many code chunks for complex code"