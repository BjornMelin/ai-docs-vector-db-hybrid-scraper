...
from src.utils.imports import setup_import_paths, resolve_imports
...
class TestSetupImportPaths:
    ...
    def test_setup_import_paths_adds_missing_paths(self):
        ...
        try:
            ...
            test_sys_path = [p for p in sys.path if str(src_dir) not in p and str(project_root) not in p]
            ...
            with patch.object(sys, 'path', test_sys_path):
                ...
                assert sys.path[0] == str(project_root) or sys.path[1] == str(project_root)
        finally:
