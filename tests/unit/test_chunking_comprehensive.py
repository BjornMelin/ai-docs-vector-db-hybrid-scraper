...
"""
    ...
    def test_basic_chunking(self, chunker: DocumentChunker) -> None:
        ...
        assert all(
            len(chunk.content) <= chunker.config.max_chunk_size for chunk in chunks
        )
    ...
    def test_get_current_code_block(self, chunker: DocumentChunker) -> None:
        ...
        code_blocks = [
            CodeBlock(
                content="block1", start_pos=10, end_pos=20, language=CodeLanguage.PYTHON
            ),
            CodeBlock(
                content="block2", start_pos=30, end_pos=40, language=CodeLanguage.PYTHON
            ),
        ]
