...
class TestQueryPreprocessor:
    ...
    async def test_synonym_expansion(self, initialized_preprocessor):
        ...
        for input_query, expected_expansion in test_cases:
            ...
            assert (expected_expansion.lower() in result.processed_query.lower() or
                    len(result.expansions_added) > 0)
    ...
    async def test_version_detection(self, initialized_preprocessor):
        ...
        for query in test_cases:
            ...
            context = result.context_extracted
            assert "version" in context or any("version" in str(v) for v in context.values())
    ...
    async def test_complexity_indicators(self, initialized_preprocessor):
        ...
        for query in complex_queries:
            ...
            if "complexity_indicators" in context:
                ...
                assert any(indicator in ["architectural", "performance", "security", "integration"]
    ...
    async def test_technical_abbreviations_normalization(self, initialized_preprocessor):
        ...
        assert "with" in result.processed_query
        assert "for example" in result.processed_query
        assert "and so on" in result.processed_query