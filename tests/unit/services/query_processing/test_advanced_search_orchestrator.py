...
class TestEnums:
    ...
    def test_search_mode_enum(self):
        ...
        expected_modes = {
            "simple", "enhanced", "intelligent", 
            "federated", "personalized", "comprehensive"
        }
    ...
    def test_processing_stage_enum(self):
        ...
        # Test all enum values are defined
        expected_stages = {
            "preprocessing", "expansion", "filtering", "execution",
            "clustering", "ranking", "federation", "postprocessing"
        }
    ...
    def test_search_pipeline_enum(self):
        ...
        expected_pipelines = {
            ...
            "discovery", "precision", "personalized"
        }
...
class TestAdvancedSearchOrchestratorInitialization:
    ...
    def test_pipeline_configs_initialization(self):
        ...
        comprehensive_config = orchestrator.pipeline_configs[SearchPipeline.COMPREHENSIVE.value]
...
class TestSearchModes:
    ...
    async def test_enhanced_search_mode(self, mock_orchestrator, basic_search_request):
        ...
        from src.services.query_processing.expansion import QueryExpansionResult, ExpansionScope
        ...
        mock_orchestrator.query_expansion_service.expand_query.return_value = mock_result
    ...
    async def test_intelligent_search_mode(self, mock_orchestrator, basic_search_request):
    ...
    async def test_federated_search_mode(self, mock_orchestrator, comprehensive_search_request):
    ...
    async def test_personalized_search_mode(self, mock_orchestrator, comprehensive_search_request):
    ...
    async def test_comprehensive_search_mode(self, mock_orchestrator, comprehensive_search_request):
        ...
        mock_orchestrator.query_expansion_service.expand_query.return_value = expansion_result
        ...
        mock_orchestrator.clustering_service.cluster_results.return_value = clustering_result
        ...
        from src.services.query_processing.federated import CollectionSelectionStrategy, ResultMergingStrategy
...
class TestProcessingStages:
    ...
    async def test_stage_execution_order(self, mock_orchestrator, comprehensive_search_request):
        ...
        for stage in executed_stages:
            while expected_index < len(expected_order) and expected_order[expected_index] != stage:
                expected_index += 1
    ...
    async def test_skip_stages(self, mock_orchestrator, basic_search_request):
        ...
        basic_search_request.skip_stages = [ProcessingStage.EXPANSION, ProcessingStage.CLUSTERING]
        basic_search_request.enable_expansion = True
        basic_search_request.enable_clustering = True
...
class TestPipelineConfigurations:
    ...
    async def test_fast_pipeline(self, mock_orchestrator):
        ...
        expansion_stages = [s for s in result.stage_results if s.stage == ProcessingStage.EXPANSION]
        clustering_stages = [s for s in result.stage_results if s.stage == ProcessingStage.CLUSTERING]
    ...
    async def test_comprehensive_pipeline(self, mock_orchestrator):
        ...
        expansion_stages = [s for s in result.stage_results if s.stage == ProcessingStage.EXPANSION]
        clustering_stages = [s for s in result.stage_results if s.stage == ProcessingStage.CLUSTERING]
        ranking_stages = [s for s in result.stage_results if s.stage == ProcessingStage.RANKING]
        federation_stages = [s for s in result.stage_results if s.stage == ProcessingStage.FEDERATION]
...
class TestPerformanceTracking:
    ...
    async def test_processing_time_tracking(self, mock_orchestrator, basic_search_request):
    ...
    async def test_performance_stats_accumulation(self, mock_orchestrator, basic_search_request):
    ...
    async def test_feature_usage_tracking(self, mock_orchestrator, comprehensive_search_request):
    ...
    async def test_quality_stats_accumulation(self, mock_orchestrator, basic_search_request):
...
class TestErrorHandling:
    ...
    async def test_preprocessing_stage_failure(self, mock_orchestrator, basic_search_request):
        """Test handling of preprocessing stage failure."""
        # Make preprocessing fail
        with patch.object(mock_orchestrator, '_execute_preprocessing_stage') as mock_preprocess:
    ...
    async def test_expansion_stage_failure(self, mock_orchestrator, basic_search_request):
        """Test handling of expansion stage failure."""
        basic_search_request.enable_expansion = True
        
        # Make expansion service fail
        mock_orchestrator.query_expansion_service.expand_query.side_effect = Exception("Expansion failed")
    ...
    async def test_search_execution_failure(self, mock_orchestrator, basic_search_request):
    ...
    async def test_clustering_stage_failure(self, mock_orchestrator, basic_search_request):
        """Test handling of clustering stage failure."""
        basic_search_request.enable_clustering = True
        
        # Make clustering service fail
        mock_orchestrator.clustering_service.cluster_results.side_effect = Exception("Clustering failed")
    ...
    async def test_ranking_stage_failure(self, mock_orchestrator, basic_search_request):
        ...
        mock_orchestrator.ranking_service.rank_results.side_effect = Exception("Ranking failed")
    ...
    async def test_federation_stage_failure(self, mock_orchestrator, basic_search_request):
        """Test handling of federation stage failure."""
        basic_search_request.enable_federation = True
        
        # Make federated service fail
        mock_orchestrator.federated_service.search.side_effect = Exception("Federation failed")
    ...
    async def test_filtering_stage_failure_continues(self, mock_orchestrator, basic_search_request):
        ...
        filtering_stages = [s for s in result.stage_results if s.stage == ProcessingStage.FILTERING]
        if filtering_stages:
            assert not filtering_stages[0].success

    async def test_general_exception_handling(self, mock_orchestrator, basic_search_request):
...
class TestFeatureToggling:
    ...
    async def test_personalization_toggle(self, mock_orchestrator, basic_search_request):
    ...
    async def test_personalization_without_user_id(self, mock_orchestrator, basic_search_request):
        ...
        if ranking_stages:
            ...
            assert "skipped_no_user" in ranking_stages[0].metadata

    async def test_conditional_filter_application(self, mock_orchestrator, basic_search_request):
...
class TestIntegrationScenarios:
    ...
    async def test_full_pipeline_integration(self, mock_orchestrator, comprehensive_search_request):
        ...
        mock_orchestrator.federated_service.search.return_value = AsyncMock(
            results=[
                {"id": "fed_1", "title": "External Resource", "score": 0.8, "source": "external"}
            ],
    ...
    async def test_progressive_fallback_scenario(self, mock_orchestrator, basic_search_request):
    ...
    async def test_partial_feature_scenario(self, mock_orchestrator, basic_search_request):
    ...
    async def test_high_performance_scenario(self, mock_orchestrator, basic_search_request):
...
class TestUtilityMethods:
    ...
    def test_merge_federated_results(self, orchestrator):
        ...
        merged = orchestrator._merge_federated_results(primary_results, federated_results)
...
class TestPerformanceStatsAndCleanup:
    ...
    async def test_stage_performance_tracking(self, mock_orchestrator, basic_search_request):
