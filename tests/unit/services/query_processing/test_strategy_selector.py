...
class TestSearchStrategySelector:
    ...
    def test_initialization(self, strategy_selector):
        ...
        assert len(strategy_selector._complexity_adjustments) == 4  # All complexity levels
    ...
    async def test_conceptual_intent_strategy(self, initialized_selector, sample_intent_classification):
    ...
    async def test_factual_intent_strategy(self, initialized_selector):
        ...
        assert selection.primary_strategy == SearchStrategy.HYBRID
    ...
    async def test_advanced_intent_strategies(self, initialized_selector):
        ...
        for intent, expected_strategy in test_cases:
            ...
            assert selection.primary_strategy == expected_strategy, f"Intent {intent} should use {expected_strategy}"
    ...
    async def test_complexity_adjustments(self, initialized_selector):
        ...
        simple_selection = await initialized_selector.select_strategy(simple_classification)
        ...
        expert_selection = await initialized_selector.select_strategy(expert_classification)
        
        # Expert queries should have larger dimensions or additional fallbacks
        assert (expert_selection.matryoshka_dimension.value >= 
                simple_selection.matryoshka_dimension.value)
        assert len(expert_selection.fallback_strategies) >= len(simple_selection.fallback_strategies)

    async def test_performance_requirements_latency(self, initialized_selector, sample_intent_classification):
    ...
    async def test_performance_requirements_quality(self, initialized_selector, sample_intent_classification):
        ...
        assert (selection.primary_strategy in high_quality_strategies or 
                any(strategy in high_quality_strategies for strategy in selection.fallback_strategies))
        assert "quality" in selection.reasoning.lower()

    async def test_context_adjustments(self, initialized_selector, sample_intent_classification):
        """Test context-based strategy adjustments."""
        context_cases = [
            ({"programming_language": ["python", "javascript"]}, SearchStrategy.SEMANTIC),
    ...
    async def test_secondary_intents_incorporation(self, initialized_selector):
        ...
        all_strategies = [selection.primary_strategy] + selection.fallback_strategies
        assert (troubleshooting_strategy in all_strategies or 
                performance_strategy in all_strategies)

    async def test_confidence_calculation(self, initialized_selector, sample_intent_classification):
        ...
        high_confidence_selection = await initialized_selector.select_strategy(high_confidence_classification)
        ...
        low_confidence_selection = await initialized_selector.select_strategy(low_confidence_classification)
        
        # Higher intent confidence should result in higher strategy confidence
        assert high_confidence_selection.confidence > low_confidence_selection.confidence

    async def test_estimated_metrics(self, initialized_selector, sample_intent_classification):
        """Test estimated quality and latency metrics."""
        selection = await initialized_selector.select_strategy(sample_intent_classification)
    ...
    async def test_fallback_strategy_limits(self, initialized_selector):
        ...
        classification = QueryIntentClassification(
            primary_intent=QueryIntent.CONCEPTUAL,
            secondary_intents=[QueryIntent.PROCEDURAL, QueryIntent.FACTUAL, QueryIntent.TROUBLESHOOTING],
            confidence_scores={
                QueryIntent.CONCEPTUAL: 0.7,
        ...
        # Simple complexity should limit fallbacks
        assert len(selection.fallback_strategies) <= 1
    ...
    async def test_dimension_boost_for_complexity(self, initialized_selector):
        ...
        complex_selection = await initialized_selector.select_strategy(complex_classification)
        
        # Complex should have same or larger dimension
        assert complex_selection.matryoshka_dimension.value >= simple_selection.matryoshka_dimension.value
    ...
    async def test_uninitialized_selector_error(self, strategy_selector, sample_intent_classification):
    ...
    async def test_reasoning_content(self, initialized_selector, sample_intent_classification):
        ...
        assert sample_intent_classification.primary_intent.value in selection.reasoning.lower()

    async def test_dimension_multiplier_effects(self, initialized_selector, sample_intent_classification):
        ...
        classification_small.primary_intent = QueryIntent.FACTUAL  # Uses SMALL dimension
        
        classification_large = sample_intent_classification.model_copy()
        classification_large.primary_intent = QueryIntent.PROCEDURAL  # Uses LARGE dimension
        
        small_selection = await initialized_selector.select_strategy(classification_small)
        large_selection = await initialized_selector.select_strategy(classification_large)
        
        # Larger dimensions should have higher latency and quality
        assert large_selection.estimated_latency_ms >= small_selection.estimated_latency_ms
