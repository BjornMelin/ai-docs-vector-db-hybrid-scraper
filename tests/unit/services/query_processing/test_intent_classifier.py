...
def mock_embedding_manager():
    ...
    embeddings.extend([[0.2 + i*0.01] * 768 for i in range(14)])  # 14 reference embeddings
...
class TestQueryIntentClassifier:
    ...
    async def test_classify_conceptual_query(self, initialized_classifier):
        ...
        assert result.complexity_level in [QueryComplexity.SIMPLE, QueryComplexity.MODERATE, QueryComplexity.COMPLEX, QueryComplexity.EXPERT]
    ...
    async def test_secondary_intents(self, initialized_classifier):
        ...
        assert any(intent in [QueryIntent.PERFORMANCE, QueryIntent.DEBUGGING] 
                  for intent in result.secondary_intents)

    async def test_complexity_assessment(self, initialized_classifier):
        """Test query complexity assessment."""
        # Simple query
        simple_result = await initialized_classifier.classify_query_advanced("What is Python?")
        assert simple_result.complexity_level in [QueryComplexity.SIMPLE, QueryComplexity.MODERATE, QueryComplexity.COMPLEX, QueryComplexity.EXPERT]
        
        # Complex query
        complex_query = ("How to design a distributed microservices architecture "
                        "with event sourcing, CQRS, and saga patterns for handling "
                        "cross-service transactions in a high-throughput system?")
        complex_result = await initialized_classifier.classify_query_advanced(complex_query)
        assert complex_result.complexity_level in [QueryComplexity.COMPLEX, QueryComplexity.EXPERT]
