...
class TestPrometheusMiddleware:
    ...
    def test_prometheus_middleware_initialization(
    ...
    def test_prometheus_middleware_endpoints_added(
        self, mock_metrics_registry, mock_health_manager
    ):
    ...
    def test_unhealthy_service_response(
        self, mock_metrics_registry, mock_health_manager
    ):
        """Test response when services are unhealthy."""
        # Configure unhealthy service
        mock_health_manager.check_all = AsyncMock(
            return_value={
                "qdrant": MagicMock(
                    status="unhealthy",
...
class TestCustomMetricsMiddleware:
    ...
    def app_with_custom_middleware(self, mock_metrics_registry):
        ...
        app = FastAPI()
        app.add_middleware(
            CustomMetricsMiddleware, metrics_registry=mock_metrics_registry
        )
    ...
    def test_custom_middleware_initialization(self, mock_metrics_registry):
        ...
        middleware = CustomMetricsMiddleware(
            app, metrics_registry=mock_metrics_registry
        )
...
class TestMiddlewareIntegration:
    ...
    def test_full_monitoring_integration(
        self, full_monitoring_app, mock_health_manager
    ):
