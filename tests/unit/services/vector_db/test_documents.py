...
class TestQdrantDocuments:
    ...
    async def test_build_filter_empty(self, documents_service):
        ...
        result = documents_service._build_filter(None)
        assert result is None

        result = documents_service._build_filter({})
        assert result is None

    async def test_build_filter_keyword_fields(self, documents_service):
        """Test filter building with keyword fields."""
        filters = {"doc_type": "api", "language": "python", "version": "3.8"}

        result = documents_service._build_filter(filters)
    ...
    async def test_build_filter_text_fields(self, documents_service):
        """Test filter building with text fields."""
        filters = {"title": "test document", "content_preview": "sample content"}
    ...
    async def test_build_filter_range_fields(self, documents_service):
        """Test filter building with range fields."""
        filters = {
            "created_after": 1609459200,  # 2021-01-01
            "created_before": 1640995200,  # 2022-01-01
            "min_word_count": 100,
            "max_word_count": 1000,
        }
    ...
    async def test_build_filter_exact_match_fields(self, documents_service):
        """Test filter building with exact match fields."""
        filters = {"chunk_index": 0, "depth": 2}
    ...
    async def test_build_filter_invalid_keyword_value(self, documents_service):
        """Test filter building with invalid keyword value."""
        filters = {
            "doc_type": ["invalid", "list"]  # Should be simple type
        }

        with pytest.raises(
            ValueError, match="Filter value for doc_type must be a simple type"
        ):
            documents_service._build_filter(filters)

    async def test_build_filter_invalid_text_value(self, documents_service):
        """Test filter building with invalid text value."""
        filters = {
    ...
    async def test_build_filter_mixed_types(self, documents_service):
        """Test filter building with mixed field types."""
        filters = {
            "doc_type": "api",  # keyword
            "title": "test",  # text
            "created_after": 1609459200,  # range
            "chunk_index": 0,  # exact match
        }
