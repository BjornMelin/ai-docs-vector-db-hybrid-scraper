...
class TestQueryClassifier:
    ...
    @pytest.mark.parametrize("query,expected_type", [
        # Code queries
        ("def factorial(n): return 1 if n <= 1 else n * factorial(n-1)", QueryType.CODE),
        ...
        ("What parameters does the createUser endpoint accept?", QueryType.API_REFERENCE),
    ...
    @pytest.mark.parametrize("query,expected_complexity", [
        ...
        # Moderate queries
        ("How to implement error handling in async functions?", QueryComplexity.MODERATE),
        ("Explain dependency injection pattern", QueryComplexity.MODERATE),
        ("Compare React and Vue performance", QueryComplexity.MODERATE),
        
        # Complex queries
        ("How to optimize database performance and implement caching strategies while maintaining ACID properties?", QueryComplexity.COMPLEX),
        ("Design microservices architecture with event sourcing and CQRS patterns", QueryComplexity.COMPLEX),
        ("Implement distributed consensus algorithm with Byzantine fault tolerance", QueryComplexity.COMPLEX),
    ])
    ...
    async def test_programming_language_detection(self, classifier, query, expected_language):
    ...
    async def test_technical_depth_assessment(self, classifier):
        ...
        # Advanced query
        advanced_query = "Implement complex optimization algorithm with performance analysis"
        advanced_depth = classifier._assess_technical_depth(advanced_query.lower(), advanced_query.split())
        assert advanced_depth == "advanced"
        
        # Basic query
        basic_query = "What is a simple introduction to programming?"
        basic_depth = classifier._assess_technical_depth(basic_query.lower(), basic_query.split())
        assert basic_depth == "basic"
        
        # Medium query
        medium_query = "How to create a web application?"
        medium_depth = classifier._assess_technical_depth(medium_query.lower(), medium_query.split())
    ...
    async def test_semantic_complexity_calculation(self, classifier):
        ...
        high_complexity_query = "Compare the relationship between different algorithms and their trade-offs"
        tokens = high_complexity_query.lower().split()
        high_score = classifier._calculate_semantic_complexity(high_complexity_query.lower(), tokens)
        
        # Low complexity query
        low_complexity_query = "How to print hello world"
        tokens = low_complexity_query.lower().split()
        low_score = classifier._calculate_semantic_complexity(low_complexity_query.lower(), tokens)
    ...
    async def test_keyword_density_calculation(self, classifier):
        ...
        high_density = classifier._calculate_keyword_density(high_density_query.lower(), tokens)
        
        # Low density query
        low_density_query = "This is a normal sentence without technical terms"
        tokens = low_density_query.split()
        low_density = classifier._calculate_keyword_density(low_density_query.lower(), tokens)
    ...
    async def test_confidence_calculation(self, classifier):
        ...
        features = classifier._extract_features(query)
        confidence = classifier._calculate_confidence(query, features, QueryType.CODE, QueryComplexity.MODERATE)
    ...
    @pytest.mark.parametrize("query,expected_features", [
        ("async def process_data():", {"has_programming_syntax": True, "has_code_keywords": True}),
        ("What is machine learning?", {"has_code_keywords": False, "question_type": "what"}),
        ("import tensorflow as tf", {"has_programming_syntax": True, "programming_language_indicators": ["tensorflow"]}),
        ("How to fix 404 error?", {"question_type": "how"}),
    ])
    async def test_specific_feature_extraction(self, classifier, query, expected_features):
