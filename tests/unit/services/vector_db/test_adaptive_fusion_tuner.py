...
from src.models.vector_search import AdaptiveFusionWeights
from src.models.vector_search import EffectivenessScore
...
class TestAdaptiveFusionTuner:
    ...
    async def test_effectiveness_with_user_feedback(
        self, tuner, sample_dense_results, sample_sparse_results
    ):
    ...
    async def test_historical_weights_retrieval(
        self, tuner, sample_query_classification
    ):
    ...
    async def test_rule_based_weight_computation(self, tuner):
        ...
        test_cases = [
            (
                ...
                "code queries should balance dense/sparse",
            ),
            (
            ...
            (
                ...
                QueryComplexity.MODERATE,
                "API queries should favor sparse",
            ),
        ]
    ...
    async def test_strategy_combination(self, tuner):
        ...
        strategies = [
            {
                "weights": {"dense": 0.8, "sparse": 0.2, "confidence": 0.7},
                "confidence": 0.8,
            },
            {
                "weights": {"dense": 0.6, "sparse": 0.4, "confidence": 0.6},
                "confidence": 0.6,
            },
            {
                "weights": {"dense": 0.7, "sparse": 0.3, "confidence": 0.8},
                "confidence": 0.9,
            },
        ]
    ...
    async def test_multi_armed_bandit_optimization(
        self, tuner, sample_query_classification
    ):
    ...
    async def test_error_handling_in_weight_computation(
        self, tuner, sample_query_classification
    ):
    ...
    @pytest.mark.parametrize(
        "query_type,expected_dense_bias",
        [
    ...
    async def test_query_type_specific_weighting(
        self, tuner, query_type, expected_dense_bias
    ):
