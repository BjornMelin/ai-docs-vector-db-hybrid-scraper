...
class TestQdrantService:
    ...
    @pytest.fixture
    def qdrant_service(
        self, config: Config, mock_client_manager: Mock
    ) -> QdrantService:
    ...
    async def test_service_initialize_success(
        ...
        with mock_qdrant_modules() as mocks:
            ...
            mocks["collections_class"].assert_called_once_with(
                qdrant_service.config, mock_qdrant_client
            )
            mocks["search_class"].assert_called_once_with(
                mock_qdrant_client, qdrant_service.config
            )
            mocks["indexing_class"].assert_called_once_with(
                mock_qdrant_client, qdrant_service.config
            )
            mocks["documents_class"].assert_called_once_with(
                mock_qdrant_client, qdrant_service.config
            )
    ...
    async def test_service_initialize_already_initialized(
        self, qdrant_service: QdrantService, mock_client_manager: Mock
    ) -> None:
    ...
    async def test_service_initialize_module_initialization_error(
        ...
        with patch.multiple(
            ...
            mock_collections.initialize.side_effect = Exception(
                "Collections init failed"
            )
    ...
    async def test_initialize_deployment_services_feature_flags_enabled(
        ...
        mock_client_manager.get_feature_flag_manager.return_value = (
            mock_feature_flag_manager
        )
        mock_client_manager.get_ab_testing_manager.return_value = (
            mock_ab_testing_manager
        )
        mock_client_manager.get_blue_green_manager.return_value = (
            mock_blue_green_manager
        )
    ...
    async def test_initialize_deployment_services_error_handling(
        ...
        # Setup client manager to raise an error
        mock_client_manager.get_feature_flag_manager.side_effect = Exception(
            "Feature flag init failed"
        )
    ...
    def test_service_properties_before_initialization(
...
class TestQdrantServiceIntegration:
    ...
    async def test_error_recovery_scenarios(
        self, integration_config: Config, mock_client_manager: Mock
    ) -> None:
...
class TestQdrantServicePerformance:
    ...
    async def test_initialization_performance(
        self, config: Config, mock_client_manager: Mock, mock_qdrant_client: Mock
    ) -> None:
...
class TestQdrantServiceEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_initialization_with_invalid_config(
