...
class TestDependencyLifecycle:
    ...
    async def test_embedding_client_lifecycle(self):
        ...
        with patch('src.services.embeddings.manager.EmbeddingManager') as MockEmbeddingManager:
            mock_instance = AsyncMock()
            MockEmbeddingManager.return_value = mock_instance
            
            async for embedding_client in get_embedding_client(mock_config, mock_client_manager):
    ...
    async def test_crawling_client_lifecycle(self):
        ...
        with patch('src.services.crawling.manager.CrawlManager') as MockCrawlManager:
            ...
            async for crawling_client in get_crawling_client(mock_config, mock_rate_limiter):
...
class TestFastAPIIntegration:
    ...
    def test_config_dependency_injection(self):
        ...
        async def get_config_endpoint(config: Config = Depends(get_config)):
            return {"provider": config.embedding_provider.value if hasattr(config, 'embedding_provider') else "default"}
    ...
    async def test_batch_processing_patterns(self):
        ...
        class BatchProcessor:
            ...
            async def _flush_batch(self):
                ...
                # Resolve futures
                for future, result in zip(self.futures, results):
                    if not future.done():
