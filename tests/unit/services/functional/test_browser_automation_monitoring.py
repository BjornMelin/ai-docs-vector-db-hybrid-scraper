...
class MockBrowserMonitoringSystem:
    ...
    async def comprehensive_health_check(self) -> Dict[str, Any]:
        ...
        healthy_tiers = sum(1 for result in health_results.values() if result['healthy'])
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        avg_response_time = sum(tier.response_time for tier in self.tiers.values()) / len(self.tiers)
        ...
        return {
            ...
            'aggregate_metrics': {
                ...
                'healthy_tier_count': sum(1 for tier in self.tiers.values() if tier.is_healthy)
            },
...
class TestBrowserAutomationMonitoring:
    ...
    async def test_failover_mechanism(self, monitoring_system):
        ...
        result = await monitoring_system.execute_with_failover('complex_scrape', 'lightweight')
    ...
    async def test_preferred_tier_selection(self, monitoring_system):
        ...
        result = await monitoring_system.execute_with_failover('simple_scrape', 'crawl4ai')
    ...
    def test_tier_performance_metrics(self, monitoring_system):
        ...
        for tier_name in ['lightweight', 'playwright', 'crawl4ai', 'browser_use', 'firecrawl']:
    ...
    async def test_tier_failure_recovery(self, monitoring_system):
        ...
        for i in range(25):  # Force some failures
    ...
    async def test_tier_response_time_characteristics(self, monitoring_system):
        ...
        for tier_name in ['lightweight', 'playwright', 'crawl4ai', 'browser_use', 'firecrawl']:
            try:
                result = await monitoring_system.execute_with_failover('test_request', tier_name)
