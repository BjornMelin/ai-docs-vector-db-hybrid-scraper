...
class TestAdaptiveConcurrencyLimiter:
    ...
    async def test_adaptive_scaling_up(self, limiter):
        ...
        with patch.object(limiter, '_get_system_metrics', return_value={
            'cpu_percent': 50.0,
...
class TestIntelligentTaskScheduler:
    ...
    async def test_batch_processing(self, scheduler):
        ...
        metrics = scheduler.get_metrics()
        assert metrics['scheduler_metrics']['batches_processed'][TaskPriority.NORMAL] > 0
...
class TestAdaptiveCacheOptimizer:
    ...
    async def test_optimization_performance(self, optimizer):
        ...
        # Add some patterns
        patterns = {
            f"key_{i}": AccessPattern(key=f"key_{i}")
            for i in range(5)
        }
...
class TestPerformanceOptimizationShowcase:
    ...
    async def test_showcase_initialization(self, showcase):
        ...
        with patch('src.services.cache.manager.CacheManager') as mock_cache, \
             patch('src.services.cache.adaptive_cache.EnhancedCacheManager') as mock_enhanced_cache, \
             patch('src.services.performance.async_optimization.initialize_async_optimizer') as mock_async_init, \
             patch('src.services.performance.benchmark_suite.PerformanceBenchmarkSuite') as mock_benchmark:
...
class TestPerformanceIntegration:
    ...
    async def test_end_to_end_optimization(self):
        ...
        from src.services.performance.async_optimization import initialize_async_optimizer
