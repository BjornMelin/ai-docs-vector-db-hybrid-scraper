...
def visualization_engine(mock_config):
    ...
    with patch('src.services.analytics.vector_visualization.get_config', return_value=mock_config):
...
class TestVectorVisualizationEngine:
    ...
    async def test_reduce_dimensions_pca(self, initialized_engine, sample_embeddings, sample_documents):
    ...
    async def test_reduce_dimensions_tsne(self, initialized_engine, sample_embeddings, sample_documents):
    ...
    async def test_cluster_vectors(self, initialized_engine, sample_embeddings, sample_documents):
        ...
        cluster_ids = [point["cluster_id"] for point in points if point["cluster_id"] is not None]
        assert len(cluster_ids) <= len(sample_embeddings)  # Some points might not have cluster assignments
        assert all(isinstance(cluster_id, (int, np.integer)) for cluster_id in cluster_ids)
    ...
    async def test_calculate_similarities(self, initialized_engine, sample_embeddings, sample_documents):
    ...
    async def test_create_visualization_2d(self, initialized_engine, sample_embeddings, sample_documents):
    ...
    async def test_create_visualization_3d(self, initialized_engine, sample_embeddings, sample_documents):
    ...
    async def test_create_visualization_with_query(self, initialized_engine, sample_embeddings, sample_documents):
        ...
        for point in points:
            ...
            if point["similarity_score"] is not None:
                assert -1.0 <= point["similarity_score"] <= 1.0  # Cosine similarity range

    async def test_find_similar_vectors(self, initialized_engine, sample_embeddings, sample_documents):
    ...
    async def test_analyze_embedding_quality(self, initialized_engine, sample_embeddings, sample_documents):
    ...
    async def test_export_visualization_data(self, initialized_engine, sample_embeddings, sample_documents):
        ...
        try:
            ...
            serializable_data = {
                ...
                "quality_metrics": visualization["quality_metrics"]  # Already dictionary
            }
    ...
    async def test_caching_behavior(self, initialized_engine, sample_embeddings, sample_documents):
