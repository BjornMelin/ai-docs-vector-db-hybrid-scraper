...
class TestQueryOrchestratorComprehensive:
    ...
    def test_get_strategy_recommendation(self, orchestrator):
        ...
        assert orchestrator._get_strategy_recommendation(edge_high_stats) == "continue_using"
        
        edge_low_stats = {"avg_performance": 0.6}
        assert orchestrator._get_strategy_recommendation(edge_low_stats) == "monitor_performance"
    ...
    async def test_tool_analyze_query_intent(self, orchestrator, mock_deps):
        ...
        test_queries = [
            ...
            ("Research the academic literature on quantum computing", "complex", "academic")
        ]
    ...
    @pytest.mark.asyncio
    async def test_tool_coordinate_multi_stage_search_error(self, orchestrator, mock_deps):
    ...
    @given(
        ...
        st.one_of(st.just("general"), st.just("technical"), st.just("business"), st.just("academic"))
    )
    ...
    def test_completion_time_estimation_properties(self, orchestrator, agent_type, complexity):
    ...
    def test_orchestrator_tool_integration_patterns(self, orchestrator):
        ...
        for query, expected_tools in query_patterns:
            ...
            if any(indicator in query_lower for indicator in ["what is", "define"]):
                complexity = "simple"
            elif any(indicator in query_lower for indicator in ["analyze", "evaluate", "recommend"]):
                complexity = "complex"
            
            # Determine domain
            domain = "general"
            if any(keyword in query_lower for keyword in ["technical", "programming", "database", "algorithm"]):
                domain = "technical"
            ...
            for expected_tool in expected_tools:
                assert expected_tool in tools, f"Expected tool {expected_tool} not found for query: {query}"