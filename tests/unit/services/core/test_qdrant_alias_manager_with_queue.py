...
class TestQdrantAliasManagerWithTaskQueue:
    ...
    @pytest.mark.asyncio
    async def test_safe_delete_collection_with_queue(
        self, alias_manager, task_queue_manager
    ):
        """Test safe_delete_collection uses task queue."""
        # Setup
        alias_manager.list_aliases = AsyncMock(
        ...
        await alias_manager.safe_delete_collection(
            "test_collection", grace_period_minutes=30
        )
    ...
    async def test_safe_delete_collection_no_queue(self, config, client):
        ...
        with pytest.raises(
            RuntimeError,
            match="TaskQueueManager is required for safe collection deletion",
        ):
    ...
    async def test_safe_delete_collection_with_aliases(
        ...
        alias_manager.list_aliases = AsyncMock(
            return_value={"alias1": "test_collection"}
        )
    ...
    @pytest.mark.asyncio
    async def test_safe_delete_collection_queue_failure(
        self, alias_manager, task_queue_manager
    ):
        ...
        with pytest.raises(
            RuntimeError,
            match="Failed to schedule deletion of test_collection via task queue",
        ):
            await alias_manager.safe_delete_collection("test_collection")
