...
class TestConfigurationWizardSetup:
    ...
    def test_template_based_setup_development(self, mock_questionary):
        ...
        mock_questionary.select.return_value.ask.return_value = "🛠️  Development - Debug logging, local database, fast iteration"
        mock_questionary.confirm.return_value.ask.return_value = False  # No customization
        mock_questionary.path.return_value.ask.return_value = str(self.temp_dir / "config.json")
    ...
    def test_interactive_setup_basic(self, mock_questionary):
        ...
        mock_questionary.path.return_value.ask.return_value = str(self.temp_dir / "interactive_config.json")
    ...
    def test_migration_setup(self, mock_questionary):
        ...
        mock_questionary.confirm.return_value.ask.return_value = True  # Confirm migration
...
class TestConfigurationWizardValidation:
    ...
    @patch('src.config.wizard.ConfigurationValidator')
    @patch('src.config.wizard.questionary')
    def test_validation_wizard_with_errors_fix_declined(self, mock_questionary, mock_validator_class):
...
class TestConfigurationWizardBackup:
    ...
    def test_backup_wizard_basic(self, mock_questionary):
        ...
        self.wizard.backup_manager.create_backup = MagicMock(return_value=mock_backup_id)
...
class TestConfigurationWizardRestore:
    ...
    def test_restore_wizard_with_backups(self, mock_questionary):
        ...
        self.wizard.backup_manager.list_backups = MagicMock(return_value=[mock_backup_metadata])
        
        # Mock user selections
        mock_questionary.select.return_value.ask.return_value = "backup_12345... - test_config"
        mock_questionary.path.return_value.ask.return_value = str(self.temp_dir / "restored_config.json")
...
class TestConfigurationWizardUtilities:
    ...
    def test_customize_template_server_name(self, mock_questionary):
        ...
        mock_questionary.select.return_value.ask.return_value = "development"  # Change environment
        mock_questionary.confirm.return_value.ask.side_effect = [
            True,   # Change debug mode 
            True,   # Configure API keys
        ]
        mock_questionary.password.return_value.ask.return_value = "sk-test123456789012345678901234567890123456789012345678"
        ...
        assert customized["openai"]["api_key"] == "sk-test123456789012345678901234567890123456789012345678"
...
class TestConfigurationWizardIntegration:
    ...
    def test_run_setup_wizard_template_mode(self, mock_questionary):
        ...
        mock_questionary.confirm.return_value.ask.return_value = False  # no customization
        mock_questionary.path.return_value.ask.return_value = str(self.temp_dir / "setup_config.json")
    ...
    def test_run_setup_wizard_interactive_mode(self, mock_questionary):
        ...
        mock_questionary.path.return_value.ask.return_value = str(self.temp_dir / "interactive_setup.json")
