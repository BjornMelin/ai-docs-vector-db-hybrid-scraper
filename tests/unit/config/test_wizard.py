...
class TestConfigurationWizard:
    ...
    def test_template_based_setup_basic(self, mock_path, mock_confirm, mock_select):
        ...
        # Mock user inputs
        mock_select.return_value.ask.return_value = "🛠️  Development - Debug logging, local database, fast iteration"
        ...
        with patch.object(self.wizard.templates, 'apply_template_to_config') as mock_apply:
    ...
    @patch('src.config.wizard.questionary.confirm')
    @patch('src.config.wizard.questionary.path')
    def test_template_based_setup_with_customization(self, mock_path, mock_confirm, mock_select):
        """Test template-based setup with customization."""
        # Mock user inputs for template selection
        mock_select.return_value.ask.return_value = "🚀 Production - Security hardening, performance optimization"
    ...
    @patch('src.config.wizard.questionary.path')
    def test_interactive_setup_complete(self, mock_path, mock_text, mock_password, mock_confirm, mock_select):
        ...
        mock_password.return_value.ask.side_effect = [
            "sk-test123456789012345678901234567890123456789012345678",   # OpenAI API key
    ...
    @patch('src.config.wizard.questionary.text')
    @patch('src.config.wizard.questionary.confirm')
    def test_migration_setup_success(self, mock_confirm, mock_text, mock_path, sample_config_file):
        ...
        with patch.object(self.wizard.migration_manager, 'get_current_version') as mock_get_version:
            mock_get_version.return_value = "1.0.0"

            with patch.object(self.wizard.migration_manager, 'create_migration_plan') as mock_create_plan:
                ...
                with patch.object(self.wizard.migration_manager, 'apply_migration_plan') as mock_apply:
    ...
    def test_customize_template(self, mock_password, mock_confirm, mock_select):
        ...
        assert result["openai"]["api_key"] == "sk-test123456789012345678901234567890123456789012345678"
        assert result["firecrawl"]["api_key"] == "fc-test123456789012345678901234567890123456789012345678"
    ...
    def test_display_validation_report(self):
        ...
        with patch.object(self.wizard.console, 'print') as mock_print:
            ...
            assert any("ℹ️  Information:" in str(arg) for arg in call_args_list)

    @patch('builtins.open', new_callable=mock_open)
    @patch('src.config.wizard.json.load')
    @patch('src.config.wizard.json.dump')
    @patch('src.config.wizard.questionary.password')
    def test_attempt_fixes_success(self, mock_password, mock_json_dump, mock_json_load, mock_file, sample_config_file):
        ...
        mock_password.return_value.ask.return_value = "sk-test123456789012345678901234567890123456789012345678"
        ...
        with patch.object(self.wizard.backup_manager, 'create_backup') as mock_backup:
            ...
            with patch('src.config.wizard.UnifiedConfig.load_from_file') as mock_load:
                with patch('src.config.wizard.ConfigurationValidator') as mock_validator_class:
                    ...
                    assert saved_config["openai"]["api_key"] == "sk-test123456789012345678901234567890123456789012345678"

    @patch('src.config.wizard.questionary.text')
    @patch('src.config.wizard.questionary.confirm')
    def test_run_backup_wizard_success(self, mock_confirm, mock_text, sample_config_file):
    ...
    def test_run_restore_wizard_success(self, mock_confirm, mock_path, mock_select):
        ...
        with patch.object(self.wizard.backup_manager, 'list_backups') as mock_list:
            ...
            with patch.object(self.wizard.backup_manager, 'restore_backup') as mock_restore:
    ...
    def test_import_setup_validation_failure(self, sample_config_file):
        ...
        with patch('src.config.wizard.questionary.path') as mock_path:
            with patch('src.config.wizard.questionary.confirm') as mock_confirm:
                mock_path.return_value.ask.side_effect = [str(sample_config_file), str(target_path)]
                ...
                with patch('src.config.wizard.ConfigurationValidator') as mock_validator_class:
                    ...
                    with patch.object(UnifiedConfig, 'load_from_file') as mock_load:
                        mock_load.return_value = UnifiedConfig(environment="development", debug=True, log_level="DEBUG")
