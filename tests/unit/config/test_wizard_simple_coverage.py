...
from src.config.enhanced_validators import ValidationReport, ValidationIssue, ValidationSeverity
...
class TestConfigurationWizardSimpleCoverage:
    ...
    def sample_config_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            f.write('{"environment": "development", "debug": true, "log_level": "DEBUG"}')
            f.flush()
            yield Path(f.name)
    ...
    def test_backup_wizard_error_handling(self, wizard, sample_config_file):
        ...
        with patch('src.config.wizard.questionary.text') as mock_text:
            with patch('src.config.wizard.questionary.select') as mock_select:
                with patch.object(wizard.backup_manager, 'create_backup') as mock_create:
