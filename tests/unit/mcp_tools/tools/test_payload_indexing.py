...
async def test_create_payload_indexes_success(
    ...
    # Setup mocks
    mock_client_manager.qdrant_service.list_collections.return_value = [
        "test_collection",
        "other_collection",
    ]
    mock_client_manager.qdrant_service.get_payload_index_stats.return_value = (
        mock_index_stats
    )

    # Mock security validator
    with patch(
        "src.mcp_tools.tools.payload_indexing.SecurityValidator"
    ) as mock_security:
        mock_validator = Mock()
        ...
        mock_client_manager.qdrant_service.create_payload_indexes.assert_called_once_with(
            "test_collection"
        )
...
@pytest.mark.asyncio
async def test_create_payload_indexes_collection_not_found(
    ...
    with patch(
        ...
        with pytest.raises(
            ValueError, match="Collection 'missing_collection' not found"
        ):
            await registered_tools["create_payload_indexes"](
                collection_name="missing_collection", ctx=mock_context
            )
...
@pytest.mark.asyncio
async def test_list_payload_indexes_success(
    mock_client_manager, mock_context, mock_index_stats
):
...
async def test_reindex_collection_success(
    ...
    mock_client_manager.qdrant_service.get_payload_index_stats.side_effect = [
        stats_before,
        stats_after,
    ]
    ...
    with patch(
        ...
        # Verify services were called correctly
        assert (
            mock_client_manager.qdrant_service.get_payload_index_stats.call_count == 2
        )
        mock_client_manager.qdrant_service.reindex_collection.assert_called_once_with(
            "test_collection"
        )
...
async def test_benchmark_filtered_search_success(mock_client_manager, mock_context):
    ...
    mock_client_manager.qdrant_service.get_collection_info.return_value = (
        mock_collection_info
    )

    mock_index_stats = {"indexed_fields": ["site_name", "title", "category"]}
    mock_client_manager.qdrant_service.get_payload_index_stats.return_value = (
        mock_index_stats
    )
...
@pytest.mark.asyncio
async def test_benchmark_filtered_search_default_query(
    mock_client_manager, mock_context
):
...
async def test_create_payload_indexes_error_handling(mock_client_manager, mock_context):
    ...
    with patch(
        ...
        mock_validator = Mock()
        mock_validator.validate_collection_name.side_effect = Exception(
            "Validation error"
        )
        mock_security.from_unified_config.return_value = mock_validator
...
async def test_list_payload_indexes_error_handling(mock_client_manager, mock_context):
    ...
    mock_client_manager.qdrant_service.get_payload_index_stats.side_effect = Exception(
        "Service error"
    )
...
async def test_reindex_collection_error_handling(mock_client_manager, mock_context):
    ...
    mock_client_manager.qdrant_service.reindex_collection.side_effect = Exception(
        "Reindex failed"
    )
...
@pytest.mark.asyncio
async def test_benchmark_filtered_search_error_handling(
    mock_client_manager, mock_context
):
    ...
    mock_client_manager.embedding_manager.generate_embeddings.side_effect = Exception(
        "Embedding failed"
    )
