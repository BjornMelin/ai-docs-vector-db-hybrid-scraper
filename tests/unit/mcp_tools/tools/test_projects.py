...
@pytest.mark.asyncio
async def test_error_handling_collection_creation_failure(mock_client_manager, mock_context):
    ...
    qdrant = await mock_client_manager.get_qdrant_service()
    qdrant.create_collection_with_quality.side_effect = Exception("Collection creation failed")
