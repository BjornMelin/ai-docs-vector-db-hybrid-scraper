...
@pytest.mark.asyncio
async def test_search_documents_core_sparse_not_available(
...
@pytest.mark.asyncio
async def test_search_documents_core_with_search_accuracy(
    mock_client_manager, mock_context
):
...
@pytest.mark.asyncio
async def test_search_documents_core_request_id_tracking(
    mock_client_manager, mock_context
):
...
@pytest.mark.asyncio
async def test_search_documents_core_long_query_truncation(
    mock_client_manager, mock_context
):
    ...
    mock_context.debug.assert_any_call(
        f"Generating embeddings for query: {long_query[:50]}..."
    )


@pytest.mark.asyncio
async def test_search_documents_core_custom_embedding_model(
    mock_client_manager, mock_context
):
