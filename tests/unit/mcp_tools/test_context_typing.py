...
class TestContextTyping:
    ...
    def test_all_tools_have_context_protocol(self):
        ...
        for module_name in MCP_TOOL_MODULES:
            ...
            assert hasattr(module, "Context"), f"{module_name} module missing Context definition"
            
            # Check if Context is a Protocol
            context_class = getattr(module, "Context")
            assert hasattr(context_class, "info"), f"{module_name} Context missing info method"
            assert hasattr(context_class, "debug"), f"{module_name} Context missing debug method"
            assert hasattr(context_class, "warning"), f"{module_name} Context missing warning method"
            assert hasattr(context_class, "error"), f"{module_name} Context missing error method"
    ...
    def test_register_tools_functions_exist(self):
        ...
        for module_name in MCP_TOOL_MODULES:
            ...
            assert hasattr(module, "register_tools"), f"{module_name} module missing register_tools function"
            
            # Check if it's callable
            register_func = getattr(module, "register_tools")
            assert callable(register_func), f"{module_name} register_tools is not callable"
            
            # Check function signature has correct parameters
            sig = inspect.signature(register_func)
            params = list(sig.parameters.keys())
            assert "mcp" in params, f"{module_name} register_tools missing 'mcp' parameter"
            assert "client_manager" in params, f"{module_name} register_tools missing 'client_manager' parameter"
    ...
    def test_no_fastmcp_runtime_import(self):
        ...
        for module_name in MCP_TOOL_MODULES:
            ...
            assert "fastmcp" not in module_dict, f"{module_name} has runtime fastmcp import"
    ...
    def test_tool_registry_imports_all_tools(self):
        ...
        assert hasattr(tool_registry, "register_all_tools"), "tool_registry missing register_all_tools function"
        
        # Check it's callable
        assert callable(tool_registry.register_all_tools), "register_all_tools is not callable"
