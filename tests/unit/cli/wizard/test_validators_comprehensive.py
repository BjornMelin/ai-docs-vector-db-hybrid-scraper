...
class TestWizardValidatorCore:
    ...
    def test_validation_cache_mechanism(self):
        ...
        result1 = validator.validate_api_key("openai", "sk-test123456789012345678901234567890")
        result2 = validator.validate_api_key("openai", "sk-test123456789012345678901234567890")
...
class TestConfigurationValidation:
    ...
    def test_validate_file_path_nonexistent_required(self, tmp_path):
        ...
        is_valid, error = validator.validate_file_path(str(nonexistent_file), must_exist=True)
        assert is_valid is False
    ...
    def test_validate_directory_path_existing(self, tmp_path):
        ...
        is_valid, error = validator.validate_directory_path(str(test_dir), must_exist=True)
        assert is_valid is True
        assert error is None
    ...
    def test_validate_directory_path_nonexistent_required(self, tmp_path):
        ...
        is_valid, error = validator.validate_directory_path(str(nonexistent_dir), must_exist=True)
        assert is_valid is False
        assert "does not exist" in error
