...
class TestConfigurationWizard:
    ...
    def test_save_configuration_toml(self, mock_prompt, mock_toml_dump, _mock_file):
        ...
        mock_prompt.side_effect = [
            "toml",  # format
            "/tmp/test_config.toml",  # path  # test temp path
        ]
        ...
        assert result == Path("/tmp/test_config.toml")  # test temp path
    ...
    def test_run_setup_complete_flow(
        ...
        mock_save.return_value = Path("/tmp/config.json")  # test temp path
        ...
        # Verify return value
        assert result == Path("/tmp/config.json")  # test temp path
...
class TestSetupCommand:
    ...
    def test_setup_command_basic(self, mock_wizard_class, mock_confirm, cli_runner):
        ...
        mock_wizard.run_setup.return_value = Path("/tmp/config.json")  # test temp path
