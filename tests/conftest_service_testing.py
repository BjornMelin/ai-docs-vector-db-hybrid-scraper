...
def mock_config():
    ...
    config.browser.tier_priorities = ['lightweight', 'playwright', 'crawl4ai', 'browser_use', 'firecrawl']
...
def mutation_test_cases():
    ...
    return {
        'circuit_breaker_mutations': [
            {'original': 'failure_count >= threshold', 'mutation': 'failure_count > threshold'},
            {'original': 'time >= recovery_timeout', 'mutation': 'time > recovery_timeout'},
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "mutation_testing: mark test as mutation test"
    )
    config.addinivalue_line(
