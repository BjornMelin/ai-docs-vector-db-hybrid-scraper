...
class TestInfrastructureAsCode:
    ...
    def test_terraform_configuration_validation(
        ...
        terraform_config = {
            ...
            "resource": {
                "docker_network": {
                    "ai_docs_network": {"name": "ai-docs-network", "driver": "bridge"}
                },
                "docker_container": {
                    "qdrant": {
    ...
    @pytest.mark.infrastructure
    def test_docker_compose_validation(self, temp_deployment_dir: Path):
        ...
        compose_config = {
            ...
            "volumes": {"qdrant_data": {}, "dragonfly_data": {}},
            "networks": {"ai-docs-network": {"driver": "bridge"}},
        }
    ...
    @pytest.mark.infrastructure
    def test_kubernetes_manifest_validation(self, temp_deployment_dir: Path):
        ...
        namespace_manifest = {
            ...
            "metadata": {
                "name": "ai-docs",
                "labels": {"app": "ai-docs-scraper", "environment": "production"},
            },
        }
        ...
        deployment_manifest = {
            ...
            "metadata": {
                "name": "ai-docs-api",
                "namespace": "ai-docs",
                "labels": {"app": "ai-docs-api", "version": "1.0.0"},
            },
            "spec": {
                "replicas": 3,
                "selector": {"matchLabels": {"app": "ai-docs-api"}},
                "template": {
                    "metadata": {"labels": {"app": "ai-docs-api"}},
                    "spec": {
                        "containers": [
                            {
                                "name": "api",
        ...
        deployment_validation = next(
            m for m in validation_result["manifests"] if m["kind"] == "Deployment"
        )
...
class TestResourceProvisioning:
        ...
        provision_config = {
            ...
            "containers": [
                ...
                },
                {
                    ...
                    "image": "docker.dragonflydb.io/dragonflydb/dragonfly:latest",
                    "ports": {"6379": "6379"},
                    "health_check": {"command": "redis-cli ping", "timeout": 30},
                },
            ],
        }
    ...
    async def test_storage_provisioning(
        ...
        qdrant_volume = next(
            v
            for v in provision_result["provisioned_volumes"]
            if v["name"] == "qdrant_data"
        )
        assert qdrant_volume["size_gb"] == 10
        assert (
            qdrant_volume["backup_configured"] == deployment_environment.backup_enabled
        )
    ...
    async def test_network_provisioning(
        ...
        backend_network = next(
            n
            for n in provision_result["provisioned_networks"]
...
class TestNetworkConfiguration:
    ...
    async def test_service_connectivity(
        ...
        assert len(connectivity_result["successful_connections"]) == len(
            expected_connections
        )
...
class TerraformValidator:
    ...
    def validate_configuration(self, terraform_file: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "resources": resources,
            "provider_configurations": list(
                config.get("terraform", {}).get("required_providers", {}).keys()
            ),
        }
                ...
                validation.update(
                    {
                        "health_checks_configured": (
                            ...
                            or "readinessProbe" in container
                        ),
...
class AutoScaler:
    ...
    async def simulate_metric_trigger(
...
class VerticalScaler:
    ...
    async def apply_resources(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        memory_change = (
            100 if "1Gi" in str(config["resources"]["limits"]["memory"]) else 0
        )
...
class ConnectivityTester:
    ...
    async def test_connectivity(
        ...
        return {
            "all_connections_successful": len(successful_connections)
...
class NetworkSecurityTester:
    ...
    async def test_security_groups(
        ...
        public_access_rules = [
            rule for rule in security_rules if rule["source"] == "0.0.0.0/0"
        ]
