...
class TestCICDPipeline:
    ...
    @pytest.mark.pipeline
    @pytest.mark.asyncio
    async def test_pipeline_stage_failure_handling(
        ...
        pipeline_config = {
            ...
            "stages": [
                {"name": "build", "type": "build", "timeout": 300},
                {"name": "test", "type": "test", "timeout": 600, "simulate_failure": True},
                {"name": "deploy", "type": "deploy", "timeout": 900},
            ]
        }
    ...
    async def test_pipeline_rollback_on_failure(
        ...
        pipeline_config = {
            ...
            "stages": [
                {"name": "build", "type": "build", "timeout": 300},
                {"name": "test", "type": "test", "timeout": 600},
                {"name": "deploy", "type": "deploy", "timeout": 900, "simulate_failure": True},
            ]
        }
...
"""
        ...
        assert build_result["image_tag"] == f"{mock_docker_registry}/ai-docs-scraper:test-1.0.0"
...
class TestTestExecution:
    ...
    async def test_unit_test_execution(
        ...
        if test_config["coverage_required"]:
            ...
            assert test_result["coverage_percentage"] >= test_config["coverage_threshold"]
    
    @pytest.mark.pipeline
    @pytest.mark.asyncio
    async def test_integration_test_execution(
        self, temp_deployment_dir: Path
    ):
...
class PipelineConfigValidator:
    ...
    def validate_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "rollback_on_failure" not in config:
            warnings.append("Consider enabling rollback_on_failure for production pipelines")
    ...
    def _validate_stage(self, stage: Dict[str, Any], index: int) -> List[str]:
        ...
        if "type" in stage and stage["type"] not in valid_types:
            errors.append(f"Stage {index}: Invalid type '{stage['type']}'. Must be one of: {valid_types}")
