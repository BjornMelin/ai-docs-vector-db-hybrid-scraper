...
class TestSmokeTests:
        ...
        for endpoint_result in smoke_results["endpoint_results"]:
            if endpoint_result["critical"]:
                assert endpoint_result["success"], (
                    f"Critical endpoint {endpoint_result['path']} failed"
                )
                assert (
                    endpoint_result["response_time_ms"]
                    < endpoint_result["timeout"] * 1000
                )
    ...
    async def test_database_connectivity(
        ...
        if deployment_environment.database_type == "postgresql":
            transaction_result = await db_tester.test_transactions(
    ...
    async def test_cache_functionality(
        ...
        cache_operations = await cache_tester.test_cache_operations(
            deployment_environment
        )
        ...
        performance_result = await cache_tester.test_cache_performance(
            deployment_environment
        )
    ...
    async def test_vector_database_functionality(
        ...
        connection_result = await vector_db_tester.test_connection(
            deployment_environment
        )
        ...
        vector_operations = await vector_db_tester.test_vector_operations(
            deployment_environment
        )
        ...
        collection_status = await vector_db_tester.test_collection_status(
            deployment_environment
        )
    ...
    def test_configuration_loaded_correctly(
        ...
        service_configs = config_tester.test_service_configurations(
            deployment_environment
        )

        for service_name, service_config in service_configs.items():
            assert service_config["configured"], (
                f"Service {service_name} not properly configured"
            )
            assert service_config["environment_appropriate"], (
                f"Service {service_name} config not appropriate for {deployment_environment.name}"
            )
    ...
    async def test_monitoring_systems_active(
        ...
        metrics_result = await monitoring_tester.test_metrics_collection(
            deployment_environment
        )
        ...
        if deployment_environment.monitoring_level in ("full", "enterprise"):
            alerting_result = await monitoring_tester.test_alerting_system(
                deployment_environment
            )
        ...
        if deployment_environment.monitoring_level == "enterprise":
            dashboard_result = await monitoring_tester.test_dashboard_availability(
                deployment_environment
            )
