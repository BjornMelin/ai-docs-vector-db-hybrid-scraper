...
class TestEnvironmentValidation:
    ...
    def test_environment_progression(
        ...
        assert (
            infra_complexity[dev.infrastructure]
            <= infra_complexity[staging.infrastructure]
        )
        assert (
            infra_complexity[staging.infrastructure]
            <= infra_complexity[prod.infrastructure]
        )

        # Monitoring should become more comprehensive
        monitoring_levels = {"basic": 0, "full": 1, "enterprise": 2}
        assert (
            monitoring_levels[dev.monitoring_level]
            <= monitoring_levels[staging.monitoring_level]
        )
        assert (
            monitoring_levels[staging.monitoring_level]
            <= monitoring_levels[prod.monitoring_level]
        )
...
class TestConfigurationDrift:
    ...
    def test_detect_configuration_drift(self, sample_configs: dict[str, Path]):
        ...
        assert any(
            "vector_db.collection_name" in drift["path"] for drift in staging_prod_drift
        )

    @pytest.mark.environment
    def test_acceptable_drift_vs_problematic_drift(
        self, sample_configs: dict[str, Path]
    ):
        ...
        host_drifts = [
            d for d in categorized["acceptable"] if "database.host" in d["path"]
        ]
        ...
        if any("app.name" in d["path"] or "app.version" in d["path"] for d in drift):
            problematic_drifts = [
                d
                for d in categorized["problematic"]
    ...
    def detect_drift(
        ...
        for key in all_keys:
            ...
            if key not in config1:
                ...
                nested_drift = self.detect_drift(
...
class SecretsValidator:
    ...
    def _scan_dict(
        self, obj: Any, violations: list[dict[str, Any]], path: str = ""
    ) -> None:
        ...
        if isinstance(obj, dict):
            ...
            value_looks_like_secret = any(
                __import__("re").match(pattern, obj)
        ...
        for var_name, requirements in var_definitions.items():
            ...
            if value:
                ...
                if (
                    ...
                    errors.append(
                        f"Value too short (minimum {requirements['min_length']} characters)"
                    )
                ...
                if (
                    "allowed_values" in requirements
                    and value not in requirements["allowed_values"]
                ):
                    errors.append(
                        f"Value not in allowed list: {requirements['allowed_values']}"
                    )
