    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                elif any(decorator.id == 'pytest.fixture' if hasattr(decorator, 'id') 
        ...
        if category_results["directory_exists"]:
            for file_name in files:
                ...
                if file_path.exists():
                    ...
                    validation_results["summary"]["total_tests"] += file_results["test_count"]
                else:
...
def print_validation_report(results: dict):
    ...
    print(f"Complete Categories: {summary['categories_complete']}/{len(results['categories'])}")
    print()
