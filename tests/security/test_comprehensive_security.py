...
class TestDistributedRateLimiter:
    ...
    async def test_rate_limit_basic_functionality(self, rate_limiter):
        ...
        for _ in range(limit):
            is_allowed, info = await rate_limiter.check_rate_limit(identifier, limit, window)
            assert is_allowed
    ...
    async def test_rate_limit_burst_capacity(self, rate_limiter):
        ...
        for i in range(burst_limit):
            ...
            assert is_allowed, f"Request {i+1} should be allowed (burst limit: {burst_limit})"
