        ...
        with self.client.post(
            ...
            json={
                ...
                "filters": {
                    ...
                    "metadata.difficulty": random.choice(["beginner", "intermediate", "advanced"]),
                },
            },
            catch_response=True,
...
class LoadTestMetricsCollector:
    ...
    def add_request_metric(self, request_type: str, name: str, response_time: float, 
                          content_length: int, exception: Optional[Exception]):
        """Add request metric."""
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        test_duration = (self.test_end_time or time.time()) - (self.test_start_time or time.time())
        ...
        summary = {
            ...
            "response_times_ms": {
                ...
                "mean": (sum(response_times) / len(response_times)) * 1000 if response_times else 0,
    ...
    def _check_thresholds(self) -> List[str]:
        ...
        if avg_response_time > self.performance_thresholds["max_response_time_ms"]:
            violations.append(f"Average response time {avg_response_time:.1f}ms exceeds threshold {self.performance_thresholds['max_response_time_ms']}ms")
        
        # Check error rate threshold
        error_rate = (len(failed_requests) / len(self.request_metrics)) * 100
        if error_rate > self.performance_thresholds["max_error_rate_percent"]:
            violations.append(f"Error rate {error_rate:.1f}% exceeds threshold {self.performance_thresholds['max_error_rate_percent']}%")
        ...
        if response_times:
            ...
            if p95_time > self.performance_thresholds["max_p95_response_time_ms"]:
                violations.append(f"P95 response time {p95_time:.1f}ms exceeds threshold {self.performance_thresholds['max_p95_response_time_ms']}ms")
...
def save_load_test_report(summary: Dict[str, Any], environment: Environment):
    ...
    try:
        ...
        logger.error(f"Failed to save load test report: {e}")


def create_load_test_environment(
    host: str = "http://localhost:8000",
    user_classes: Optional[List] = None,
    **kwargs
) -> Environment:
