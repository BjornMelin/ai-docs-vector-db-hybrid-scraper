...
class FailureInjector:
    ...
    async def inject_network_failure(self, failure_rate: float = 0.3, duration: float = 30.0):
        ...
        logger.warning(f"Injecting network failures: {failure_rate:.1%} rate for {duration}s")
    ...
    async def inject_memory_pressure(self, pressure_mb: int = 100, duration: float = 30.0):
    ...
    async def inject_cpu_saturation(self, cpu_load: float = 0.8, duration: float = 30.0):
        ...
        def cpu_intensive_work():
            ...
            rest_duration = work_duration * (1 - cpu_load) / cpu_load if cpu_load > 0 else 0
    ...
    async def inject_disk_io_stress(self, io_intensity: float = 0.5, duration: float = 30.0):
        ...
        logger.warning(f"Injecting disk I/O stress: {io_intensity:.1%} intensity for {duration}s")
        ...
        def disk_io_work():
            ...
            while not stop_io_work.is_set():
                try:
                    ...
                    # Write data
                    data = b"x" * int(1024 * 1024 * io_intensity)  # Variable size based on intensity
    ...
    def get_failure_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_types": list(set(f["type"] for f in self.active_failures.values())),
...
class StressTestOrchestrator:
        ...
        try:
            ...
            if scenario.failure_type == "network":
                await self.failure_injector.inject_network_failure(
                    failure_rate=scenario.intensity,
                ...
                await self.failure_injector.inject_cpu_saturation(
                    cpu_load=scenario.intensity,
                    duration=scenario.duration
                )
            
            elif scenario.failure_type == "disk":
                await self.failure_injector.inject_disk_io_stress(
                    io_intensity=scenario.intensity,
                    duration=scenario.duration
                )
            
            # Recovery phase
            if scenario.recovery_time > 0:
                logger.info(f"Recovery phase for {scenario.name}: {scenario.recovery_time}s")
    ...
    async def run_multi_phase_stress_test(self, phases: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, phase in enumerate(phases):
            logger.info(f"Starting stress test phase {i+1}/{len(phases)}: {phase.get('name', f'Phase {i+1}')}")
            ...
            try:
                ...
                if "max_fds" in constraints:
                    constraint_contexts.append(
                        self.resource_manager.constrain_file_descriptors(constraints["max_fds"])
                    )
                ...
                logger.error(f"Stress test phase {i+1} failed: {e}")
...
STRESS_TEST_PROFILES = {
    ...
    "extreme_stress": StressTestProfile(
        ...
        chaos_scenarios=["memory_pressure", "cpu_spike", "network_failure", "disk_io_stress"],
...
def stress_test_profile():
    ...
    def get_profile(profile_name: str) -> StressTestProfile:
        return STRESS_TEST_PROFILES.get(profile_name, STRESS_TEST_PROFILES["light_stress"])
    return get_profile
...
def stress_test_environment():
    ...
    logger.info(f"System: {psutil.cpu_count()} CPUs, {psutil.virtual_memory().total / (1024**3):.2f} GB RAM")
    
    # Check resource limits
    if os.name == 'posix':
        try:
            fd_limit = resource.getrlimit(resource.RLIMIT_NOFILE)
            logger.info(f"File descriptor limit: {fd_limit[0]} (soft), {fd_limit[1]} (hard)")
