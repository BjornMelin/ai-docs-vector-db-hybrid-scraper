...
class MockRateLimiter:
    ...
    async def allow_request(self) -> bool:
        ...
        refill_amount = int(time_since_refill * (self.config.max_requests / self.config.time_window))
        if refill_amount > 0:
            self.burst_allowance = min(self.config.burst_size, self.burst_allowance + refill_amount)
            self.last_refill = current_time
        
        # Check rate limit
        current_requests = len(self.request_times)
        self.metrics.current_rate = current_requests / self.config.time_window * 60  # requests per minute
...
class TestCircuitBreakers:
    ...
    async def test_circuit_breaker_trigger_points(self, load_test_runner):
        ...
        failure_scenarios = [
            ...
            {"rate": 0.3, "duration": 30, "expected_open": False},  # Moderate failure rate
            {"rate": 0.7, "duration": 30, "expected_open": True},   # High failure rate
            {"rate": 0.9, "duration": 30, "expected_open": True},   # Very high failure rate
        ]
        
        scenario_results = []
        
        for i, scenario in enumerate(failure_scenarios):
            logger.info(f"Testing circuit breaker with {scenario['rate']:.1%} failure rate")
            ...
            async def circuit_protected_request(**kwargs):
                ...
                return await circuit_breaker.call(failing_service.process_request, **kwargs)
        ...
        # Assertions
        assert len(scenario_results) == len(failure_scenarios), "Not all scenarios completed"
    ...
    async def test_circuit_breaker_recovery_behavior(self, load_test_runner):
        ...
        phases = [
            ...
            {"name": "failure", "duration": 15, "service_healthy": False},  # Trigger circuit open
            {"name": "recovery_wait", "duration": 8, "service_healthy": True},  # Service recovers, CB still open
            {"name": "recovery_test", "duration": 20, "service_healthy": True},  # CB should transition to half-open and close
        ]
        ...
        for phase in phases:
            ...
            phase_metrics = {
                "requests": final_metrics["total_requests"] - initial_metrics["total_requests"],
                "rejections": final_metrics["rejections"] - initial_metrics["rejections"],
                "state_changes": final_metrics["opened_count"] + final_metrics["closed_count"] - 
                               initial_metrics["opened_count"] - initial_metrics["closed_count"],
            }
        ...
        recovery_test_phase = next(p for p in phase_results if p["phase"] == "recovery_test")
        ...
        total_state_changes = sum(p["phase_metrics"]["state_changes"] for p in phase_results)
        assert total_state_changes >= 2, \
            f"Expected at least 2 state changes (open -> half-open -> closed), got {total_state_changes}"
        
        logger.info(f"Circuit breaker recovery completed successfully")
...
class TestRateLimiters:
    ...
    async def test_rate_limiter_effectiveness(self, load_test_runner):
        ...
        load_patterns = [
            {"name": "under_limit", "users": 20, "rps": 30, "duration": 60},  # Under rate limit
            {"name": "at_limit", "users": 30, "rps": 50, "duration": 60},     # At rate limit
            {"name": "over_limit", "users": 50, "rps": 80, "duration": 60},   # Over rate limit
        ...
        for pattern in load_patterns:
            logger.info(f"Testing rate limiter with load pattern: {pattern['name']}")
    ...
    async def test_thundering_herd_protection(self, load_test_runner):
        ...
        class ExpensiveService:
            ...
            async def expensive_operation(self, **kwargs):
                ...
                try:
                    ...
                    processing_time = 2.0 + (self.concurrent_executions * 0.5)  # Slower with more concurrency
        ...
        if expensive_service.execution_times:
            avg_execution_time = sum(expensive_service.execution_times) / len(expensive_service.execution_times)
            assert avg_execution_time < 5.0, \
                f"Expensive service performance degraded: {avg_execution_time:.2f}s average"
        
        logger.info(f"Thundering herd protection successful:")
        logger.info(f"  - {rejection_rate:.2f}% requests rejected")
        logger.info(f"  - Max concurrent executions: {expensive_service.max_concurrent}")
        logger.info(f"  - Total expensive operations: {expensive_service.total_executions}")
        logger.info(f"  - Service remained responsive")
