...
class SystemMonitor:
    ...
    def _monitoring_loop(self):
        ...
        while self.monitoring:
            try:
                ...
                last_disk_io = (system_metrics.disk_io_read_mb + system_metrics.disk_io_write_mb, time.time())
                last_network_io = (system_metrics.network_io_sent_mb + system_metrics.network_io_recv_mb, time.time())
    ...
    def _collect_system_metrics(self, last_disk_io: Optional[tuple[float, float]], 
        ...
        try:
            tcp_connections = len([c for c in self.process.connections() if c.type == psutil.SOCK_STREAM])
        except (psutil.AccessDenied, psutil.NoSuchProcess):
            tcp_connections = 0
    ...
    def _collect_application_metrics(self) -> ApplicationMetrics:
        ...
        active_requests = len([m for m in self.system_metrics[-10:] if m.cpu_percent > 10]) if self.system_metrics else 0
        ...
        for metric_name, value, description in alerts_to_check:
            if metric_name in self.alert_thresholds:
                ...
                if value >= thresholds["critical"]:
                    self._create_alert(metric_name, value, "critical", 
                                     f"{description} critical: {value:.2f}", current_time)
                elif value >= thresholds["warning"]:
                    self._create_alert(metric_name, value, "warning", 
                                     f"{description} warning: {value:.2f}", current_time)
    
    def _create_alert(self, metric_name: str, value: float, severity: str, 
        ...
        recent_alerts = [a for a in self.performance_alerts 
                        if a.metric_name == metric_name and a.severity == severity 
    ...
    def get_metrics_summary(self) -> dict[str, Any]:
        ...
        return {
            "collection_duration": self.system_metrics[-1].timestamp - self.system_metrics[0].timestamp,
...
class MetricsCollector:
    ...
    def get_performance_analysis(self) -> dict[str, Any]:
        ...
        response_analysis = {
            ...
            "std_dev_ms": statistics.stdev(response_times_ms) if len(response_times_ms) > 1 else 0,
        }
...
class TestSystemMonitoring:
    ...
    async def test_comprehensive_system_monitoring(self, load_test_runner):
        ...
        try:
            ...
            class SystemStressingService:
                ...
                async def stress_system(self, stress_type: str = "mixed", **kwargs):
                    ...
                    try:
                        ...
                        metrics_collector.record_custom_metric("memory_allocations", len(self.memory_hogs))
                        metrics_collector.record_custom_metric("cpu_tasks", len(self.cpu_intensive_tasks))
            ...
            stress_patterns = [
                {"name": "memory_stress", "stress_type": "memory", "users": 30, "duration": 45},
                {"name": "cpu_stress", "stress_type": "cpu", "users": 20, "duration": 45},
                {"name": "io_stress", "stress_type": "io", "users": 25, "duration": 45},
                {"name": "mixed_stress", "stress_type": "mixed", "users": 40, "duration": 60},
            ]
            ...
            assert len(monitor.application_metrics) > 0, "No application metrics collected"
            
            # Verify metrics capture system stress
            cpu_stressed = any(p for p in pattern_results 
                             if p["stress_type"] == "cpu" and 
                             p["metrics_after"]["system_metrics"]["cpu"]["max"] > 30)
            assert cpu_stressed, "CPU stress not captured in metrics"
            
            memory_stressed = any(p for p in pattern_results 
                                if p["stress_type"] == "memory" and 
                                p["metrics_after"]["system_metrics"]["memory"]["max"] > 
                                p["metrics_before"]["system_metrics"]["memory"]["max"])
            assert memory_stressed, "Memory stress not captured in metrics"
            
            # Verify performance analysis
            assert performance_analysis["response_times"]["count"] > 0, "No response times recorded"
            assert performance_analysis["duration_seconds"] > 0, "Invalid test duration"
            
            # Check for performance alerts
            if monitor.performance_alerts:
                alert_types = set(alert.severity for alert in monitor.performance_alerts)
                logger.info(f"Performance alerts generated: {len(monitor.performance_alerts)} ({alert_types})")
            
            logger.info(f"System monitoring test completed:")
            logger.info(f"  - Total metrics collected: {final_metrics['total_data_points']}")
            logger.info(f"  - Performance alerts: {final_metrics['total_alerts']}")
            logger.info(f"  - Peak CPU: {final_metrics['system_metrics']['cpu']['max']:.2f}%")
            logger.info(f"  - Peak memory: {final_metrics['system_metrics']['memory']['max']:.2f}%")
            logger.info(f"  - Response time P95: {performance_analysis['response_times']['p95_ms']:.2f}ms")
    ...
    async def test_performance_degradation_detection(self, load_test_runner):
        ...
        try:
            ...
            class DegradingService:
                ...
                async def degrading_operation(self, **kwargs):
                    ...
                    if self.call_count % 100 == 0:
                        ...
                        logger.info(f"Performance degraded: factor = {self.degradation_factor:.2f}")
                    ...
                    # Record metrics
                    metrics_collector.record_response_time(response_time)
                    metrics_collector.record_custom_metric("degradation_factor", self.degradation_factor)
            ...
            async def collect_degradation_sample():
                ...
                while metrics_collector.start_time and time.time() - metrics_collector.start_time < config.duration_seconds:
                    await asyncio.sleep(30)  # Sample every 30 seconds
                    
                    if degrading_service.call_count > 0:
                        # Analyze recent response times
                        recent_times = list(metrics_collector.response_times)[-50:]  # Last 50 requests
                        if recent_times:
                            avg_response_time = statistics.mean(recent_times) * 1000  # Convert to ms
            ...
            assert len(degradation_samples) >= 5, "Insufficient degradation samples collected"
            
            # Verify performance degraded over time
            first_sample = degradation_samples[0]
            last_sample = degradation_samples[-1]
            
            degradation_ratio = last_sample["avg_response_time_ms"] / first_sample["avg_response_time_ms"]
            assert degradation_ratio > 1.5, f"Insufficient performance degradation detected: {degradation_ratio:.2f}x"
            
            # Verify degradation factor increased
            assert last_sample["degradation_factor"] > first_sample["degradation_factor"], \
            ...
            if total_errors > 0:
                error_rate = (total_errors / performance_analysis["response_times"]["count"]) * 100
                assert error_rate > 0, "No errors recorded despite performance degradation"
            
            logger.info(f"Performance degradation detected:")
            logger.info(f"  - Degradation ratio: {degradation_ratio:.2f}x")
            logger.info(f"  - Initial response time: {first_sample['avg_response_time_ms']:.2f}ms")
            logger.info(f"  - Final response time: {last_sample['avg_response_time_ms']:.2f}ms")
            logger.info(f"  - Error rate: {performance_analysis['error_rate_percent']:.2f}%")
            logger.info(f"  - P95 response time: {performance_analysis['response_times']['p95_ms']:.2f}ms")
