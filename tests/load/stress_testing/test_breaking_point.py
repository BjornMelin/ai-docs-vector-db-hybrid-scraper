...
class TestBreakingPointAnalysis:
    ...
    async def test_throughput_breaking_point(
        ...
        class ThroughputBreakingPointAnalyzer:
            ...
            async def find_throughput_limit(self, max_rps: int = 200, step_size: int = 20):
                ...
                for target_rps in range(step_size, max_rps + 1, step_size):
                    ...
                    test_result = {
                        ...
                        "system_stable": success_rate >= 80.0 and avg_response_time < 2.0,
                    }
                    ...
                    if success_rate < 50.0:
                        print(f"System breaking detected at {target_rps} RPS (success rate: {success_rate:.1f}%)")
                        break
                
                # Analyze results to find breaking point
                stable_results = [r for r in test_results if r["system_stable"]]
                unstable_results = [r for r in test_results if not r["system_stable"]]
                
                if stable_results:
                    max_stable_rps = max(r["actual_rps"] for r in stable_results)
                    breaking_point = next((r for r in test_results if not r["system_stable"]), None)
                else:
                    max_stable_rps = 0
        ...
        if analysis["breaking_point"]:
            ...
            print(f"Breaking point: {bp['target_rps']} RPS target ({bp['actual_rps']:.2f} actual)")
    ...
    async def test_concurrent_users_breaking_point(
        ...
        class ConcurrentUsersBreakingPointAnalyzer:
            ...
            async def find_concurrency_limit(self, max_users: int = 1000, step_size: int = 50):
                ...
                for target_users in range(step_size, max_users + 1, step_size):
                    ...
                    test_result = {
                        ...
                        "system_stable": success_rate >= 75.0 and avg_response_time < 3.0,
                    }
                    ...
                    if success_rate < 30.0:
                        print(f"System failure at {target_users} users (success rate: {success_rate:.1f}%)")
                        break
                    
                    # Stop if response times are extremely high
                    if avg_response_time > 10.0:  # 10 second average
                        print(f"Unacceptable response times at {target_users} users ({avg_response_time:.2f}s avg)")
                        break
        ...
        for result in analysis["all_results"]:
            ...
            print(f"  {result['concurrent_users']:3d} users: {result['success_rate']:5.1f}% success, {result['avg_response_time_ms']:6.1f}ms avg - {stability}")
    ...
    async def test_response_time_degradation_point(
        ...
        class ResponseTimeDegradationAnalyzer:
            ...
            async def analyze_response_time_degradation(self):
                ...
                for load_level in load_levels:
                    ...
                    print(f"Testing {load_level['name']}: {load_level['users']} users, {load_level['rps']} RPS")
                    ...
                    if result.metrics.response_times:
                        ...
                        cv = (stdev_response_time / avg_response_time) if avg_response_time > 0 else 0
                    else:
                        avg_response_time = 0
                    ...
                    degradation_factor = (avg_response_time / baseline_response_time) if baseline_response_time > 0 else 1
                    ...
                    degradation_result = {
                        ...
                        "success_rate": success_rate,
                        "total_requests": result.metrics.total_requests,
                        "acceptable_performance": avg_response_time < 2.0 and success_rate >= 90.0,
                    }
                    ...
                    if avg_response_time > 30.0:  # 30 second average
                        print(f"Stopping test due to extreme response times: {avg_response_time:.2f}s")
                        break
                
                # Analyze degradation pattern
                acceptable_results = [r for r in degradation_results if r["acceptable_performance"]]
                unacceptable_results = [r for r in degradation_results if not r["acceptable_performance"]]
                ...
                if unacceptable_results:
                    first_unacceptable = min(unacceptable_results, key=lambda x: x["users"])
                else:
                    first_unacceptable = None
                
                return {
                    "baseline_response_time_ms": baseline_response_time * 1000 if baseline_response_time else 0,
        ...
        if analysis["last_acceptable_load"]:
            ...
            print(f"Last acceptable load: {last_ok['users']} users ({last_ok['avg_response_time_ms']:.1f}ms avg)")
        
        if analysis["first_unacceptable_load"]:
            first_bad = analysis["first_unacceptable_load"]
            print(f"First unacceptable load: {first_bad['users']} users ({first_bad['avg_response_time_ms']:.1f}ms avg)")
        ...
        for result in analysis["all_results"]:
            ...
            print(f"  {result['load_level']:15s}: avg={result['avg_response_time_ms']:6.1f}ms, "
    ...
    async def test_error_cascade_breaking_point(
        ...
        class ErrorCascadeAnalyzer:
            ...
            async def error_prone_task(self, **kwargs):
                ...
                total_error_rate = min(0.8, base_error_rate + load_factor * 0.15 + cascade_error_rate)
                ...
                if random.random() < total_error_rate:
                    ...
                    raise Exception(f"Cascading error (cascade level: {self.error_cascade_level:.1f}, load: {load_factor:.2f})")
                else:
            ...
            async def find_error_cascade_point(self):
                ...
                for level in test_levels:
                    ...
                    print(f"Testing error cascade at {level['name']}: {level['users']} users, {level['rps']} RPS")
                    
                    result = await load_test_runner.run_load_test(
                        config=config,
                        target_function=self.error_prone_task
                    )
                    ...
                    if success_rate < 10.0:
                        print(f"System complete failure at {level['name']} ({success_rate:.1f}% success rate)")
                        break
                
                # Find cascade breaking point
                cascade_triggered = [r for r in cascade_results if r["cascade_triggered"]]
                system_failed = [r for r in cascade_results if r["system_failed"]]
                
                return {
                    "cascade_results": cascade_results,
                    "cascade_breaking_point": cascade_triggered[0] if cascade_triggered else None,
        ...
        if analysis["cascade_breaking_point"]:
            ...
            print(f"  Cascade level: {cbp['initial_cascade_level']:.1f} → {cbp['final_cascade_level']:.1f}")
        ...
        for result in analysis["cascade_results"]:
            ...
            print(f"  {result['level']:10s}: {result['success_rate']:5.1f}% success, "
        ...
        assert len([r for r in analysis["cascade_results"] if r["total_requests"] > 0]) > 0, "No meaningful test results"