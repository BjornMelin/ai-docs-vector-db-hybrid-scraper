...
class ResourceMonitor:
    ...
    def get_peak_usage(self) -> Dict[str, float]:
        ...
        return {
            ...
            "peak_connections": max(self.metrics.connection_count) if self.metrics.connection_count else 0,
            "peak_file_descriptors": max(self.metrics.file_descriptor_count) if self.metrics.file_descriptor_count else 0,
            "total_network_io_mb": max(self.metrics.network_io_mb) if self.metrics.network_io_mb else 0,
        }
...
class TestResourceExhaustion:
    ...
    async def test_memory_exhaustion_large_documents(self, load_test_runner):
        ...
        try:
            ...
            # Assertions
            assert memory_exhaustion_detected, "Memory exhaustion scenario was not triggered"
            assert peak_usage["peak_memory_mb"] > 512, f"Peak memory usage too low: {peak_usage['peak_memory_mb']} MB"
            ...
            if result.metrics.failed_requests > 0:
                memory_errors = sum(1 for e in result.metrics.errors if "memory" in e.lower())
                assert memory_errors < result.metrics.total_requests * 0.8, "Too many memory-related failures"
    ...
    async def test_cpu_saturation_parallel_embeddings(self, load_test_runner):
        ...
        try:
            ...
            assert peak_usage["peak_cpu_percent"] > 60, f"Peak CPU usage too low: {peak_usage['peak_cpu_percent']}%"
            ...
            if result.metrics.response_times:
                p95_response_time = sorted(result.metrics.response_times)[int(len(result.metrics.response_times) * 0.95)]
                assert p95_response_time < 10.0, f"P95 response time too high under CPU load: {p95_response_time:.2f}s"
    ...
    async def test_network_bandwidth_stress(self, load_test_runner):
        ...
        try:
            ...
            async def network_intensive_operation(**kwargs):
                ...
                network_delay = min(payload_size_mb / 10, 2.0)  # Scale delay with payload size
                await asyncio.sleep(network_delay)
                
                large_payload_responses.append(large_payload)
                
                # Check if we've accumulated enough data to stress network
                total_data_mb = sum(len(p) for p in large_payload_responses) / (1024 * 1024)
                if total_data_mb > 100:  # 100MB total
                    network_stress_detected = True
                    logger.warning(f"Network stress detected: {total_data_mb:.2f} MB transferred")
            ...
            total_data_processed = sum(len(p) for p in large_payload_responses) / (1024 * 1024)
            assert total_data_processed > 50, f"Insufficient data processed: {total_data_processed:.2f} MB"
    ...
    async def test_database_connection_pool_exhaustion(self, load_test_runner):
        ...
        try:
            ...
            # Assertions
            assert pool_exhaustion_detected, "Connection pool exhaustion was not triggered"
            assert pool.pool_exhausted_count > 0, "No pool exhaustion events detected"
            assert pool.connection_requests > pool.max_connections, "Not enough connection requests to stress pool"
            
            # Verify graceful handling of pool exhaustion
            pool_errors = sum(1 for e in result.metrics.errors if "pool exhausted" in e.lower())
            total_requests = result.metrics.total_requests
            if total_requests > 0:
                pool_error_rate = pool_errors / total_requests
                assert pool_error_rate < 0.8, f"Too many pool exhaustion errors: {pool_error_rate:.2%}"
            ...
            with resource_limit("fds", 100):  # Very low limit
                ...
                async def file_intensive_operation(**kwargs):
                    ...
                    try:
                        ...
                        for i in range(5):  # Try to open multiple files per request
                            temp_file = tempfile.NamedTemporaryFile(delete=False)
                            open_files.append(temp_file)
                            
                            # Write some data
                            temp_file.write(b"test data for file descriptor stress test")
                            temp_file.flush()
                ...
                try:
                    ...
                    # Assertions
                    assert fd_exhaustion_detected or peak_usage["peak_file_descriptors"] > 50, \
                        "File descriptor stress scenario was not triggered sufficiently"
                    
                    # Verify system handling of FD exhaustion
                    fd_errors = sum(1 for e in result.metrics.errors if "file descriptor" in e.lower())
                    if result.metrics.total_requests > 0:
                        assert fd_errors > 0, "Expected some file descriptor exhaustion errors"
    ...
    async def test_cascading_resource_failure(self, load_test_runner):
        ...
        try:
                ...
                try:
                    ...
                    if len(active_failures) >= 2:
                        ...
                        if len(active_failures) >= 3:
                            raise Exception(f"System overload: {', '.join(active_failures)}")
            ...
            # Assertions
            assert cascading_failure_detected, "Cascading failure scenario was not triggered"
            
            # Verify multiple resource types were stressed
            active_failure_types = sum(1 for count in failure_types.values() if count > 0)
            assert active_failure_types >= 2, f"Not enough resource types stressed: {failure_types}"
