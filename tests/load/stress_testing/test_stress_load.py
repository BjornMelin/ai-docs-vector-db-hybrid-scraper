...
from ..load_profiles import BreakpointLoadProfile, StepLoadProfile
...
class TestStressLoad:
    ...
    def test_breaking_point_identification(self, load_test_runner):
        ...
        assert degradation_curve["is_gradual"], "System failed catastrophically instead of degrading gracefully"
    
    @pytest.mark.stress
    def test_resource_exhaustion(self, load_test_runner, mock_load_test_service):
        """Test system behavior under resource exhaustion."""
        # Configure for resource exhaustion
        mock_load_test_service.set_base_latency(0.5)  # High latency to simulate exhaustion
        ...
        assert resource_metrics["timeout_errors"] < result.metrics.total_requests * 0.5, \
    ...
    def test_cascading_failure_prevention(self, load_test_runner):
        ...
        async def test_with_dependencies(**kwargs):
            ...
            try:
                ...
                failed_services = sum(1 for h in services.service_health.values() if not h)
                if failed_services > 1:
                    raise Exception(f"Cascading failure: {failed_services} services down")
                raise
        ...
        cascading_failures = sum(1 for e in result.metrics.errors if "Cascading failure" in e)
        assert cascading_failures == 0, f"System experienced {cascading_failures} cascading failures"
        
        # Verify circuit breaker behavior
        assert services.failure_counts["embedding"] < 1000, "Circuit breaker should have prevented excessive calls"
    ...
    def _analyze_degradation(self, step_metrics: list[Dict]) -> Dict:
        ...
        for i in range(1, len(step_metrics)):
            error_delta = step_metrics[i]["error_rate"] - step_metrics[i-1]["error_rate"]
            response_delta = step_metrics[i]["avg_response_time"] - step_metrics[i-1]["avg_response_time"]
    ...
    def _analyze_recovery(self, phase_metrics: Dict) -> Dict:
        ...
        baseline_avg = sum(phase_metrics["baseline"]["response_times"]) / max(len(phase_metrics["baseline"]["response_times"]), 1)
        recovery_avg = sum(phase_metrics["recovery"]["response_times"]) / max(len(phase_metrics["recovery"]["response_times"]), 1)
    ...
    def _analyze_memory_usage(self, memory_samples: list[Dict]) -> Dict:
        ...
        duration_minutes = (memory_samples[-1]["timestamp"] - memory_samples[0]["timestamp"]) / 60
