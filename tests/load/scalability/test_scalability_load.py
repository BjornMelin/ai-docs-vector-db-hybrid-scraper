...
from ..base_load_test import VectorDBUser, create_load_test_runner
from ..conftest import LoadTestConfig, LoadTestType
from ..load_profiles import StepLoadProfile, LoadStage
...
class TestScalabilityLoad:
    ...
    def test_horizontal_scaling_pattern(self, load_test_runner):
        ...
        scaling_analysis = self._analyze_horizontal_scaling(scaling_metrics, scaling_stages)
    ...
    def test_vertical_scaling_validation(self, load_test_runner):
        ...
        class VerticalScalingSimulator:
            ...
            def scale_up(self, cpu_multiplier: float = 2.0, memory_multiplier: float = 2.0):
                """Scale up resources."""
                ...
                logger.info(f"Scaled up: CPU {old_cpu} -> {self.cpu_cores}, Memory {old_memory}GB -> {self.memory_gb}GB")
            ...
            def get_processing_capacity(self, workload_complexity: float) -> float:
                ...
                memory_utilization = min(0.9, workload_complexity / (self.memory_gb * 2))
            ...
            def get_scaling_stats(self) -> Dict:
                ...
                avg_cpu_util = sum(r["cpu_utilization"] for r in self.resource_utilization) / len(self.resource_utilization)
                avg_memory_util = sum(r["memory_utilization"] for r in self.resource_utilization) / len(self.resource_utilization)
        ...
        scaling_analysis = self._analyze_vertical_scaling(vertical_scaler.scaling_events, 
                                                        vertical_scaler.resource_utilization)
        
        # Assertions
        assert scaling_stats["scaling_events"] > 0, "No vertical scaling events occurred"
        assert scaling_stats["avg_cpu_utilization"] < 0.9, "CPU utilization too high after scaling"
        assert scaling_analysis["scaling_effectiveness"] > 0.7, "Vertical scaling not effective"
    ...
    def test_auto_scaling_triggers(self, load_test_runner):
        ...
        class AutoScalingManager:
            ...
            def update_metrics(self, cpu_utilization: float, memory_utilization: float, 
                ...
                elif memory_utilization > 85 and self.current_instances < self.max_instances:
                    self._scale_up("high_memory_utilization", memory_utilization)
                elif response_time > 2000 and self.current_instances < self.max_instances:  # 2 second threshold
            ...
            def _scale_up(self, reason: str, metric_value: float):
                ...
                logger.info(f"Scaled up: {old_instances} -> {self.current_instances} instances (reason: {reason})")
            ...
            def _scale_down(self, reason: str, metric_value: float):
                ...
                logger.info(f"Scaled down: {old_instances} -> {self.current_instances} instances (reason: {reason})")
            
            def get_current_capacity(self) -> float:
                """Get current system capacity based on instances."""
                return self.current_instances * 1.0  # Each instance provides 1.0 capacity unit
            ...
            def get_scaling_stats(self) -> Dict:
                ...
                scale_up_events = [d for d in self.scaling_decisions if d["action"] == "scale_up"]
                scale_down_events = [d for d in self.scaling_decisions if d["action"] == "scale_down"]
                ...
                return {
                    ...
                    "max_instances_reached": max([d["instances_after"] for d in self.scaling_decisions]),
                    "scaling_reasons": list(set([d["reason"] for d in self.scaling_decisions])),
                }
        ...
        scaling_analysis = self._analyze_auto_scaling(auto_scaler.scaling_decisions, 
                                                    auto_scaler.metrics_history)
        
        # Assertions
        assert scaling_stats["total_scaling_events"] > 5, "Insufficient auto-scaling activity"
        ...
        assert scaling_analysis["scaling_responsiveness"] > 0.7, "Auto-scaling not responsive enough"
    ...
    def test_database_scaling_limits(self, load_test_runner):
        ...
        class DatabaseScalingSimulator:
            ...
            async def execute_query(self, query_type: str = "read", complexity: float = 1.0):
                ...
                try:
                    ...
                    if query_type == "read":
                        ...
                        if pool["active"] > self.write_capacity / 10:  # Simplified capacity check
            ...
            def _check_scaling_needs(self):
                ...
                if (read_pool["active"] / read_pool["size"] > 0.8 and 
                    self.read_replicas < 5):
            ...
            def _add_read_replica(self):
                ...
                logger.info(f"Added read replica: {old_replicas} -> {self.read_replicas}")
            ...
            def get_database_stats(self) -> Dict:
                ...
                read_metrics = [m for m in self.performance_metrics if m["query_type"] == "read"]
                write_metrics = [m for m in self.performance_metrics if m["query_type"] == "write"]
                ...
                return {
                    ...
                    "scaling_events": len(self.scaling_events),
                    "avg_read_time": sum(m["total_time"] for m in read_metrics) / len(read_metrics) if read_metrics else 0,
                    "avg_write_time": sum(m["total_time"] for m in write_metrics) / len(write_metrics) if write_metrics else 0,
        ...
        scaling_analysis = self._analyze_database_scaling(db_scaler.scaling_events, 
                                                        db_scaler.performance_metrics)
    ...
    def _analyze_horizontal_scaling(self, metrics: List[Dict], stages: List[LoadStage]) -> Dict:
        ...
        for i, stage in enumerate(stages):
            ...
            if stage_data:
                ...
                avg_response_time = sum(m["avg_response_time"] for m in stage_data) / len(stage_data)
        ...
        user_scale_factor = peak["users"] / baseline["users"] if baseline["users"] > 0 else 1
        ...
        for i in range(1, len(stage_metrics)):
            expected_rps = baseline["rps"] * (stage_metrics[i]["users"] / baseline["users"])
            actual_rps = stage_metrics[i]["rps"]
            deviation = abs(expected_rps - actual_rps) / expected_rps if expected_rps > 0 else 0
        ...
        max_degradation = max(rt / baseline_time for rt in response_times) if baseline_time > 0 else 1
        response_time_stability = max(0.0, 1.0 - (max_degradation - 1) / 3)  # Allow 3x degradation
    ...
    def _analyze_vertical_scaling(self, scaling_events: List[Dict], 
        ...
        for event in scaling_events:
            ...
            before_metrics = [m for m in resource_utilization 
                            if event_time - 60 <= m["timestamp"] < event_time]
            
            # Get utilization after scaling (next 60 seconds)
            after_metrics = [m for m in resource_utilization 
                           if event_time < m["timestamp"] <= event_time + 60]
            
            if before_metrics and after_metrics:
                before_cpu = sum(m["cpu_utilization"] for m in before_metrics) / len(before_metrics)
                after_cpu = sum(m["cpu_utilization"] for m in after_metrics) / len(after_metrics)
    ...
    def _analyze_auto_scaling(self, scaling_decisions: List[Dict], 
        ...
        for decision in scaling_decisions:
            ...
            leading_metrics = [m for m in metrics_history 
        ...
        avg_responsiveness = sum(responsiveness_scores) / len(responsiveness_scores) if responsiveness_scores else 0
        ...
        if metrics_history:
            ...
            efficiency = max(0.0, 1.0 - abs(avg_cpu - ideal_utilization) / ideal_utilization)
        else:
            efficiency = 0
    ...
    def _analyze_database_scaling(self, scaling_events: List[Dict], 
        ...
        write_metrics = [m for m in performance_metrics if m["query_type"] == "write"]
        ...
        if read_metrics:
            ...
            if early_reads and late_reads:
                early_avg_time = sum(m["total_time"] for m in early_reads) / len(early_reads)
                late_avg_time = sum(m["total_time"] for m in late_reads) / len(late_reads)
                
                if early_avg_time > 0:
                    read_scaling_effectiveness = min(1.0, early_avg_time / late_avg_time)
        
        # Analyze connection pool efficiency
        connection_waits = [m["connection_wait"] for m in performance_metrics]
        avg_wait = sum(connection_waits) / len(connection_waits) if connection_waits else 0
        
        # Efficiency is inversely related to wait time
        connection_pool_efficiency = max(0.0, 1.0 - (avg_wait / 0.1))  # 100ms max acceptable wait
...
class HorizontalScalingSimulator:
    ...
    def get_scaling_factor(self, current_load: int) -> float:
        ...
        if target_units != self.current_units:
            ...
            logger.info(f"Scaled from {old_units} to {self.current_units} units (load: {current_load})")
