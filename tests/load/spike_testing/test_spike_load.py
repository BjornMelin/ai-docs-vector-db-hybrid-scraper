...
from ..load_profiles import SpikeLoadProfile, DoubleSpike
...
class TestSpikeLoad:
    ...
    def test_single_traffic_spike(self, load_test_runner):
        ...
        assert spike_analysis["recovery_time"] < 90, f"Recovery took too long: {spike_analysis['recovery_time']}s"
        assert spike_analysis["performance_degradation"] < 300, "Excessive performance degradation during spike"
    ...
    def test_double_spike_pattern(self, load_test_runner):
        ...
        # Assertions
        assert spike_recovery["first_spike_recovered"], "Failed to recover from first spike"
        assert spike_recovery["second_spike_handled"], "Failed to handle second spike"
        assert spike_recovery["system_stability"] > 0.8, "System unstable between spikes"
    ...
    def test_auto_scaling_response(self, load_test_runner, mock_load_test_service):
        ...
        class AutoScalingSimulator:
            ...
            def update_utilization(self, current_load: int, response_time: float):
                ...
                # Auto-scaling logic
                if utilization > self.scale_up_threshold and self.current_capacity < self.max_capacity:
                    self.scale_up()
                elif utilization < self.scale_down_threshold and self.current_capacity > 100:
                    self.scale_down()
            
            def scale_up(self):
                """Scale up capacity."""
                old_capacity = self.current_capacity
                self.current_capacity = min(self.current_capacity * 1.5, self.max_capacity)
            ...
            def scale_down(self):
                ...
                logger.info(f"Scaled down from {old_capacity} to {self.current_capacity}")
        ...
        # Assertions
        assert scaling_analysis["scaling_events"] > 0, "No auto-scaling events triggered"
        assert scaling_analysis["peak_capacity"] > 100, "Failed to scale up during spike"
    ...
    def test_circuit_breaker_activation(self, load_test_runner):
            ...
            async def call_service(self, **kwargs):
                ...
                if self.state == "OPEN":
                    if current_time - self.last_failure_time > self.timeout_duration:
                        ...
                        self.events.append({"timestamp": current_time, "event": "HALF_OPEN"})
                    else:
                ...
                try:
                    ...
                    if self.state == "HALF_OPEN":
                        ...
                        if self.success_count >= self.success_threshold:
                            ...
                            self.failure_count = 0
                            self.success_count = 0
                            self.events.append({"timestamp": current_time, "event": "CLOSED"})
        ...
        async def circuit_breaker_operation(**kwargs):
            ...
            kwargs["spike_intensity"] = min(kwargs.get("concurrent_users", 50) / 500.0, 0.3)
        ...
        # Assertions
        assert breaker_analysis["activation_count"] > 0, "Circuit breaker never activated"
        assert breaker_analysis["recovery_count"] > 0, "Circuit breaker never recovered"
        assert breaker_analysis["protection_effectiveness"] > 0.7, "Circuit breaker not effective"
    ...
    def _analyze_double_spike_recovery(self, metrics: list[Dict]) -> Dict:
        ...
        for i in range(1, len(metrics)):
            if metrics[i]["error_rate"] > metrics[i-1]["error_rate"] + 5:  # 5% increase
                error_spikes += 1
