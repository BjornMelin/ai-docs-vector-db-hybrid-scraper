...
class TestNormalLoad:
    ...
    def test_steady_state_load(self, load_test_runner):
        ...
        assert result.performance_grade in ["A", "B"], f"Poor performance grade: {result.performance_grade}"
        
        # Check response times
        p95_response_time = load_test_runner._percentile(result.metrics.response_times, 95)
        assert p95_response_time < 1.0, f"P95 response time too high: {p95_response_time}s"
    ...
    def test_gradual_ramp_up(self, load_test_runner):
        ...
        assert result.metrics.peak_concurrent_users >= end_users * 0.9, "Failed to reach target users"
        
        # Verify stable performance during hold phase
        hold_phase_metrics = self._get_phase_metrics(result.metrics, ramp_time, hold_time)
        assert hold_phase_metrics["error_rate"] < 0.05, "High error rate during hold phase"
    ...
    def test_mixed_operation_load(self, load_test_runner, mock_load_test_service):
        ...
        assert operation_stats["avg_read_time"] < operation_stats["avg_write_time"], "Read should be faster than write"
    ...
    def test_diurnal_pattern(self, load_test_runner):
        ...
        assert transition_errors == 0, f"Found {transition_errors} errors during transitions"
    ...
    def test_cache_effectiveness(self, load_test_runner, mock_load_test_service):
        ...
        # Assertions
        assert cache_metrics["hit_rate"] > 0.7, f"Low cache hit rate: {cache_metrics['hit_rate']}"
        assert cache_metrics["cached_response_time"] < cache_metrics["uncached_response_time"] * 0.5, \
    ...
    def _get_phase_metrics(self, metrics, start_time: float, duration: float) -> Dict:
        ...
        phase_errors = metrics.errors[start_idx:end_idx] if start_idx < len(metrics.errors) else []
        
        return {
            "response_times": phase_response_times,
            "error_rate": len(phase_errors) / max(len(phase_response_times), 1),
            "avg_response_time": sum(phase_response_times) / max(len(phase_response_times), 1),
        }
    ...
    def _analyze_operation_mix(self, result) -> Dict:
        ...
        total_ops = result.metrics.total_requests
