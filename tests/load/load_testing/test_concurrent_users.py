...
class TestConcurrentUsersLoad:
    ...
    async def test_concurrent_user_steady_state(
        ...
        result, stability = await steady_test.run_steady_state_test(
            duration_minutes=1
        )  # Short test for CI
    ...
    async def test_concurrent_mixed_workloads(
        ...
        for workload_config in workload_configs:
            ...
            result = await load_test_runner.run_load_test(
                config=config, target_function=workload_config["target_function"]
            )
        ...
        search_result = next(
            r for r in workload_results if r["workload"] == "search_heavy"
        )
        document_result = next(
            r for r in workload_results if r["workload"] == "document_heavy"
        )
        mixed_result = next(
            r for r in workload_results if r["workload"] == "mixed_workload"
        )

        # All workloads should have processed requests
        assert search_result["_total_requests"] > 0
        assert document_result["_total_requests"] > 0
        assert mixed_result["_total_requests"] > 0
