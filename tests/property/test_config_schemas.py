...
class TestCacheConfigProperties:
    ...
    @example({"enable_caching": False, "enable_local_cache": False, "enable_dragonfly_cache": False})
    @given(cache_configurations())
...
class TestChunkingConfigProperties:
    ...
    @given(st.lists(
        st.sampled_from(["python", "javascript", "typescript", "markdown", "java", "go", "rust", "c++", "swift"]),
        min_size=1,
        max_size=15,
        unique=True
    ))
