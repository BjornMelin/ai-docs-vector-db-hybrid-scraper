...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">🚀 AI Docs System - Interactive API Explorer</h1>', unsafe_allow_html=True)
    st.markdown("### Experience the production-grade AI documentation system in real-time")
    ...
    with tab1:
        ...
        st.markdown("Demonstrate advanced search capabilities with dense + sparse vectors and neural reranking")
        
        col1, col2 = st.columns([2, 1])
        
        with col1:
            query = st.text_input("Search Query", value="machine learning optimization techniques", key="search_query")
            ...
            with col1c:
                search_strategy = st.selectbox("Search Strategy", ["hybrid", "dense", "sparse"])
        ...
        if 'result' in locals() and result["success"]:
            ...
            if result["data"].get("results"):
                ...
                for i, doc in enumerate(result["data"]["results"][:5]):
                    with st.expander(f"Result {i+1}: {doc.get('title', 'Untitled')[:100]}...", expanded=i==0):
                        col1, col2 = st.columns([3, 1])
                        
                        with col1:
                            st.write(f"**Source**: {doc.get('source', 'Unknown')}")
                            st.write(doc.get('content', 'No content available')[:500] + "...")
                        
                        with col2:
                            st.metric("Relevance Score", f"{doc.get('score', 0):.3f}")
                            if doc.get('rerank_score'):
                                st.metric("Rerank Score", f"{doc.get('rerank_score', 0):.3f}")
    ...
    with tab2:
        ...
        st.markdown("Showcase intelligent tier selection from lightweight HTTP to full browser automation")
        
        col1, col2 = st.columns([2, 1])
        
        with col1:
            url = st.text_input("URL to Scrape", value="https://docs.python.org/3/", key="scrape_url")
            
            col2a, col2b = st.columns(2)
            with col2a:
                tier_preference = st.selectbox("Tier Preference", ["auto", "lightweight", "crawl4ai", "playwright"])
            with col2b:
        ...
        with col2:
            ...
            for demo_url in demo_urls:
                if st.button(f"🔗 {demo_url.split('//')[1][:25]}...", key=f"demo_url_{demo_url}"):
        ...
        if 'result' in locals() and result["success"]:
            ...
            with col2:
                st.metric("Scrape Time", f"{data.get('performance', {}).get('scrape_time_ms', 0)}ms")
            with col3:
            ...
            if data.get('content'):
                ...
                if content.get('metadata'):
                    ...
                    with col2:
                        st.metric("Reading Time", metadata.get('reading_time', 'Unknown'))
                    with col3:
                ...
                # Content preview
                st.text_area("Content Preview", content.get('text', '')[:1000] + "...", height=200)
        ...
        if 'result' in locals() and result["success"]:
            ...
            if data.get('components'):
                ...
                for comp_name, comp_data in components.items():
                    with st.expander(f"{comp_name.replace('_', ' ').title()}", expanded=True):
                        col1, col2, col3 = st.columns(3)
    ...
    with tab4:
        ...
        with col1:
            ...
            if st.button("🧠 Generate Embeddings", type="primary"):
                with st.spinner("Generating embeddings..."):
                    ...
                    result = make_api_request("/api/v1/embeddings/generate", "POST", payload)
        ...
        if 'result' in locals() and result["success"]:
            ...
            with col2:
                st.metric("Dimensions", len(data.get('embeddings', [{}])[0]) if data.get('embeddings') else 0)
            with col3:
            ...
            st.error(f"❌ Embedding generation failed: {result.get('error', 'Unknown error')}")
    ...
    with tab5:
        ...
        if st.session_state.performance_metrics:
            ...
            with col4:
                if len(metrics_df) > 1:
                    time_span = (metrics_df['timestamp'].max() - metrics_df['timestamp'].min()).total_seconds()
            ...
            st.info("📊 No performance data available yet. Try making some API requests!")
            
            # Sample performance data
            st.subheader("🎯 Expected Performance Targets")
            
            targets_data = {
                "Metric": ["P50 Latency", "P95 Latency", "P99 Latency", "Throughput", "Error Rate"],
    ...
    with col3:
        st.markdown("**🔗 Links**")
        st.markdown("[GitHub](https://github.com/BjornMelin/ai-docs-vector-db-hybrid-scraper) | [Docs](docs/portfolio/)")
