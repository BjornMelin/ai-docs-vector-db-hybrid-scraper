        ...
        for i, test_case in enumerate(test_queries, 1):
            ...
            if result["success"]:
                ...
                if 'result' in result:
                    ...
                    print(f"Plan: {result_data.get('orchestration_plan', 'No plan available')}")
            else:
...
async def demo_tool_composition():
    ...
    try:
        ...
        print(f"Initialized tool composition engine with {len(engine.tool_registry)} tools")
        ...
        goals = [
            ...
            },
            {
                ...
                "constraints": {"min_quality_score": 0.9, "performance_priority": "quality"},
        ...
        for i, goal_case in enumerate(goals, 1):
            ...
            tool_chain = await engine.compose_tool_chain(
                goal=goal_case["goal"],
                constraints=goal_case["constraints"]
            )
            
            print(f"Composed chain with {len(tool_chain)} steps:")
            for j, step in enumerate(tool_chain):
                print(f"  {j+1}. {step.tool_name} (parallel: {step.parallel}, optional: {step.optional})")
            ...
            if result["success"]:
                ...
                print(f"Steps executed: {metadata['steps_executed']}/{metadata['chain_length']}")
            else:
    ...
    try:
        ...
        for i, query in enumerate(interactions, 1):
            ...
            if result["success"]:
                ...
                print(f"  Conversation history length: {len(deps.session_state.conversation_history)}")
                print(f"  Performance metrics count: {len(deps.session_state.performance_metrics)}")
    ...
    try:
        ...
        optimization_scenarios = [
            ...
            },
            {
                ...
                "constraints": {"min_quality_score": 0.95, "performance_priority": "quality"},
        ...
        for scenario in optimization_scenarios:
            ...
            performance_data = {
                ...
                "reported_time_ms": result.get("metadata", {}).get("total_execution_time_ms", 0),
