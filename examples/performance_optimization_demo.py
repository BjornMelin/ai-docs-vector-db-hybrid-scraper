...
async def run_performance_demo():
    ...
    try:
            ...
            for metric, data in metrics.items():
                ...
                if improvement > 0:
                    ...
                    direction = (
                        "🔽"
                        if metric.endswith(
                            ("latency_ms", "error_rate", "cpu_percent", "memory_mb")
                        )
                        else "🔼"
                    )
                    print(f"   {direction} {metric}: {improvement:.1f}% improvement")
                    print(
                        f"      {data['baseline_value']:.3f} → {data['optimized_value']:.3f}"
                    )
            print()
        ...
        print(
            f"   • Technical Complexity: {results.technical_complexity_score:.2f}/1.0"
        )
        ...
        print(
            f"   • Infrastructure Cost Reduction: {cost_savings.get('infrastructure_cost_reduction_percent', 0):.1f}%"
        )
        print(
            f"   • Operational Efficiency Gain: {cost_savings.get('operational_efficiency_gain_percent', 0):.1f}%"
        )
        print(
            f"   • Capacity Increase: {scalability.get('capacity_increase_percent', 0):.1f}%"
        )
        print(
            f"   • Est. Annual Savings: ${cost_savings.get('estimated_annual_savings_usd', 0):,}"
        )
        print()
        ...
        print(
            f"   • Complete Results: {output_dir}/performance_optimization_results.json"
        )
        print(f"   • Summary Metrics: {output_dir}/performance_summary.json")
        print(
            f"   • Portfolio Documentation: {output_dir}/PERFORMANCE_OPTIMIZATION_SHOWCASE.md"
        )
        print()
...
async def run_quick_demo():
    ...
    try:
        ...
        high_priority_tasks = [
            execute_optimized(sample_workload(0.001), priority=TaskPriority.HIGH)
            for _ in range(10)
        ]

        normal_priority_tasks = [
            execute_optimized(sample_workload(0.01), priority=TaskPriority.NORMAL)
            for _ in range(20)
        ]

        # Execute all tasks
        results = await asyncio.gather(*high_priority_tasks, *normal_priority_tasks)
        ...
        print(
            f"   ✓ Current concurrency limit: {concurrency_metrics.get('current_limit', 'N/A')}"
        )
        ...
        print(
            f"   ✓ Cache layers active: {cache_stats.get('manager', {}).get('enabled_layers', [])}"
        )
