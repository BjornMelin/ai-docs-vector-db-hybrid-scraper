...
def demo_basic_encryption_operations():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        demo_configs = {
            ...
            "secret_config": {
                "data": {"encryption_key": "super-secret-key", "admin_token": "admin-123"},
        ...
        for config_name in demo_configs.keys():
...
def demo_audit_logging():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        encrypt_events = config_manager.get_audit_events(
            limit=10,
            operation_filter=ConfigOperationType.ENCRYPT
        )
...
def demo_security_monitoring_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        logger.info(f"  Encryption Enabled: {security_status.get('encryption_enabled')}")
        logger.info(f"  Audit Enabled: {security_status.get('audit_enabled')}")
        logger.info(f"  Integrity Checks: {security_status.get('integrity_checks_enabled')}")
        logger.info(f"  Backup Enabled: {security_status.get('backup_enabled')}")
        logger.info(f"  Total Configurations: {security_status.get('total_configurations')}")
        logger.info(f"  Key Version: {security_status.get('key_version')}")
        logger.info(f"  Security Monitoring: {security_status.get('security_monitoring_integration')}")
        logger.info(f"  Threat Detection: {security_status.get('threat_detection_enabled')}")
...
def demo_data_classification():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        classifications = [
            ...
            (ConfigDataClassification.CONFIDENTIAL, {"business_logic": {"algorithm": "proprietary"}}),
            (ConfigDataClassification.SECRET, {"credentials": {"api_key": "super-secret"}}),
        ]
        ...
        for classification, data in classifications:
            ...
            logger.info(f"Storing {classification.value.upper()} configuration: {config_name}")
            ...
            if success:
                logger.info(f"  ✓ {classification.value.upper()} config stored securely")
            else:
                logger.error(f"  ✗ Failed to store {classification.value.upper()} config")
        ...
        for classification in ConfigDataClassification:
            classification_events = [
                ...
                if e.data_classification == classification
            ]
            logger.info(f"  {classification.value.upper()}: {len(classification_events)} events")
