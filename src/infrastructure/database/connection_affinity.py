...
class QueryPattern:
    ...
    def add_connection_performance(self, connection_id: str, execution_time_ms: float) -> None:
        ...
        if (
            not self.preferred_connection_id or
            self.connection_performance[connection_id] < 
...
class ConnectionStats:
    ...
    query_type_counts: Dict[QueryType, int] = field(default_factory=lambda: defaultdict(int))
    
    # Resource utilization
    cpu_usage_percent: float = 0.0
    memory_usage_mb: float = 0.0
    
    def update_usage(self, query_type: QueryType, execution_time_ms: float, success: bool = True) -> None:
        ...
        if success:
            self.success_rate = (
                alpha + (1 - alpha) * self.success_rate
            )
        else:
            self.error_count += 1
    ...
    def get_load_score(self) -> float:
        ...
        base_load = min(1.0, self.active_queries / 10.0)  # Assume 10 concurrent queries = full load
        
        # Factor in recent usage
        time_since_last_use = time.time() - self.last_used
        recency_factor = max(0.0, 1.0 - (time_since_last_use / 60.0))  # Decay over 1 minute
        
        # Factor in response time (higher response time = higher load)
        response_time_factor = min(1.0, self.avg_response_time_ms / 1000.0)  # 1 second = full load
...
class ConnectionAffinityManager:
    ...
    def __init__(self, max_patterns: int = 1000, max_connections: int = 50):
        ...
        # Performance cache
        self.performance_cache: Dict[Tuple[str, str], float] = {}  # (pattern_id, connection_id) -> avg_time
    ...
    async def get_optimal_connection(
        ...
        if pattern.preferred_connection_id and pattern.preferred_connection_id not in exclude_connections:
            connection_id = pattern.preferred_connection_id
            if await self._is_connection_available(connection_id):
                await self._track_connection_selection(connection_id, pattern_id, "affinity")
                return connection_id
        
        # Fall back to specialized pool routing
        connection_id = await self._route_to_specialized_pool(query_type, exclude_connections)
        if connection_id:
            await self._track_connection_selection(connection_id, pattern_id, "specialized")
            return connection_id
        
        # Last resort: round-robin load balancing
        connection_id = await self._round_robin_selection(query_type, exclude_connections)
        if connection_id:
            await self._track_connection_selection(connection_id, pattern_id, "round_robin")
            return connection_id
    ...
    async def register_connection(
        ...
        logger.info(f"Registered connection {connection_id} with specialization {specialization.value}")
    ...
    async def get_connection_recommendations(self, query_type: QueryType) -> Dict[str, Any]:
        ...
        if load_scores and max(load_scores) - min(load_scores) > 0.5:
            recommendations.append("Consider redistributing load - significant imbalance detected")
        
        # Check for performance issues
        slow_connections = [
            conn_id for conn_id, stats in load_analysis.items()
            if stats["avg_response_time_ms"] > 500  # 500ms threshold
        ]
        if slow_connections:
            recommendations.append(f"Performance issue detected in connections: {slow_connections}")
    ...
    def _classify_query_type(self, query: str) -> QueryType:
        ...
        if any(keyword in query_lower for keyword in ['begin', 'commit', 'rollback', 'savepoint']):
        ...
        analytics_keywords = ['group by', 'having', 'window', 'partition by', 'over(', 'sum(', 'count(', 'avg(', 'max(', 'min(']
        ...
        if any(keyword in query_lower for keyword in ['analyze', 'vacuum', 'reindex', 'cluster']):
    ...
    async def _route_to_specialized_pool(
        ...
        preferred_specs = specialization_preference.get(query_type, [ConnectionSpecialization.GENERAL])
        
        for specialization in preferred_specs:
            available_connections = [
                conn_id for conn_id in self.specialized_pools[specialization]
                if conn_id not in exclude_connections and await self._is_connection_available(conn_id)
            ]
    ...
    async def _round_robin_selection(
        ...
        available_connections = [
            conn_id for conn_id in self.connection_stats.keys()
            if conn_id not in exclude_connections and await self._is_connection_available(conn_id)
        ]
        ...
        selected_connection = available_connections[counter % len(available_connections)]
            ...
            "connection_performance": {
                conn_id: {
                    ...
                    "query_type_distribution": {
                        qtype.value: count for qtype, count in stats.query_type_counts.items()
                    },
                }
