    ...
    async def take_configuration_snapshot(self) -> dict[str, Any]:
        ...
        monitoring_context = (
            performance_monitor.monitor_operation(
                "config_drift_snapshot_batch", 
                category="config_drift"
            ) if performance_monitor else None
        )

        try:
            async with monitoring_context if monitoring_context else asyncio.nullcontext():
                ...
                if monitoring_context and hasattr(monitoring_context, "__enter__"):
                    with monitoring_context as perf_data:
                        perf_data["custom_metrics"]["snapshots_taken"] = snapshot_results["snapshots_taken"]
                        perf_data["custom_metrics"]["sources_monitored"] = len(self.config.drift_detection.monitored_paths)
                        perf_data["custom_metrics"]["errors_count"] = len(snapshot_results["errors"])
    ...
    async def compare_configurations(self) -> dict[str, Any]:
        ...
        try:
            async with monitoring_context if monitoring_context else asyncio.nullcontext():
                for source in self.config.drift_detection.monitored_paths:
                    try:
                        ...
                        for event in events:
                            ...
                            if (self.config.drift_detection.enable_auto_remediation and 
                                event.auto_remediable):
                ...
                if monitoring_context and hasattr(monitoring_context, "__enter__"):
                    with monitoring_context as perf_data:
                        perf_data["custom_metrics"]["sources_compared"] = comparison_results["sources_compared"]
                        perf_data["custom_metrics"]["drift_events_detected"] = len(comparison_results["drift_events"])
                        perf_data["custom_metrics"]["alerts_sent"] = comparison_results["alerts_sent"]
                        perf_data["custom_metrics"]["errors_count"] = len(comparison_results["errors"])
