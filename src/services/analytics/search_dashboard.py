...
class QueryPattern(BaseModel):
    ...
    sample_queries: List[str] = Field(..., description="Sample queries matching pattern")
...
class SearchAnalyticsDashboard(BaseService):
    ...
    async def get_realtime_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            dashboard_data = {
                ...
                "query_patterns": [pattern.model_dump() for pattern in self.detected_patterns[:10]],
    ...
    async def get_query_analytics(
        ...
        try:
            ...
            filtered_queries = [
                ...
                (user_id is None or q["user_id"] == user_id)
            ]
            ...
            avg_processing_time = sum(q["processing_time_ms"] for q in filtered_queries) / total_queries
            ...
            analytics = {
                ...
                "temporal_patterns": await self._analyze_temporal_patterns(filtered_queries),
                "performance_distribution": self._calculate_distribution([q["processing_time_ms"] for q in filtered_queries]),
    ...
    async def get_optimization_recommendations(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if recent_queries:
                avg_time = sum(q["processing_time_ms"] for q in recent_queries) / len(recent_queries)
                cache_rate = sum(1 for q in recent_queries if q["cache_hit"]) / len(recent_queries)
            ...
            if self.detected_patterns:
                frequent_patterns = [p for p in self.detected_patterns if p.frequency > 10]
        ...
        try:
            ...
            if recent_queries:
                self.realtime_stats.update({
                    ...
                    "avg_response_time": sum(q["processing_time_ms"] for q in recent_queries) / len(recent_queries),
    ...
    async def _detect_query_patterns(self) -> None:
        ...
        try:
            ...
            for query_data in self.query_history[-200:]:  # Analyze last 200 queries
                ...
                patterns[pattern]["performance_times"].append(query_data["processing_time_ms"])
            ...
            for pattern, data in patterns.items():
                if len(data["queries"]) >= 3:  # At least 3 occurrences
                    query_pattern = QueryPattern(
                        ...
                        avg_performance=sum(data["performance_times"]) / len(data["performance_times"]),
            ...
            if all_features:
                ...
                if most_used[1] > len(self.query_history[-50:]) * 0.8:  # >80% usage
                    insights.append(UserBehaviorInsight(
                        ...
                        data_points={
                            "feature": most_used[0],
                            "usage_rate": most_used[1] / len(self.query_history[-50:])
                        }
                    ))
    ...
    async def _calculate_performance_trends(self) -> Dict[str, Any]:
        ...
        try:
            ...
            older_queries = self.query_history[-40:-20] if len(self.query_history) >= 40 else []
            
            trends = {}
            
            if older_queries:
                # Calculate trends
                recent_avg_time = sum(q["processing_time_ms"] for q in recent_queries) / len(recent_queries)
                older_avg_time = sum(q["processing_time_ms"] for q in older_queries) / len(older_queries)
                
                time_trend = "improving" if recent_avg_time < older_avg_time else "degrading"
                
                recent_success = sum(1 for q in recent_queries if q["success"]) / len(recent_queries)
                older_success = sum(1 for q in older_queries if q["success"]) / len(older_queries)
                
                success_trend = "improving" if recent_success > older_success else "stable"
                ...
                trends = {
                    ...
                    "success_rate": {
                        ...
                        "change_percent": ((recent_success - older_success) / older_success) * 100 if older_success > 0 else 0
                    }
                }
        ...
        try:
            ...
            for query_data in self.query_history:
                if query_data["timestamp"] >= cutoff:
                    hour = query_data["timestamp"].replace(minute=0, second=0, microsecond=0)
    ...
    async def _analyze_temporal_patterns(self, queries: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                ).total_seconds() / 3600 if len(queries) > 1 else 0
            }
