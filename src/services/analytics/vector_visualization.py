...
class ClusterInfo(BaseModel):
    ...
    centroid: Tuple[float, float] = Field(..., description="Cluster centroid coordinates")
...
class EmbeddingQualityMetrics(BaseModel):
    ...
    variance_explained: float = Field(..., description="Variance explained by top components")
...
class VectorVisualizationEngine(BaseService):
        ...
        try:
            ...
            if method.lower() == "pca":
                ...
                if embeddings_array.shape[1] > 50:
                    ...
                    n_components = min(50, embeddings_array.shape[0] - 1, embeddings_array.shape[1])
                    if n_components > 0:
            ...
            if query_embedding:
                ...
                similarities = cosine_similarity(embeddings_array, query_array).flatten()
            
            # Create vector points
            points = []
            for i, (coords, text) in enumerate(zip(reduced_embeddings, texts)):
                ...
                point = VectorPoint(
                    ...
                    cluster_id=clusters_info["assignments"][i] if clusters_info else None,
                    similarity_score=float(similarities[i]) if similarities is not None else None
                )
                points.append(point)
            ...
            visualization_data = {
                ...
                "query_point": {
                    "text": query_text,
                    "similarities_range": {
                        "min": float(np.min(similarities)) if similarities is not None else None,
                        "max": float(np.max(similarities)) if similarities is not None else None,
                        "mean": float(np.mean(similarities)) if similarities is not None else None
                    }
                ...
                "metadata": {
                    ...
                    "variance_preserved": getattr(reducer, "explained_variance_ratio_", []).sum() if hasattr(reducer, "explained_variance_ratio_") else None
                }
            }
    ...
    async def analyze_embedding_space(
        ...
        try:
            ...
            dimensionality_analysis = await self._analyze_dimensionality(embeddings_array)
            
            # Semantic coherence analysis
            coherence_analysis = await self._analyze_semantic_coherence(embeddings_array, texts)
            ...
            analysis = {
                "space_overview": {
                    ...
                    "avg_norm": float(np.mean(np.linalg.norm(embeddings_array, axis=1))),
                },
    ...
    async def compare_query_embeddings(
        ...
        try:
            ...
            comparisons = []
            for i, (query_text, query_similarities) in enumerate(zip(query_texts, similarities)):
                # Get top matches
                ...
                comparisons.append({
                    ...
                    "similarity_stats": {
                        ...
                        "selectivity": max_sim - avg_sim  # How selective the query is
                    }
                })
            ...
            overall_stats = {
                ...
                "similarity_distribution": {
                    "high_similarity": float(np.sum(all_similarities > 0.8) / len(all_similarities)),
                    "medium_similarity": float(np.sum((all_similarities > 0.5) & (all_similarities <= 0.8)) / len(all_similarities)),
                    "low_similarity": float(np.sum(all_similarities <= 0.5) / len(all_similarities))
                }
            }
            ...
            for comp in comparisons:
                ...
                quality_score = (stats["max_similarity"] * 0.7) + (stats["selectivity"] * 0.3)
            ...
            return {
                ...
                "recommendations": await self._generate_query_recommendations(query_qualities, overall_stats)
            }
    ...
    async def _find_similarity_relationships(
        ...
        try:
            ...
            if len(embeddings_array) > 100:
                ...
                num_samples = 200
                indices = np.random.choice(len(embeddings_array), min(num_samples, len(embeddings_array)), replace=False)
    ...
    async def _calculate_quality_metrics(
        ...
        embeddings_array: np.ndarray,
        reduced_embeddings: np.ndarray,
        clusters_info: Optional[Dict[str, Any]]
    ) -> EmbeddingQualityMetrics:
        """Calculate embedding quality metrics."""
        try:
            # Dimensionality metrics
            pca = PCA(n_components=min(50, embeddings_array.shape[1], embeddings_array.shape[0]))
            pca.fit(embeddings_array)
            variance_explained = float(np.sum(pca.explained_variance_ratio_[:10]))  # Top 10 components
            ...
            if len(embeddings_array) <= 100:
                ...
                embedding_density = float(np.mean(pairwise_distances)) if pairwise_distances else 0.0
            else:
    ...
    async def _calculate_distance_metrics(self, embeddings_array: np.ndarray) -> Dict[str, float]:
    ...
    async def _analyze_dimensionality(self, embeddings_array: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "variance_explained_by_top_10": float(np.sum(pca.explained_variance_ratio_[:10])),
                "singular_values_ratio": float(pca.singular_values_[0] / pca.singular_values_[-1]) if len(pca.singular_values_) > 1 else 1.0
            }
        ...
        try:
            ...
            for i in range(min(20, len(sample_texts))):  # Sample pairs
                for j in range(i + 1, min(i + 11, len(sample_texts))):  # Check next 10
                    ...
                    text_similarity = len(words_i & words_j) / len(words_i | words_j) if words_i | words_j else 0
            ...
            if coherence_scores:
                text_sims, emb_sims = zip(*coherence_scores)
                correlation = np.corrcoef(text_sims, emb_sims)[0, 1] if len(coherence_scores) > 1 else 0
            else:
                correlation = 0
            
            return {
                "semantic_correlation": float(correlation) if not np.isnan(correlation) else 0.0,
                "avg_embedding_similarity": float(np.mean(emb_sims)) if coherence_scores else 0.0,
    ...
    async def _analyze_embedding_density(self, embeddings_array: np.ndarray) -> Dict[str, Any]:
    ...
    async def _generate_query_recommendations(
        self,
        query_qualities: List[Dict[str, Any]],
        ...
        try:
            ...
            high_sim_rate = overall_stats.get("similarity_distribution", {}).get("high_similarity", 0)
            ...
            poor_quality_queries = [q for q in query_qualities if q["quality_grade"] == "poor"]
            ...
            low_selectivity_queries = [q for q in query_qualities if q["selectivity"] < 0.2]
