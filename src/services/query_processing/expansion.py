...
class ExpandedTerm(BaseModel):
    ...
    source: str = Field(..., description="Source of expansion (model, dictionary, etc.)")
...
class QueryExpansionRequest(BaseModel):
    ...
    enable_caching: bool = Field(
        True, description="Enable expansion result caching"
    )
...
class QueryExpansionService:
    ...
    async def expand_query(self, request: QueryExpansionRequest) -> QueryExpansionResult:
        ...
        try:
            ...
            expanded_terms = await self._generate_expansions(
                key_terms, request
            )
            
            # Filter and rank expanded terms
            filtered_terms = self._filter_and_rank_terms(
                expanded_terms, request
            )
            ...
            result = QueryExpansionResult(
                ...
                expansion_metadata={
                    ...
                    "expansion_sources": list(set(term.source for term in filtered_terms)),
                    "relation_types": list(set(term.relation_type for term in filtered_terms))
                }
            )
                ...
                if synonym != term and synonym not in (request.exclude_terms or []):
                    ...
                    if confidence >= request.min_confidence:
                        expanded_terms.append(ExpandedTerm(
                            ...
                            frequency_weight=self._get_term_frequency_weight(synonym)
                        ))
    ...
    async def _semantic_expansion(
        ...
        for term in key_terms:
            related_terms = self._get_semantically_related_terms(term, request.query_context)
            
            for related_term, similarity_score in related_terms:
                if related_term != term and related_term not in (request.exclude_terms or []):
                    confidence = similarity_score * 0.8  # Adjust for semantic uncertainty
                    ...
                    if confidence >= request.min_confidence:
                        expanded_terms.append(ExpandedTerm(
                            ...
                            frequency_weight=self._get_term_frequency_weight(related_term)
                        ))
    ...
    async def _context_aware_expansion(
        ...
        for term in key_terms:
            ...
            for context_term in context_terms:
                if context_term != term and context_term not in (request.exclude_terms or []):
                    ...
                    if confidence >= request.min_confidence:
                        expanded_terms.append(ExpandedTerm(
                            ...
                            frequency_weight=self._get_term_frequency_weight(context_term)
                        ))
        ...
        for term in key_terms:
            for domain in target_domains:
                ...
                for domain_term, relevance_score in domain_terms:
                    if domain_term != term and domain_term not in (request.exclude_terms or []):
                        ...
                        if confidence >= request.min_confidence:
                            expanded_terms.append(ExpandedTerm(
                                ...
                                frequency_weight=self._get_term_frequency_weight(domain_term)
                            ))
    ...
    async def _learning_based_expansion(
        ...
        for term in key_terms:
            ...
            for learned_term, success_score in learned_terms:
                if learned_term != term and learned_term not in (request.exclude_terms or []):
                    ...
                    if confidence >= request.min_confidence:
                        expanded_terms.append(ExpandedTerm(
                            ...
                            frequency_weight=self._get_term_frequency_weight(learned_term)
                        ))
    ...
    def _filter_and_rank_terms(
        ...
        for term in expanded_terms:
            if term.term not in unique_terms or term.confidence > unique_terms[term.term].confidence:
    ...
    def _build_expanded_query(
        ...
        for original_term, related_terms in term_groups.items():
            if original_term in expanded_query:
                ...
                all_terms = [original_term] + related_terms[:3]  # Limit to avoid overly complex queries
                term_expansion = f"({' OR '.join(all_terms)})"
                expanded_query = expanded_query.replace(original_term, term_expansion, 1)
    ...
    def _generate_term_statistics(self, expanded_terms: list[ExpandedTerm]) -> dict[str, Any]:
    ...
    def _get_domain_specific_terms(
        ...
        if domain in self.domain_vocabularies and term in self.domain_vocabularies[domain]:
            for related_term in self.domain_vocabularies[domain][term]:
                domain_terms.append((related_term, 0.8))  # High relevance for domain terms
    ...
    def _get_cached_result(self, request: QueryExpansionRequest) -> QueryExpansionResult | None:
    ...
    def _cache_result(self, request: QueryExpansionRequest, result: QueryExpansionResult) -> None:
    ...
    def _update_performance_stats(self, strategy: ExpansionStrategy, processing_time: float) -> None:
