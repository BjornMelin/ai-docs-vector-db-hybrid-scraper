...
class SearchStrategySelector:
    ...
    async def select_strategy(
        ...
        base_config = self._intent_strategy_map.get(primary_intent, {
            "primary": SearchStrategy.SEMANTIC,
        ...
        adjusted_strategy = self._apply_complexity_adjustments(
            base_config, complexity
        )
        ...
        primary_confidence = intent_classification.confidence_scores.get(primary_intent, 0.5)
        ...
        if complexity_config.get("force_reranking", False):
            if adjusted["primary"] != SearchStrategy.RERANKED:
                ...
                if SearchStrategy.RERANKED not in adjusted["fallbacks"]:
                    adjusted["fallbacks"] = [SearchStrategy.RERANKED] + adjusted["fallbacks"][:2]
    ...
    def _apply_performance_requirements(
        ...
        if max_latency:
            ...
            if primary_latency > max_latency:
                ...
                fast_strategies = [
                    (strategy, perf) for strategy, perf in self._strategy_performance.items()
                ...
                if fast_strategies:
                    ...
                    adjusted["reasoning"] += f" (optimized for <{max_latency}ms latency)"
        ...
        if min_quality:
            ...
            if primary_quality < min_quality:
                ...
                if quality_strategies:
                    ...
                    best_strategy = min(quality_strategies, key=lambda x: x[1]["latency"])
                    adjusted["fallbacks"] = [adjusted["primary"]] + adjusted["fallbacks"][:1]
                    adjusted["primary"] = best_strategy[0]
    ...
    def _apply_context_adjustments(
        ...
        if "programming_language" in context:
            ...
            if any(lang in ["python", "javascript", "java"] for lang in languages):
                ...
                if SearchStrategy.SEMANTIC not in [adjusted["primary"]] + adjusted["fallbacks"]:
                    adjusted["fallbacks"] = [SearchStrategy.SEMANTIC] + adjusted["fallbacks"][:2]
        ...
        if "framework" in context:
            ...
            if frameworks:
                ...
                if SearchStrategy.FILTERED not in [adjusted["primary"]] + adjusted["fallbacks"]:
                    adjusted["fallbacks"] = [SearchStrategy.FILTERED] + adjusted["fallbacks"][:2]
        ...
        if "version" in context:
            ...
            if SearchStrategy.HYBRID not in [adjusted["primary"]] + adjusted["fallbacks"]:
    ...
    def _incorporate_secondary_intents(
        ...
        for intent in secondary_intents[:2]:  # Limit to top 2 secondary intents
            if intent in self._intent_strategy_map:
                ...
                if secondary_strategy not in [adjusted["primary"]] + adjusted["fallbacks"]:
