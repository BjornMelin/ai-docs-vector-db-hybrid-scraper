...
class ToolCompositionEngine:
    ...
    async def initialize(self) -> None:
        ...
        logger.info(
            f"Tool composition engine initialized with {len(self.tool_registry)} tools"
        )
    ...
    async def _register_embedding_tools(self) -> None:
        ...
        embedding_tools = {
            ...
            "rerank_results": ToolMetadata(
                ...
                input_schema={"query": "str", "results": "List[Dict[str, Any]]"},
    ...
    async def _register_content_intelligence_tools(self) -> None:
        ...
        content_tools = {
            "classify_content": ToolMetadata(
                ...
                input_schema={"content": "str", "metadata": "Optional[Dict[str, Any]]"},
    ...
    async def _register_analytics_tools(self) -> None:
        ...
        self.tool_executors["analyze_query_performance"] = (
            self._execute_analyze_query_performance
        )
    ...
    async def compose_tool_chain(
        ...
        logger.info(
            f"Composed tool chain with {len(tool_chain)} steps for goal: {goal}"
        )
    ...
    async def execute_tool_chain(
        ...
        try:
            ...
            for step_idx, step in enumerate(chain):
                ...
                try:
                    ...
                    if time.time() - start_time > timeout_seconds:
                        raise TimeoutError(
                            f"Tool chain execution exceeded {timeout_seconds}s timeout"
                        )
                    ...
                    context["execution_times"][step.tool_name] = (
                    ...
                    if not step_result.success:
                        if step.optional:
                            logger.warning(
                                f"Optional step {step.tool_name} failed: {step_result.error}"
                            )
                            ...
                            raise RuntimeError(
                                f"Required step {step.tool_name} failed: {step_result.error}"
                            )
                ...
                logger.debug(
                    f"Step {step_idx} ({step.tool_name}) completed in {step_time:.1f}ms"
                )
            ...
            logger.info(
                f"Tool chain {execution_id} completed successfully in {total_time:.1f}ms"
            )
            ...
            logger.error(
                f"Tool chain {execution_id} failed after {total_time:.1f}ms: {e}"
            )
    ...
    async def _select_optimal_tools(
        ...
        for tool_name, metadata in candidate_tools:
            ...
            if performance_priority == "speed":
                ...
                score += metadata.performance_metrics.get(
                    "accuracy_score", 0.5
                ) * 2.5 + 2.5 / (metadata.estimated_latency_ms / 100.0)

            # Complexity match score
            if (complexity == "simple" and metadata.estimated_latency_ms < 200) or (
                complexity == "complex"
                and metadata.category in [ToolCategory.RAG, ToolCategory.ANALYTICS]
            ):
                score += 2.0
        ...
        if primary_action == "search":
            ...
            search_tools = [
                t
                for t, _ in sorted_tools
                if self.tool_registry[t].category == ToolCategory.SEARCH
            ]
            if search_tools:
            ...
            rag_tools = [
                t
                for t, _ in sorted_tools
                if self.tool_registry[t].category == ToolCategory.RAG
            ]
            if search_tools:
                selected_tools.append(search_tools[0])
            if rag_tools:
            ...
            analytics_tools = [
                t
                for t, _ in sorted_tools
                if self.tool_registry[t].category == ToolCategory.ANALYTICS
            ]
            if search_tools:
                selected_tools.append(search_tools[0])
            if analytics_tools:
        ...
        if complexity == "complex":
            content_tools = [
                t
                for t, _ in sorted_tools
                if self.tool_registry[t].category == ToolCategory.CONTENT_INTELLIGENCE
            ]
            if content_tools:
    ...
    async def _create_execution_chain(
        self, selected_tools: list[str], goal_analysis: dict[str, Any]
    ) -> list[ToolChainStep]:
        ...
        for idx, tool_name in enumerate(selected_tools):
            ...
            elif metadata.category == ToolCategory.RAG:
                input_mapping = {
                    "query": "input_data.query",
            ...
            step = ToolChainStep(
                ...
                parallel=metadata.category
                in [ToolCategory.CONTENT_INTELLIGENCE, ToolCategory.ANALYTICS],
    ...
    async def _record_chain_performance(
        self, chain: list[ToolChainStep], result: dict[str, Any]
    ) -> None:
    ...
    async def _execute_hybrid_search(
        self, query: str, collection: str, **kwargs
    ) -> list[dict[str, Any]]:
    ...
    async def _execute_generate_embeddings(
    ...
    async def _execute_rerank_results(
    ...
    async def _execute_classify_content(
        self, content: str, **kwargs
    ) -> dict[str, float]:
        """Execute content classification tool."""
        return {"technical": 0.7, "informational": 0.8, "quality_score": 0.85}

    async def _execute_assess_content_quality(
        self, content: str, **kwargs
    ) -> dict[str, float]:
    ...
    async def _execute_generate_rag_answer(
        self, query: str, search_results: list[dict[str, Any]], **kwargs
    ) -> dict[str, Any]:
    ...
    async def _execute_analyze_query_performance(
        self, query: str, results: list[dict[str, Any]], **kwargs
    ) -> dict[str, Any]:
    ...
    def get_performance_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_success_rate": sum(1 for r in recent_history if r["success"])
            / len(recent_history),
            "avg_execution_time_ms": sum(r["execution_time_ms"] for r in recent_history)
            / len(recent_history),
    ...
    def _analyze_performance_trends(self) -> dict[str, float]:
        ...
        older_avg = (
            sum(r["execution_time_ms"] for r in self.performance_history[-100:-50])
            / min(50, len(self.performance_history) - 50)
        ...
        return {
            ...
            "performance_change_pct": ((older_avg - recent_avg) / older_avg * 100)
            if older_avg > 0
            else 0,
        }
