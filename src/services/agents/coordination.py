...
from src.services.agents.core import BaseAgent, BaseAgentDependencies, AgentState
...
class ParallelAgentCoordinator:
    ...
    async def unregister_agent(self, agent_name: str) -> None:
        ...
        tasks_to_cancel = [
            task_id for task_id, assignment in self.running_tasks.items()
    ...
    async def execute_coordinated_workflow(
        ...
        try:
            ...
            if strategy == CoordinationStrategy.SEQUENTIAL:
                ...
                results = await self.execute_coordinated_workflow(tasks, optimal_strategy, timeout_seconds)
            else:
            ...
            logger.error(f"Coordinated workflow {workflow_id} failed: {e}", exc_info=True)
            return {
                "workflow_id": workflow_id,
                "success": False,
                "error": str(e),
    ...
    async def get_coordination_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "healthy_agents": sum(1 for status in self.agent_health_status.values() if status),
            ...
            "agent_status": {
                name: {
                    ...
                    "circuit_breaker_open": (
                        self.circuit_breakers.get(name, CircuitBreakerPattern()).is_open()
                        if self.enable_circuit_breaker else False
                    )
                }
    ...
    async def _process_pending_tasks(self) -> None:
        ...
        for task in sorted_tasks:
            ...
            assignment = AgentAssignment(
                ...
                estimated_completion=datetime.now() + timedelta(milliseconds=task.estimated_duration_ms)
            )
    ...
    async def _execute_task(self, task: TaskDefinition, assignment: AgentAssignment) -> None:
        ...
        try:
            ...
            if self.enable_circuit_breaker and self.circuit_breakers[agent_name].is_open():
            ...
            if self.enable_circuit_breaker:
                try:
                    await self.circuit_breakers[agent_name].call(lambda: (_ for _ in ()).throw(e))
                except:
    ...
    async def _find_suitable_agent(self, task: TaskDefinition) -> Optional[str]:
        ...
        healthy_agents = [
            name for name, status in self.agent_health_status.items()
            if status and name not in [a.agent_name for a in self.running_tasks.values()]
        ]
        ...
        capable_agents = [
            name for name in healthy_agents
            if all(capability in self.agent_capabilities.get(name, set()) 
        ...
        if task.priority == TaskPriority.CRITICAL:
            coordinator_agents = [
                name for name in capable_agents
        ...
        if self.completed_tasks:
            ...
            for assignment in self.completed_tasks:
                if assignment.actual_start and assignment.actual_completion:
                    duration = (assignment.actual_completion - assignment.actual_start).total_seconds() * 1000
                    completion_times.append(duration)
                    
            if completion_times:
                self.metrics.avg_completion_time_ms = sum(completion_times) / len(completion_times)
                self.metrics.max_completion_time_ms = max(completion_times)
                
        # Calculate resource utilization
        if self.available_agents:
            active_agents = len(self.running_tasks)
            self.metrics.resource_utilization = active_agents / len(self.available_agents)
    ...
    async def _execute_sequential(self, task_ids: List[str], timeout_seconds: Optional[float]) -> Dict[str, Any]:
        ...
        for task_id in task_ids:
            ...
            while task_id in self.running_tasks or task_id in [t.task_id for t in self.pending_tasks]:
                await asyncio.sleep(0.1)
                if timeout_seconds and (time.time() - start_time) > timeout_seconds:
                    break
    ...
    async def _execute_parallel(self, task_ids: List[str], timeout_seconds: Optional[float]) -> Dict[str, Any]:
        """Execute tasks in parallel."""
        results = {}
        start_time = time.time()
        
        # Wait for all tasks to complete or timeout
        while task_ids and (not timeout_seconds or (time.time() - start_time) < timeout_seconds):
            # Check for completed tasks
            completed_ids = []
            for task_id in task_ids:
                if task_id not in self.running_tasks and task_id not in [t.task_id for t in self.pending_tasks]:
                    completed_assignment = next(
                        (a for a in self.completed_tasks if a.task_id == task_id), None
                    )
                    if completed_assignment:
    ...
    async def _execute_pipeline(self, task_ids: List[str], timeout_seconds: Optional[float]) -> Dict[str, Any]:
    ...
    async def _execute_hierarchical(self, task_ids: List[str], timeout_seconds: Optional[float]) -> Dict[str, Any]:
    ...
    async def _select_optimal_strategy(self, tasks: List[TaskDefinition]) -> CoordinationStrategy:
        ...
        if len(tasks) == 1:
            return CoordinationStrategy.SEQUENTIAL
        elif has_dependencies and len(tasks) > 3:
            return CoordinationStrategy.PIPELINE
        elif high_priority_count > len(tasks) / 2:  # >50% high priority
            return CoordinationStrategy.HIERARCHICAL
        elif avg_duration < 1000:  # Short tasks benefit from parallel execution
