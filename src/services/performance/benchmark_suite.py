...
class PerformanceBenchmarkSuite:
    ...
    async def run_comprehensive_benchmark(
        ...
        try:
            ...
            # Regression analysis
            regression_analysis = await self._analyze_performance_regression(final_metrics)
            ...
            if self.metrics_registry:
                self.metrics_registry.record_benchmark_completion(
                    ...
                    success_rate=final_metrics.successful_requests / max(final_metrics.total_requests, 1),
        ...
        logger.info(f"Completed benchmark: {benchmark_name} in {results['duration_seconds']:.2f}s")
        return results
    ...
    async def _run_warmup_phase(self, test_function: Callable) -> dict[str, Any]:
        ...
        for _ in range(10):
            ...
            try:
                ...
                warmup_results.append(BenchmarkResult(
        ...
        return {
            ...
            'avg_latency_ms': statistics.mean([r.duration_ms for r in successful_results]) if successful_results else 0,
    ...
    async def _run_load_test_phase(
        ...
        while current_rps <= config.max_rps:
            ...
            avg_latency = statistics.mean([r.duration_ms for r in successful_results]) if successful_results else float('inf')
            ...
            if (success_rate < config.success_threshold or 
        ...
        return {
            ...
            'max_stable_rps': max([r['target_rps'] for r in load_results if r['success_rate'] >= config.success_threshold], default=0),
        ...
        while not breaking_point_found and current_rps <= config.max_rps * 10:
            ...
            while time.time() - burst_start < burst_duration:
                ...
                try:
                    ...
                    burst_results.append(BenchmarkResult(
    ...
    async def _run_recovery_phase(self, test_function: Callable) -> dict[str, Any]:
        ...
        return {
            ...
            'recovery_latency_ms': statistics.mean([r.duration_ms for r in successful_results]) if successful_results else 0,
            'total_requests': len(recovery_results),
        }
    
    async def _execute_single_measurement(self, test_function: Callable) -> BenchmarkResult:
        ...
        try:
            ...
            if self.async_optimizer:
                await self.async_optimizer.execute_optimized(
                    test_function() if asyncio.iscoroutinefunction(test_function) else asyncio.create_task(asyncio.to_thread(test_function)),
    ...
    def _calculate_final_metrics(self) -> BenchmarkMetrics:
        ...
        return BenchmarkMetrics(
            ...
            throughput_rps=len(successful_measurements) / (len(all_measurements) * 0.05),  # Assuming 0.05s intervals
            error_rate=(len(all_measurements) - len(successful_measurements)) / len(all_measurements),
    ...
    def _calculate_efficiency_score(self, measurements: list[BenchmarkResult]) -> float:
        ...
        throughput = len(measurements) / (len(measurements) * 0.05)  # Assuming 0.05s intervals
    ...
    async def _analyze_performance_regression(self, current_metrics: BenchmarkMetrics) -> dict[str, Any]:
        ...
        for metric_name, direction, threshold_percent in comparisons:
            ...
            if direction == 'lower_is_better' and percent_change > threshold_percent:
            ...
            elif direction == 'higher_is_better' and percent_change < -threshold_percent:
                regression_analysis['has_regression'] = True
                regression_analysis['regression_details'].append({
                    'metric': metric_name,
                    'baseline_value': baseline_value,
            ...
            elif ((direction == 'lower_is_better' and percent_change < -threshold_percent) or
    ...
    def _generate_performance_recommendations(self, metrics: BenchmarkMetrics) -> list[str]:
        """Generate performance optimization recommendations."""
        recommendations = []
        
        # Latency recommendations
        if metrics.avg_latency_ms > 100:
            recommendations.append("Consider implementing request caching to reduce average latency")
        
        if metrics.p95_latency_ms > 500:
            recommendations.append("High P95 latency detected - investigate tail latency optimizations")
        
        # Error rate recommendations
        if metrics.error_rate > 0.01:  # > 1%
            recommendations.append("Error rate is elevated - implement circuit breakers and retry logic")
        ...
        if metrics.stability_score < 0.8:
            recommendations.append("Performance variance is high - investigate load balancing and resource allocation")
        
        # Efficiency recommendations
        if metrics.efficiency_score < 0.5:
            recommendations.append("Resource efficiency is low - optimize algorithms and reduce resource overhead")
        
        # Throughput recommendations
        if metrics.throughput_rps < 10:
            recommendations.append("Low throughput detected - consider async optimizations and connection pooling")
        
        return recommendations
    
    def _convert_to_benchmark_metrics(self, metrics_dict: dict[str, Any]) -> BenchmarkMetrics:
        ...
        return BenchmarkMetrics(
            ...
            failed_requests=metrics_dict.get('total_requests', 0) - metrics_dict.get('successful_requests', 0),
    ...
    def get_historical_performance_trend(self, metric_name: str, window_size: int = 10) -> dict[str, Any]:
        """Get historical performance trend for a specific metric."""
        if len(self.benchmark_history) < 2:
            return {'status': 'insufficient_data', 'message': 'Need at least 2 benchmark runs for trend analysis'}
...
async def run_performance_benchmark(
    ...
    return await suite.run_comprehensive_benchmark(benchmark_name, test_function, config)