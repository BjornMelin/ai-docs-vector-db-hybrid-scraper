...
class AdaptiveConcurrencyLimiter:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized adaptive concurrency limiter with limit: {initial_limit}"
        )
    ...
    async def _maybe_adjust_limit(self) -> None:
        ...
        async with self._update_lock:
            ...
            if total_tasks > 0:
                self.metrics.current_throughput = total_tasks / (
    ...
    async def _should_decrease_limit(self) -> bool:
        ...
        if len(self.performance_history) >= 5:
            recent_avg = (
                sum(
                    p["avg_execution_time"] for p in list(self.performance_history)[-5:]
                )
                / 5
            )
    ...
    async def _increase_limit(self) -> None:
        ...
        self.current_limit = min(
            int(self.current_limit * self.increase_factor), self.max_limit
        )
    ...
    async def _decrease_limit(self) -> None:
        ...
        self.current_limit = max(
            int(self.current_limit * self.decrease_factor), self.min_limit
        )
...
class IntelligentTaskScheduler:
    ...
    def __init__(
        ...
        self.task_queues = {priority: asyncio.Queue() for priority in TaskPriority}

        # Batching support
        self.batch_queues = {priority: [] for priority in TaskPriority}
        self.batch_timers = dict.fromkeys(TaskPriority)
...
class AsyncPerformanceOptimizer:
    ...
    def __init__(
        ...
        self.concurrency_limiter = (
            AdaptiveConcurrencyLimiter(initial_limit=initial_concurrency)
            if enable_adaptive_limiting
            else None
        )
        ...
        self.task_scheduler = (
            IntelligentTaskScheduler(
                ...
                or AdaptiveConcurrencyLimiter(initial_concurrency),
    ...
    async def execute_batch_optimized(
        ...
        if self.task_scheduler:
            ...
            tasks = [
                self.task_scheduler.schedule_task(coro, priority) for coro in coros
            ]
