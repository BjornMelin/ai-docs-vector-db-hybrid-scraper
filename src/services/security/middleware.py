...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        ai_validator: Optional[AISecurityValidator] = None,
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            self.security_monitor.log_security_event(
                "request_blocked",
                request,
                {"reason": str(e.detail), "status_code": e.status_code, "processing_time": processing_time}
            )
    ...
    async def _enforce_rate_limits(self, request: Request) -> None:
        ...
        if not is_allowed:
            self.security_monitor.log_rate_limit_violation(request, rate_config["limit"])
        ...
        for pattern in self.suspicious_patterns:
            ...
            if re.search(pattern, query_lower, re.IGNORECASE):
                self.security_monitor.log_suspicious_activity(
                    ...
                    {"query": query_string, "pattern": pattern}
                )
                raise HTTPException(
                    status_code=400,
    ...
    def _validate_json_data(self, data: Any, depth: int = 0) -> None:
        ...
        if isinstance(data, dict):
            ...
            if any(keyword in data.lower() for keyword in ["search", "query", "prompt", "instruction"]):
                try:
    ...
    def _create_security_error_response(self, exception: HTTPException, request: Request) -> JSONResponse:
    ...
    async def block_ip(self, ip_address: str, reason: str = "Security violation") -> None:
        ...
        self.security_monitor.log_security_event(
            "ip_blocked",
            None,
            {"blocked_ip": ip_address, "reason": reason}
        )
