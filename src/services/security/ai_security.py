...
class AISecurityValidator:
    ...
    def validate_search_query(self, query: str) -> str:
        ...
        if critical_threats:
            logger.critical(
                ...
                extra={"query": query[:100], "threats": [t.__dict__ for t in critical_threats]}
            )
            raise HTTPException(
                status_code=400,
                detail="Search query contains prohibited content"
            )
        
        if high_threats:
            logger.warning(
                f"High-risk patterns detected in search query: {[t.threat_type for t in high_threats]}",
                extra={"query": query[:100], "threats": [t.__dict__ for t in high_threats]}
            )
    ...
    def validate_document_content(self, content: str, filename: Optional[str] = None) -> Tuple[bool, List[SecurityThreat]]:
        ...
        if not is_valid:
            logger.warning(
                ...
                extra={"filename": filename, "threats": [t.__dict__ for t in critical_threats]}
            )
    ...
    def sanitize_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(metadata) > self.max_metadata_entries:
            logger.warning(f"Metadata has too many entries ({len(metadata)}), truncating")
    ...
    def validate_embedding_query(self, query: str, context: Optional[str] = None) -> str:
        ...
        if context:
            ...
            critical_context_threats = [t for t in context_threats if t.level == ThreatLevel.CRITICAL]
            ...
            if critical_context_threats:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Context contains prohibited content"
                )
