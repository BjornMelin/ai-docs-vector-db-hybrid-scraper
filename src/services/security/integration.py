...
class SecurityManager:
    ...
    def __init__(self, security_config: Optional[SecurityConfig] = None):
        ...
        self.ai_validator: Optional[AISecurityValidator] = None
    ...
    def setup_middleware(self, app: FastAPI) -> SecurityMiddleware:
        ...
        if not all([self.rate_limiter, self.ai_validator, self.security_monitor]):
            raise RuntimeError("Security components not initialized. Call initialize_components() first.")
    ...
    async def get_security_status(self) -> Dict[str, Any]:
        ...
        if self.security_monitor:
            try:
                status["security_metrics"] = self.security_monitor.get_security_metrics()
            except Exception as e:
...
def setup_application_security(app: FastAPI, security_config: Optional[SecurityConfig] = None) -> SecurityManager:
    ...
    async def security_health():
        ...
        try:
            ...
            is_healthy = (
                status.get("components_initialized", False) and
                status.get("rate_limiter", {}).get("redis_healthy", True) and  # True if no Redis requirement
                status.get("middleware", {}).get("middleware_active", False)
            )
