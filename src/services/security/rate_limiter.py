...
class DistributedRateLimiter:
    ...
    def __init__(self, redis_url: Optional[str] = None, security_config: Optional[SecurityConfig] = None):
        ...
        if REDIS_AVAILABLE and redis_url:
            try:
                ...
                logger.warning(f"Failed to initialize Redis: {e}. Using local fallback only.")
                self.redis_client = None
        else:
            if not REDIS_AVAILABLE:
                logger.warning("Redis not available. Install redis package for distributed rate limiting.")
    ...
    async def check_rate_limit(
        ...
        if self.redis_client:
            try:
                ...
                logger.warning(f"Redis rate limit check failed: {e}. Using local fallback.")
    ...
    async def _check_rate_limit_local(
        ...
        async with self._cache_lock:
            ...
            expired_keys = [
                timestamp for timestamp in request_history.keys() 
        ...
        async with self._cache_lock:
            if identifier not in self.local_cache:
                ...
                current_requests = sum(
                    1 for timestamp in request_history.keys()
                    if float(timestamp) >= window_start
                )
