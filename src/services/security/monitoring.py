...
class SecurityMonitor:
    ...
    def __init__(self, security_config: Optional[SecurityConfig] = None, log_dir: Optional[Path] = None):
    ...
    def log_security_event(
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Unknown event type: {event_type}, using SUSPICIOUS_ACTIVITY")
    ...
    def _detect_attack_patterns(self, event: SecurityEvent) -> None:
        ...
        ip_events = [
            ...
            (current_time - e.timestamp.timestamp()) < 300  # Last 5 minutes
        ]
        ...
        if len(ip_events) > 10:  # More than 10 events in 5 minutes
            self.log_security_event(
                ...
                event_data={
                    ...
                    "attack_types": list(set(e.event_type.value for e in ip_events))
                },
                severity="critical"
            )
        ...
        recent_high_severity = [
            ...
            if e.severity in (SecuritySeverity.HIGH, SecuritySeverity.CRITICAL) and
            (current_time - e.timestamp.timestamp()) < 600  # Last 10 minutes
        ]
        
        if len(recent_high_severity) > 5:
            unique_ips = set(e.client_ip for e in recent_high_severity)
    ...
    def _check_automated_responses(self, event: SecurityEvent) -> None:
        ...
        recent_ip_events = [
            ...
            if e.client_ip == event.client_ip and
            e.severity in (SecuritySeverity.HIGH, SecuritySeverity.CRITICAL)
        ]
    ...
    def get_security_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_threat_patterns": dict(
                sorted(self.threat_patterns.items(), key=lambda x: x[1], reverse=True)[:10]
            ),
            "top_source_ips": dict(
                sorted(self.ip_event_counts.items(), key=lambda x: x[1], reverse=True)[:10]
            )
        }
    ...
    def get_threat_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for event in high_severity_events:
            ...
            target_endpoints[event.endpoint] = target_endpoints.get(event.endpoint, 0) + 1
    ...
    def export_security_logs(self, hours: int = 24, format: str = "json") -> str:
        ...
        if format.lower() == "json":
            return json.dumps([e.to_dict() for e in relevant_events], indent=2, default=str)
    ...
    def cleanup_old_data(self, days: int = 30) -> int:
        ...
        active_ips = set(e.client_ip for e in self.recent_events)
        old_ip_counts = self.ip_event_counts.copy()
        self.ip_event_counts = {
            ip: count for ip, count in old_ip_counts.items()
            if ip in active_ips
        }
