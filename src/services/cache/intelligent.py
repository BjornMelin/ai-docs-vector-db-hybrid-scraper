...
class IntelligentCache(Generic[K, V]):
    ...
    async def set(
        ...
        should_compress = (
            compress if compress is not None else self.config.enable_compression
        )
    ...
    async def _check_memory_pressure(self, new_item_size: int) -> None:
        ...
        if (
            new_total_memory > self.config.max_memory_mb
            or len(self._memory_cache) >= self.config.max_items
        ):
    ...
    def _update_access_time_stats(self, access_time_ms: float) -> None:
        ...
        if self._stats.avg_access_time_ms == 0:
            ...
            self._stats.avg_access_time_ms = (
                alpha * access_time_ms + (1 - alpha) * self._stats.avg_access_time_ms
            )
    ...
    def get_memory_usage(self) -> dict[str, Any]:
        ...
        return {
            ...
            "memory_utilization": (total_size / (1024 * 1024))
