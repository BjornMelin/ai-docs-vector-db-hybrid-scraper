                ...
                try:
                    ...
                    if result:
                        if hasattr(result, "points") and result.points:
                            span.set_attribute("vector.results_count", len(result.points))
                            if result.points[0].score is not None:
                                span.set_attribute("vector.top_score", result.points[0].score)
                                span.set_attribute("vector.min_score", result.points[-1].score)
                        
                        if hasattr(result, "scoring_time"):
                            span.set_attribute("vector.scoring_time_ms", result.scoring_time * 1000)
                    ...
                    if result:
                        if isinstance(result, list) and result:
                            ...
                            if hasattr(result[0], "__len__"):
                                span.set_attribute("ai.output.embedding_dimensions", len(result[0]))
                        elif hasattr(result, "__len__"):
                            span.set_attribute("ai.output.embedding_dimensions", len(result))
...
@asynccontextmanager
async def trace_async_operation(operation_name: str, operation_type: str = "custom", **attributes):
