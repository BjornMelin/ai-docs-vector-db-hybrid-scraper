...
class Anomaly:
    ...
    def severity(self) -> AlertSeverity:
        ...
        elif self.deviation_score > 2.0:
            return AlertSeverity.HIGH
        elif self.deviation_score > 1.5:
...
class DistributedTracer:
    ...
    def start_trace(self, trace_id: str, operation_name: str, service_name: str) -> TraceSpan:
        """Start a new trace."""
        span = TraceSpan(
            trace_id=trace_id,
    ...
    def get_trace_summary(self, trace_id: str) -> Optional[Dict[str, Any]]:
        ...
        total_duration = max(s.end_time for s in completed_spans) - min(s.start_time for s in spans)
...
class AnomalyDetector:
    ...
    def detect_anomaly(self, metric_name: str, value: float, service_name: str = "unknown") -> Optional[Anomaly]:
...
class AlertManager:
    ...
    def get_alert_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "unacknowledged_alerts": sum(1 for a in active_alerts if not a.acknowledged),
            "alert_history_count": len(self.alert_history)
        }
    
    def _record_alert_event(self, alert: Alert, event_type: str, metric_value: float) -> None:
...
class EnterpriseObservabilityPlatform:
    ...
    def record_metric(
        ...
        if self.config.observability.enabled:
            ...
            if anomaly:
                ...
                logger.warning(f"Anomaly detected: {anomaly.metric_name} = {anomaly.actual_value}")
    
    def start_trace(self, operation_name: str, service_name: str, trace_id: Optional[str] = None) -> TraceSpan:
        """Start a new distributed trace."""
        if trace_id is None:
            import uuid
            trace_id = str(uuid.uuid4())
        
        return self.distributed_tracer.start_trace(trace_id, operation_name, service_name)
    ...
    def start_span(
        ...
        return self.distributed_tracer.start_span(trace_id, operation_name, service_name, parent_span_id)
    ...
    def _handle_alert_notification(self, alert: Alert) -> None:
        ...
        logger.warning(f"ALERT TRIGGERED: {alert.name} - {alert.description} (Severity: {alert.severity.value})")
...
_observability_platform: Optional[EnterpriseObservabilityPlatform] = None


async def get_observability_platform(config: EnterpriseConfig = None) -> EnterpriseObservabilityPlatform:
