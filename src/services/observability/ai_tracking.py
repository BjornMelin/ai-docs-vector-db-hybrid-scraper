...
from .span_utils import record_llm_usage, span_context
...
class OperationUsage:
    """Token, cost, and payload usage metrics."""
...
class OperationInstruments:
    """Telemetry instruments for AI operations."""
...
class AIOperationTracker:
    ...
    def __init__(self):
        ...
        self._instruments = OperationInstruments(
            ...
            token_counter=meter.create_counter(
                ...
                description="Total tokens used across AI operations",
            ),
            ...
            cache_hit_gauge=meter.create_gauge(
                "ai_cache_hit_rate",
                description="Cache hit rate for AI operations",
            ),
        )
    ...
    def record_operation(self, metrics: AIOperationMetrics) -> None:
        ...
        labels = {
            ...
            "model": metrics.model,
            "success": str(metrics.outcome.success),
        }

        # Record duration
        self._instruments.duration_histogram.record(metrics.duration_ms, labels)

        # Record cost if available
        if metrics.usage.cost_usd is not None:
            self._instruments.cost_counter.add(metrics.usage.cost_usd, labels)

        # Record token usage if available
        if metrics.usage.tokens_used is not None:
            self._instruments.token_counter.add(metrics.usage.tokens_used, labels)

        # Record quality score if available
        if metrics.outcome.quality_score is not None:
            self._instruments.quality_gauge.set(metrics.outcome.quality_score, labels)
    ...
    def track_llm_call(
        ...
        span_attributes: dict[str, Any] = {
            "ai.operation_type": "llm_call",
            "ai.provider": provider,
            "ai.model": model,
            "llm.operation": operation,
        }
        if expected_max_tokens is not None:
            span_attributes["llm.max_tokens"] = expected_max_tokens

        with span_context(tracer, "ai.llm_call", attributes=span_attributes) as span:
            start_time = time.time()
            try:
                yield operation_result
    ...
    def record_cache_performance(
        ...
        self._instruments.cache_hit_gauge.set(hit_rate, labels)
...
@lru_cache(maxsize=1)
def _tracker_singleton() -> AIOperationTracker:
    """Create or reuse the shared AI operation tracker."""
