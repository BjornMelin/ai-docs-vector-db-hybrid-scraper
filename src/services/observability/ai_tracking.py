...
class AIOperationTracker:
    ...
    def track_embedding_generation(
        ...
        with tracer.start_as_current_span("ai.embedding_generation") as span:
            ...
            if isinstance(input_texts, list):
                ...
                span.set_attribute("ai.input.total_chars", sum(len(text) for text in input_texts))
            ...
            if expected_dimensions:
                span.set_attribute("ai.embedding.expected_dimensions", expected_dimensions)
            ...
            try:
                ...
                if embeddings:
                    if isinstance(embeddings, list) and embeddings:
                        ...
                        if hasattr(embeddings[0], "__len__"):
                            span.set_attribute("ai.output.dimensions", len(embeddings[0]))
                ...
                metrics = AIOperationMetrics(
                    ...
                    output_size=len(operation_result.get("embeddings", [])) if operation_result.get("embeddings") else 0
                )
    ...
    def track_rag_pipeline(
        ...
        with tracer.start_as_current_span("ai.rag_pipeline") as span:
            ...
            add_span_event("rag_pipeline_started", {
            ...
            try:
                ...
                add_span_event("rag_pipeline_failed", {
                    "error": str(e)
                })
    ...
    def record_model_performance(
        ...
        add_span_event("model_performance_summary", {
            "provider": provider,
            "model": model,
...
def track_embedding_generation(
    ...
    return tracker.track_embedding_generation(provider, model, input_texts, expected_dimensions)
