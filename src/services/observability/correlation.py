...
class TraceCorrelationManager:
    ...
    def set_performance_context(
        ...
        if circuit_breaker_state:
            baggage.set_baggage("performance.circuit_breaker_state", circuit_breaker_state)
        ...
        if current_span and current_span.is_recording():
            ...
            if circuit_breaker_state:
                current_span.set_attribute("performance.circuit_breaker_state", circuit_breaker_state)
    ...
    def link_operations(self, parent_correlation_id: str, child_operation: str) -> str:
        ...
        if current_span and current_span.is_recording():
            ...
            current_span.add_event("operation_linked", {
...
class ErrorCorrelationTracker:
    ...
    def record_error(
        ...
        if current_span and current_span.is_recording():
            ...
            # Add error event
            current_span.add_event("error_occurred", {
                "error.id": error_id,
        ...
        def error_span_context():
            with self.tracer.start_as_current_span(f"error.{error_name}") as span:
                ...
                if parent_correlation_id:
                    span.set_attribute("error.parent_correlation_id", parent_correlation_id)
...
def set_business_context(
    ...
    manager.set_business_context(
        operation_type, query_type, search_method, ai_provider, model_name, cache_strategy
    )
...
def correlated_operation(
    ...
    **additional_context
    ...
    manager = get_correlation_manager()
    return manager.correlated_operation(operation_name, correlation_id, **additional_context)
...
def record_error(
    ...
    return tracker.record_error(error, error_type, severity, user_impact, recovery_action)
