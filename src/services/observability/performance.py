...
class PerformanceMonitor:
    ...
    def __init__(self, thresholds: Optional[PerformanceThresholds] = None):
        ...
        self._operation_history: Dict[str, deque] = defaultdict(lambda: deque(maxlen=100))
    ...
    def _check_thresholds(
        ...
        if metrics.duration_ms > self.thresholds.max_duration_ms:
            alerts.append(f"Duration {metrics.duration_ms:.1f}ms exceeds threshold {self.thresholds.max_duration_ms:.1f}ms")
        
        # Check CPU threshold
        if metrics.cpu_usage_percent and metrics.cpu_usage_percent > self.thresholds.max_cpu_percent:
            alerts.append(f"CPU usage {metrics.cpu_usage_percent:.1f}% exceeds threshold {self.thresholds.max_cpu_percent:.1f}%")
        
        # Check memory threshold
        if metrics.memory_usage_mb and metrics.memory_usage_mb > self.thresholds.max_memory_mb:
            alerts.append(f"Memory usage {metrics.memory_usage_mb:.1f}MB exceeds threshold {self.thresholds.max_memory_mb:.1f}MB")
        
        # Check error rate
        total_ops = self._success_counts[metrics.operation_name] + self._error_counts[metrics.operation_name]
        if total_ops > 10:  # Only check error rate after sufficient operations
            ...
            if error_rate > self.thresholds.max_error_rate:
                alerts.append(f"Error rate {error_rate:.3f} exceeds threshold {self.thresholds.max_error_rate:.3f}")
        
        # Record alerts
        if alerts:
            for alert in alerts:
                span.add_event("performance_threshold_violation", {"alert": alert})
                logger.warning(f"Performance alert for {metrics.operation_name}: {alert}")
    ...
    def get_system_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "overall_error_rate": sum(self._error_counts.values()) / max(
                sum(len(history) for history in self._operation_history.values()), 1
            )
        }
        ...
        if len(self._memory_history) > 1:
            summary["memory_trend"] = {
                ...
                "avg_last_minute_mb": sum(self._memory_history) / len(self._memory_history)
            }
    ...
    def monitor_cache_operation(self, cache_type: str, operation: str):
        ...
        """
        return self.monitor_operation(
            f"cache_{cache_type}_{operation}",
            category="cache",
            track_resources=False
        )
