...
async def crawl_url(
    ...
    try:
        if not crawling_client:
            raise HTTPException(
                status_code=500,
                detail="Crawling client not available"
            )
...
async def crawl_site(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Site crawling failed: {str(e)}"
        )
    ...
    try:
        ...
        if not parsed.scheme or not parsed.netloc:
            return {
                ...
                "details": {"parsed": {"scheme": parsed.scheme, "netloc": parsed.netloc}}
            }
...
async def estimate_crawl_cost(
    ...
    try:
        ...
        estimated_time_seconds = (
            single_urls * base_time_per_url +
            estimated_pages * time_per_page
        )
