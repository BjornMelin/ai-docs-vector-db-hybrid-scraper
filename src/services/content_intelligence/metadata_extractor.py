...
class MetadataExtractor:
    ...
    async def extract_metadata(
        ...
        await self._extract_technical_metadata(metadata, content, url, extraction_metadata)
    ...
    async def _extract_html_metadata(self, metadata: ContentMetadata, html: str) -> None:
    ...
    async def _extract_content_metadata(self, metadata: ContentMetadata, content: str, url: str) -> None:
        ...
        for pattern in breadcrumb_patterns:
            ...
            if match:
                metadata.breadcrumbs = [part.strip() for part in match.groups() if part.strip()]
                break

    async def _extract_semantic_metadata(self, metadata: ContentMetadata, content: str) -> None:
        ...
        # Technical topics
        technical_topics = {
            "programming": ["code", "function", "class", "method", "programming", "software"],
            ...
            "machine learning": ["ml", "ai", "model", "training", "algorithm", "neural"],
        ...
        metadata.tags = list(set([tag for tag in metadata.tags if tag and len(tag) > 1]))
    ...
    async def _extract_technical_metadata(
        ...
        if extraction_metadata:
            ...
            metadata.page_load_time_ms = extraction_metadata.get("automation_time_ms", 0.0)
        ...
        if parsed_url.path:
            ...
            if len(path_parts) > 1:
                ...
                if len(path_parts) >= 2:
                    metadata.parent_url = f"{parsed_url.scheme}://{parsed_url.netloc}/{path_parts[0]}/"

    async def _extract_hierarchy_metadata(self, metadata: ContentMetadata, content: str, url: str) -> None:
        ...
        for found_url in found_urls:
            ...
            if (parsed_found.netloc and 
                parsed_found.netloc != parsed_current.netloc and
                found_url not in related_urls):
                related_urls.append(found_url)
        
        metadata.related_urls = related_urls[:10]  # Limit to 10 related URLs

    async def _extract_structured_data(self, metadata: ContentMetadata, html: str) -> None:
