...
class QualityAssessor:
    ...
    async def assess_quality(
        ...
        duplicate_similarity = await self._assess_duplicate_similarity(content, existing_content)
    ...
    def _assess_completeness(self, content: str) -> float:
        ...
        if word_count < self._quality_config["min_word_count"]:
            ...
            word_score = 0.5 + (word_count - self._quality_config["min_word_count"]) / \
                        (self._quality_config["optimal_word_count"] - self._quality_config["min_word_count"]) * 0.4
        elif word_count <= self._quality_config["max_word_count"]:
            word_score = 0.9 + (self._quality_config["max_word_count"] - word_count) / \
                        (self._quality_config["max_word_count"] - self._quality_config["optimal_word_count"]) * 0.1
        else:
            # Penalize overly long content
            word_score = max(0.7, 1.0 - (word_count - self._quality_config["max_word_count"]) / 1000 * 0.1)

        # Score based on sentence structure
        if sentence_count < self._quality_config["min_sentence_count"]:
            sentence_score = sentence_count / self._quality_config["min_sentence_count"] * 0.5
        else:
        ...
        try:
            ...
            if self.embedding_manager:
                result = await self.embedding_manager.generate_embeddings(
                    texts=[content, query_context],
                    quality_tier=None,
                    auto_select=True
                )
                
                if result.get("success", False) and len(result.get("embeddings", [])) == 2:
    ...
    def _assess_confidence(self, content: str, extraction_metadata: dict[str, Any] | None) -> float:
        ...
        positive_matches = sum(
            1 for pattern in self._quality_indicators["positive"]
        ...
        negative_matches = sum(
            1 for pattern in self._quality_indicators["negative"]
            if re.search(pattern, content, re.IGNORECASE)
        )
    ...
    def _assess_freshness(self, content: str, extraction_metadata: dict[str, Any] | None) -> float:
        ...
        fresh_matches = sum(
            1 for pattern in fresh_indicators
            if re.search(pattern, content, re.IGNORECASE)
        )
    ...
    def _calculate_freshness_score(self, days_old: int) -> float:
        ...
        if days_old < 0:
            ...
            return 0.8 - (days_old - self._quality_config["freshness_days_excellent"]) / \
                   (self._quality_config["freshness_days_good"] - self._quality_config["freshness_days_excellent"]) * 0.3
        elif days_old <= self._quality_config["freshness_days_poor"]:
            return 0.5 - (days_old - self._quality_config["freshness_days_good"]) / \
                   (self._quality_config["freshness_days_poor"] - self._quality_config["freshness_days_good"]) * 0.3
        else:
            return max(0.1, 0.2 - (days_old - self._quality_config["freshness_days_poor"]) / 365 * 0.1)
    ...
    def _assess_structure_quality(self, content: str) -> float:
        ...
        structural_matches = sum(
            1 for pattern in self._quality_indicators["structural"]
            if re.search(pattern, content, re.MULTILINE)
        )
        
        if structural_matches > 0:
            score += min(structural_matches / 5, 0.4)  # Up to 0.4 for structural elements
    ...
    def _assess_readability(self, content: str) -> float:
        ...
        if avg_sentence_length <= self._readability_config["avg_sentence_length_optimal"]:
            sentence_score = 1.0
        elif avg_sentence_length <= self._readability_config["avg_sentence_length_max"]:
            sentence_score = 1.0 - (avg_sentence_length - self._readability_config["avg_sentence_length_optimal"]) / \
                           (self._readability_config["avg_sentence_length_max"] - self._readability_config["avg_sentence_length_optimal"]) * 0.5
        else:
            sentence_score = max(0.2, 0.5 - (avg_sentence_length - self._readability_config["avg_sentence_length_max"]) / 20 * 0.3)
        ...
        if complex_ratio <= self._readability_config["complex_words_threshold"]:
            ...
            complexity_score = max(0.3, 1.0 - (complex_ratio - self._readability_config["complex_words_threshold"]) / 0.2)
        ...
        if passive_ratio <= self._readability_config["passive_voice_threshold"]:
            ...
            passive_score = max(0.5, 1.0 - (passive_ratio - self._readability_config["passive_voice_threshold"]) / 0.3)

        # Combine scores
        return sentence_score * 0.4 + complexity_score * 0.4 + passive_score * 0.2

    async def _assess_duplicate_similarity(self, content: str, existing_content: list[str] | None) -> float:
        ...
        try:
            ...
            if self.embedding_manager and len(existing_content) > 0:
                ...
                if result.get("success", False) and len(result.get("embeddings", [])) == len(all_texts):
                    embeddings = result["embeddings"]
                    ...
                    for existing_embedding in existing_embeddings:
                        similarity = self._cosine_similarity(content_embedding, existing_embedding)
    ...
    def _identify_quality_issues(
        ...
        if completeness < 0.5:
            word_count = len(content.split())
            if word_count < self._quality_config["min_word_count"]:
                issues.append(f"Content too short ({word_count} words, minimum {self._quality_config['min_word_count']})")
            else:
                issues.append("Content appears incomplete")

        if structure_quality < 0.4:
            issues.append("Poor content structure (missing headings, paragraphs, or formatting)")

        if readability < 0.4:
            issues.append("Low readability (complex sentences or difficult vocabulary)")

        if duplicate_similarity > self._quality_config["similarity_threshold_duplicate"]:
            issues.append(f"High similarity to existing content ({duplicate_similarity:.2f})")
        elif duplicate_similarity > self._quality_config["similarity_threshold_similar"]:
            issues.append(f"Moderate similarity to existing content ({duplicate_similarity:.2f})")
    ...
    def _generate_improvement_suggestions(
        ...
        if structure_quality < 0.6:
            suggestions.append("Improve content structure with headings and proper paragraphs")
        ...
        if duplicate_similarity > self._quality_config["similarity_threshold_similar"]:
            suggestions.append("Add unique insights or perspectives to differentiate from similar content")
