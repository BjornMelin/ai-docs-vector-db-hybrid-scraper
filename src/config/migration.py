...
from .modern import Config as ModernConfig
from .modern import ApplicationMode, Environment, EmbeddingProvider, CrawlProvider
...
class ConfigMigrator:
    ...
    def _apply_special_transformations(self, legacy_data: Dict[str, Any], modern_data: Dict[str, Any]) -> None:
        ...
        if "embedding_provider" in legacy_data:
            ...
            if isinstance(provider, str):
                try:
                    modern_data["embedding_provider"] = EmbeddingProvider(provider.lower()).value
                except ValueError:
        ...
        if "crawl_provider" in legacy_data:
            ...
            if isinstance(provider, str):
                try:
                    modern_data["crawl_provider"] = CrawlProvider(provider.lower()).value
                except ValueError:
        ...
        if "reranking" in legacy_data or any(k.startswith("rerank_") for k in legacy_data):
            rerank_config = {}
            for key, value in legacy_data.items():
...
def create_migration_compatibility_wrapper(modern_config: ModernConfig) -> Dict[str, Any]:
    ...
    for section in ["qdrant", "openai", "firecrawl", "cache", "performance", "security", "chunking", "hyde", "reranking"]:
