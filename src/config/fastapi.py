...
class CompressionConfig(BaseModel):
    ...
    minimum_size: int = Field(
        default=1000, gt=0, description="Minimum response size to compress (bytes)"
    )
...
class SecurityConfig(BaseModel):
    ...
    rate_limit_requests: int = Field(
        default=100, gt=0, description="Requests per minute per IP"
    )
    rate_limit_window: int = Field(
        default=60, gt=0, description="Rate limit window (seconds)"
    )
...
class TracingConfig(BaseModel):
    ...
    generate_correlation_id: bool = Field(
        default=True, description="Auto-generate correlation ID if missing"
    )
    log_requests: bool = Field(default=True, description="Log request details")
    log_responses: bool = Field(
        default=False,
        description="Log response details (be careful with sensitive data)",
    )
...
class TimeoutConfig(BaseModel):
    ...
    request_timeout: float = Field(
        default=30.0, gt=0, description="Request timeout (seconds)"
    )
    ...
    failure_threshold: int = Field(
        default=5, gt=0, description="Consecutive failures before opening circuit"
    )
    recovery_timeout: float = Field(
        default=60.0, gt=0, description="Time before trying to close circuit (seconds)"
    )
    half_open_max_calls: int = Field(
        default=3, gt=0, description="Max calls in half-open state"
    )
...
class PerformanceConfig(BaseModel):
    ...
    track_memory_usage: bool = Field(
        default=False, description="Track memory usage (overhead)"
    )
    slow_request_threshold: float = Field(
        default=1.0, gt=0, description="Slow request threshold (seconds)"
    )
...
class FastAPIProductionConfig(BaseModel):
    ...
    environment: Environment = Field(
        default=Environment.DEVELOPMENT, description="Environment type"
    )
    ...
    max_requests_jitter: int | None = Field(
    ...
    def get_environment_specific_cors(self) -> list[str]:
        ...
        if self.environment == Environment.PRODUCTION:
            ...
            return ["https://yourdomain.com", "https://api.yourdomain.com"]
    ...
    def get_security_headers(self) -> dict[str, str]:
        ...
        if self.security.strict_transport_security and self.is_production():
            headers["Strict-Transport-Security"] = (
                self.security.strict_transport_security
            )
