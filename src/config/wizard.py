...
class ConfigurationWizard:
    ...
    def run_restore_wizard(self) -> bool:
        ...
        for backup in backups:
            ...
            backup_choices.append((backup.backup_id, f"{backup.backup_id[:12]}... - {backup.config_name}"))
        ...
        try:
            ...
            if result.success:
                self.console.print(f"[green]✅ Configuration restored to: {result.config_path}[/green]")
                if result.pre_restore_backup:
                    self.console.print(f"[dim]Pre-restore backup: {result.pre_restore_backup}[/dim]")
                return True
            else:
    ...
    def _template_based_setup(self, config_path: Path | None) -> Path:
        ...
        for template_name in available_templates:
            description = template_descriptions.get(template_name, f"📄 {template_name}")
        ...
        if not config_path:
            ...
            config_path = Path(questionary.path(
                "Enter path for configuration file:",
    ...
    def _interactive_setup(self, config_path: Path | None) -> Path:
        ...
        if use_postgres:
            db_url = questionary.text(
                ...
                default="postgresql+asyncpg://user:password@localhost:5432/aidocs"
            ).ask()
    ...
    def _migration_setup(self, config_path: Path | None) -> Path:
        ...
        plan = self.migration_manager.create_migration_plan(current_version, target_version)
        ...
        self.console.print(f"[yellow]Migration plan: {current_version} → {target_version}[/yellow]")
    ...
    def _import_setup(self, config_path: Path | None) -> Path:
        ...
        import_path = Path(questionary.path(
        ...
        try:
            ...
            if not report.is_valid:
                self.console.print("[yellow]⚠️  Imported configuration has validation issues:[/yellow]")
                self._display_validation_report(report)
            ...
            self.console.print(f"[green]✅ Configuration imported: {config_path}[/green]")
    ...
    def _customize_template(self, config_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if questionary.confirm("Configure API keys?").ask():
            if config_data.get("embedding_provider") == "openai":
                api_key = questionary.password("Enter OpenAI API key (leave empty to skip):").ask()
                if api_key:
            ...
            if config_data.get("crawl_provider") == "firecrawl":
                api_key = questionary.password("Enter Firecrawl API key (leave empty to skip):").ask()
                if api_key:
    ...
    def _display_validation_report(self, report) -> None:
        ...
        if report.info:
            self.console.print("[blue]ℹ️  Information:[/blue]")
