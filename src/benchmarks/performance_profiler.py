...
class ProfilingResults(BaseModel):
    ...
    memory_leak_detected: bool = Field(False, description="Whether memory leak detected")
    ...
    total_network_recv_mb: float = Field(..., description="Total network received in MB")
    ...
    bottleneck_periods: List[Dict[str, Any]] = Field(default_factory=list, description="Identified bottleneck periods")
    resource_warnings: List[str] = Field(default_factory=list, description="Resource usage warnings")
...
class PerformanceProfiler:
    ...
    async def profile_search_service(
        ...
        # Analyze results
        results = self._analyze_profiling_results(duration, start_datetime, end_datetime)
        
        logger.info(f"Profiling completed. Duration: {duration:.2f}s, Peak memory: {results.peak_memory_mb:.1f}MB")
        ...
        for i, query in enumerate(profiling_queries):
            try:
                ...
                if i % 3 == 0:
                    ...
                    tasks = [
                        search_service.advanced_hybrid_search(query)
                        for _ in range(2)
                    ]
    ...
    def _analyze_profiling_results(
        ...
        suggestions = self._generate_optimization_suggestions(avg_cpu, peak_memory, bottlenecks)
    ...
    def _identify_bottlenecks(self) -> List[Dict[str, Any]]:
        ...
        for i, snapshot in enumerate(self.resource_snapshots):
            if snapshot.cpu_percent > 90:
                ...
                if cpu_bottleneck_start is not None:
                    duration = (i - cpu_bottleneck_start) * self.sampling_interval
                    if duration > 2.0:  # At least 2 seconds
                        bottlenecks.append({
                            "type": "cpu",
                            "start_time": self.resource_snapshots[cpu_bottleneck_start].timestamp,
                            "duration_seconds": duration,
                            "severity": "high"
                        })
                    cpu_bottleneck_start = None
        ...
        for i, snapshot in enumerate(self.resource_snapshots):
            if snapshot.memory_percent > 85:
                ...
                if memory_bottleneck_start is not None:
                    duration = (i - memory_bottleneck_start) * self.sampling_interval
                    if duration > 1.0:  # At least 1 second
                        bottlenecks.append({
                            "type": "memory",
                            "start_time": self.resource_snapshots[memory_bottleneck_start].timestamp,
                            "duration_seconds": duration,
                            "severity": "high"
                        })
                    memory_bottleneck_start = None
    ...
    def _generate_warnings(self, avg_cpu: float, peak_memory: float, memory_growth: float) -> List[str]:
    ...
    def _generate_optimization_suggestions(
        ...
        if avg_cpu > 60:
            suggestions.append("Consider implementing request queuing to smooth CPU load")
        ...
        if len(cpu_bottlenecks) > 0:
            ...
            suggestions.append("Consider horizontal scaling for CPU-intensive operations")
        ...
        if not suggestions:
            suggestions.append("Performance profile looks good - consider load testing at higher concurrency")
