...
from ..models.vector_search import AdvancedHybridSearchRequest, SearchParams, FusionConfig
...
from .load_test_runner import LoadTestRunner, LoadTestConfig
...
class BenchmarkConfig(BaseModel):
    ...
    target_latency_p95_ms: float = Field(300.0, description="Target p95 latency in milliseconds")
    target_throughput_qps: float = Field(500.0, description="Target throughput in queries per second")
    ...
    test_queries_per_type: int = Field(100, description="Number of test queries per type")
    concurrent_users: List[int] = Field([10, 50, 200], description="Concurrent user levels to test")
    test_duration_seconds: int = Field(300, description="Duration for sustained load tests")
    
    # Component testing
    enable_component_benchmarks: bool = Field(True, description="Enable component-level benchmarks")
    ...
    enable_optimization_analysis: bool = Field(True, description="Enable optimization analysis")
...
class BenchmarkResults(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=datetime.now, description="Benchmark execution time")
    duration_seconds: float = Field(..., description="Total benchmark duration")
    
    # Performance metrics
    latency_metrics: Dict[str, float] = Field(default_factory=dict, description="Latency percentiles")
    throughput_metrics: Dict[str, float] = Field(default_factory=dict, description="Throughput measurements")
    resource_metrics: Dict[str, float] = Field(default_factory=dict, description="Resource utilization")
    accuracy_metrics: Dict[str, float] = Field(default_factory=dict, description="ML accuracy metrics")
    
    # Component-specific results
    component_results: Dict[str, Any] = Field(default_factory=dict, description="Component benchmark results")
    load_test_results: Dict[str, Any] = Field(default_factory=dict, description="Load test results")
    profiling_results: Dict[str, Any] = Field(default_factory=dict, description="Profiling results")
    
    # Optimization recommendations
    optimization_recommendations: List[str] = Field(default_factory=list, description="Optimization suggestions")
    performance_bottlenecks: List[str] = Field(default_factory=list, description="Identified bottlenecks")
    
    # Pass/fail status
    meets_targets: bool = Field(..., description="Whether benchmark meets all targets")
    failed_targets: List[str] = Field(default_factory=list, description="List of failed targets")
...
class AdvancedHybridSearchBenchmark:
    ...
    def _generate_test_queries(self) -> List[AdvancedHybridSearchRequest]:
        ...
        test_data = [
            ...
            ("How to implement OAuth2 flow in React with Node.js backend and PostgreSQL database?", "complex"),
            ("Best practices for scaling microservices with Kubernetes, monitoring, and CI/CD pipeline?", "complex"),
            ("Optimize database performance with indexing, caching, and query optimization techniques?", "complex"),
        ]
    ...
    async def run_comprehensive_benchmark(self) -> BenchmarkResults:
        ...
        try:
            ...
            if self.benchmark_config.enable_load_testing:
                ...
                results.throughput_metrics.update(load_results.get("throughput_metrics", {}))
            ...
            if self.benchmark_config.enable_profiling:
                ...
                results.resource_metrics.update(profiling_results.get("resource_metrics", {}))
            ...
            if self.benchmark_config.enable_optimization_analysis:
                ...
                optimization_analysis = self._analyze_optimization_opportunities(results)
                results.optimization_recommendations = optimization_analysis["recommendations"]
            ...
            logger.info(f"Benchmark completed in {results.duration_seconds:.2f} seconds")
            ...
            results.duration_seconds = time.time() - start_time
            results.failed_targets.append(f"Benchmark execution failed: {str(e)}")
            return results
    ...
    async def _run_component_benchmarks(self) -> Dict[str, Any]:
        ...
        return await self.component_benchmarks.run_all_component_benchmarks(
            self.search_service, self.test_queries[:50]  # Use subset for component tests
        )
    ...
    async def _run_profiling(self) -> Dict[str, Any]:
        ...
        return await self.performance_profiler.profile_search_service(
            self.search_service, self.test_queries[:20]  # Use subset for profiling
        )
    ...
    async def _measure_classification_accuracy(self) -> float:
        ...
        for query_text, expected_type in ground_truth.items():
            try:
                classification = await self.search_service.query_classifier.classify_query(query_text)
    ...
    async def _measure_model_selection_accuracy(self) -> float:
        ...
        for query_text, query_domain in test_cases:
            try:
                classification = await self.search_service.query_classifier.classify_query(query_text)
                selection = await self.search_service.model_selector.select_optimal_model(classification)
                
                # Check if selection is appropriate for domain
                model_info = self.search_service.model_selector.model_registry[selection.primary_model]
    ...
    async def _measure_fusion_effectiveness(self) -> float:
        ...
        for query in self.test_queries[:10]:
            try:
                classification = await self.search_service.query_classifier.classify_query(query.query)
    ...
    def _analyze_optimization_opportunities(self, results: BenchmarkResults) -> Dict[str, List[str]]:
        ...
        if p95_latency > self.benchmark_config.target_latency_p95_ms:
            bottlenecks.append(f"High p95 latency: {p95_latency:.1f}ms > {self.benchmark_config.target_latency_p95_ms}ms")
            recommendations.append("Consider caching frequently accessed embeddings")
            recommendations.append("Optimize model selection logic for faster decisions")
            
        # Throughput analysis
        max_qps = max(results.throughput_metrics.values()) if results.throughput_metrics else 0
        if max_qps < self.benchmark_config.target_throughput_qps:
            bottlenecks.append(f"Low throughput: {max_qps:.1f} QPS < {self.benchmark_config.target_throughput_qps} QPS")
            recommendations.append("Implement connection pooling for database operations")
            recommendations.append("Add async processing for non-critical ML components")
            
        # Memory analysis
        peak_memory = results.resource_metrics.get("peak_memory_mb", 0)
        if peak_memory > self.benchmark_config.target_memory_mb:
            bottlenecks.append(f"High memory usage: {peak_memory:.1f}MB > {self.benchmark_config.target_memory_mb}MB")
        ...
        # Accuracy analysis
        avg_accuracy = sum(results.accuracy_metrics.values()) / max(len(results.accuracy_metrics), 1)
        if avg_accuracy < self.benchmark_config.target_accuracy:
            bottlenecks.append(f"Low ML accuracy: {avg_accuracy:.3f} < {self.benchmark_config.target_accuracy}")
            recommendations.append("Retrain query classification with more diverse data")
    ...
    def _check_target_compliance(self, results: BenchmarkResults) -> Dict[str, Any]:
        ...
        if p95_latency > self.benchmark_config.target_latency_p95_ms:
            failed_targets.append(f"Latency target: {p95_latency:.1f}ms > {self.benchmark_config.target_latency_p95_ms}ms")
            
        # Check throughput target
        max_qps = max(results.throughput_metrics.values()) if results.throughput_metrics else 0
        if max_qps < self.benchmark_config.target_throughput_qps:
            failed_targets.append(f"Throughput target: {max_qps:.1f} < {self.benchmark_config.target_throughput_qps} QPS")
            
        # Check memory target
        peak_memory = results.resource_metrics.get("peak_memory_mb", float('inf'))
        if peak_memory > self.benchmark_config.target_memory_mb:
            failed_targets.append(f"Memory target: {peak_memory:.1f}MB > {self.benchmark_config.target_memory_mb}MB")
            
        # Check accuracy target
        avg_accuracy = sum(results.accuracy_metrics.values()) / max(len(results.accuracy_metrics), 1)
        if avg_accuracy < self.benchmark_config.target_accuracy:
            failed_targets.append(f"Accuracy target: {avg_accuracy:.3f} < {self.benchmark_config.target_accuracy}")
