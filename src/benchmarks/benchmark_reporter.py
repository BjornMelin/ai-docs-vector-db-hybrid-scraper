...
<body>
    ...
    def _format_latency_metrics(self, latency_metrics: Dict[str, float]) -> str:
        ...
        if not latency_metrics:
            return "<div class='metric-item'><span>No latency data available</span></div>"
        
        items = []
        for metric, value in latency_metrics.items():
            css_class = "metric-good" if value < 100 else "metric-warning" if value < 300 else "metric-error"
            items.append(f"""
    ...
    def _format_throughput_metrics(self, throughput_metrics: Dict[str, float]) -> str:
        ...
        for metric, value in throughput_metrics.items():
            css_class = "metric-good" if value > 100 else "metric-warning" if value > 50 else "metric-error"
            items.append(f"""
                <div class="metric-item">
                    <span>{metric.replace('_', ' ').title()}</span>
    ...
    def _format_resource_metrics(self, resource_metrics: Dict[str, float]) -> str:
        ...
        if not resource_metrics:
            return "<div class='metric-item'><span>No resource data available</span></div>"
        
        items = []
        for metric, value in resource_metrics.items():
            if "memory" in metric.lower():
                css_class = "metric-good" if value < 1000 else "metric-warning" if value < 2000 else "metric-error"
                unit = "MB"
            elif "cpu" in metric.lower():
                css_class = "metric-good" if value < 50 else "metric-warning" if value < 80 else "metric-error"
                unit = "%"
            else:
    ...
    def _format_accuracy_metrics(self, accuracy_metrics: Dict[str, float]) -> str:
        ...
        if not accuracy_metrics:
            return "<div class='metric-item'><span>No accuracy data available</span></div>"
        
        items = []
        for metric, value in accuracy_metrics.items():
            percentage = value * 100
            css_class = "metric-good" if percentage > 80 else "metric-warning" if percentage > 60 else "metric-error"
            items.append(f"""
                <div class="metric-item">
                    <span>{metric.replace('_', ' ').title()}</span>
    ...
    def _generate_component_table(self, component_results: Dict[str, Any]) -> str:
        ...
        for component_name, result in component_results.items():
            ...
            latency_class = "metric-good" if avg_latency < 50 else "metric-warning" if avg_latency < 150 else "metric-error"
            throughput_class = "metric-good" if throughput > 10 else "metric-warning" if throughput > 5 else "metric-error"
            error_class = "metric-good" if error_rate < 1 else "metric-warning" if error_rate < 5 else "metric-error"
    ...
    def _generate_recommendations_section(
        ...
        if bottlenecks:
            ...
            for bottleneck in bottlenecks:
                content.append(f'<li class="recommendation-item bottleneck-item">{bottleneck}</li>')
            content.append('</ul>')
        ...
        if recommendations:
            ...
            for recommendation in recommendations:
                content.append(f'<li class="recommendation-item">{recommendation}</li>')
            content.append('</ul>')
        
        if not content:
            content.append("<p>No specific recommendations available. System performance appears to be within acceptable ranges.</p>")
    ...
    def _generate_charts_javascript(self, results: Any) -> str:
        ...
        component_names = list(results.component_results.keys()) if results.component_results else []
        latencies = []
        throughputs = []
    ...
    async def save_reports(
        ...
        logger.info(f"Saved benchmark reports: {list(saved_files.keys())}")
        return saved_files