...
class LoadTestConfig(BaseModel):
    ...
    # Request patterns
    think_time_min_ms: int = Field(100, description="Minimum think time between requests")
    think_time_max_ms: int = Field(2000, description="Maximum think time between requests")
...
class LoadTestMetrics(BaseModel):
    ...
    # Concurrency metrics
    concurrent_users: int = Field(..., description="Number of concurrent users")
    avg_concurrent_requests: float = Field(0.0, description="Average concurrent requests")
        ...
        for _ in range(requests_per_user):
            ...
            try:
                ...
                think_time = random.randint(
                    self.config.think_time_min_ms,
                    self.config.think_time_max_ms
                ) / 1000.0
...
class LoadTestRunner:
    ...
    async def run_load_test(
        ...
        Returns:
            Comprehensive load test metrics
        """
        logger.info(f"Starting load test: {load_config.concurrent_users} users, "
        ...
        return self._calculate_load_test_metrics(user_results, total_duration, load_config)
    
    async def _track_metrics_over_time(self, users: List[LoadTestUser], duration: int) -> List[Dict[str, Any]]:
    ...
    async def run_stress_test(
        ...
        for user_count in range(step_size, max_users + 1, step_size):
            ...
            try:
                metrics = await self.run_load_test(search_service, test_queries, load_config)
    ...
    async def run_endurance_test(
        ...
        load_config = LoadTestConfig(
            ...
            total_requests=concurrent_users * duration_seconds // 10,  # Request every 10 seconds per user
        ...
        logger.info(f"Starting endurance test: {duration_hours} hours, {concurrent_users} users")
