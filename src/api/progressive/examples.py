...
from .factory import create_system, discover_features, get_quick_examples
from .protocols import EmbeddingProtocol, SearchProtocol
from .simple import AIDocSystem, SimpleSearchResult
from .types import ContentType, QualityTier, SearchOptions, SearchStrategy
...
class ProgressiveAPIShowcase:
    ...
    async def example_progressive_builders():
        ...
        system = (
            ...
            .with_cache(  # Add caching
                enabled=True, ttl=3600, max_size=1000
            )
            .with_monitoring(  # Add monitoring
                enabled=True, track_costs=False, budget_limit=None
            )
            .build()
        )
        ...
        async with system:
            ...
            results = await system.search(
                "machine learning algorithms", limit=5, options=search_options
            )
    ...
    async def example_advanced_configuration():
        ...
        system = (
            ...
            .with_advanced_config(
                lambda config: config.with_embedding_config(
                    lambda emb: emb.with_provider("openai")
                ...
                .with_search_config(
                    lambda search: search.with_strategy("semantic")
                    .with_reranking(enabled=True)
                    .with_custom_weights(
                        {"semantic": 0.7, "keyword": 0.2, "freshness": 0.1}
                    )
                    .with_fusion_algorithm("reciprocal_rank")
                )
                .with_cache_config(
                    lambda cache: cache.with_caching(enabled=True)
                    .with_ttl(7200)
                    .with_embedding_cache(enabled=True)
                    .with_compression(enabled=True)
                )
                .with_monitoring_config(
                    lambda mon: mon.with_monitoring(enabled=True)
                    .with_performance_tracking(
                        latency=True, throughput=True, errors=True
                    )
                    .with_cost_tracking(enabled=True, alerts=True, budget_limit=100.0)
    ...
    async def example_batch_processing():
        ...
        async with system:
            ...
            document_ids = await system.add_documents_from_urls(
                urls, batch_size=2, progress_callback=progress_callback
            )
    ...
    async def example_feature_discovery():
        ...
        for provider in providers:
            ...
            print(
                f"    Cost: {provider.cost_model}, Performance: {provider.performance_tier.value}"
            )
    ...
    async def example_error_handling():
        ...
        try:
            ...
            print(
                "  → Solution: Use 'async with system:' or call 'await system.initialize()'"
            )
        ...
        try:
            ...
            print(
                "  → Solution: Use discovery.discover_embedding_providers() to see options"
            )
