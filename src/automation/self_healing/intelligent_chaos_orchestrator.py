...
class SystemWeaknessAnalyzer:
    ...
    async def identify_weaknesses(self, metrics: SystemMetrics) -> list[SystemWeakness]:
        ...
        weaknesses.extend(
            await self._analyze_resource_contention(metrics, current_time)
        )
        weaknesses.extend(
            await self._analyze_performance_degradation(metrics, current_time)
        )
    ...
    async def _analyze_resource_contention(
        self, metrics: SystemMetrics, timestamp: datetime
    ) -> list[SystemWeakness]:
    ...
    async def _analyze_capacity_limits(
        self, metrics: SystemMetrics, timestamp: datetime
    ) -> list[SystemWeakness]:
    ...
    async def _filter_and_prioritize_weaknesses(
        self, weaknesses: list[SystemWeakness]
    ) -> list[SystemWeakness]:
...
class AdaptiveChaosTestGenerator:
    ...
    def _initialize_experiment_templates(self) -> list[ChaosExperimentTemplate]:
        ...
        return [
            ...
            ChaosExperimentTemplate(
                ...
                safety_constraints=[
                    "preserve_health_checks",
                    "monitor_cascade_effects",
                ],
                learning_objectives=[
                    "Network resilience",
                    "Timeout handling",
            ...
            ChaosExperimentTemplate(
                ...
                target_weakness_types=[
                    WeaknessType.PERFORMANCE_DEGRADATION,
                    WeaknessType.RESOURCE_CONTENTION,
                ],
    ...
    async def generate_targeted_tests(
        ...
        for weakness in weaknesses:
            ...
            best_template = await self._select_best_template(
                weakness, suitable_templates
            )

            # Generate experiment from template
            experiment = await self._generate_experiment_from_template(
                best_template, weakness
            )
    ...
    async def _select_best_template(
        self, weakness: SystemWeakness, templates: list[ChaosExperimentTemplate]
    ) -> ChaosExperimentTemplate:
    ...
    async def _generate_experiment_from_template(
        self, template: ChaosExperimentTemplate, weakness: SystemWeakness
        ...
        try:
            ...
            duration_range = template.parameter_ranges.get(
                "duration_seconds", (60, 300)
            )
            duration = (
                duration_range[0]
                + (duration_range[1] - duration_range[0]) * weakness.severity
            )
    ...
    async def _generate_exploratory_experiments(
        self, count: int
    ) -> list[ChaosExperiment]:
    ...
    async def _calculate_novelty_score(
        self, template: ChaosExperimentTemplate, component: str
    ) -> float:
        """Calculate novelty score for this template and component combination."""
        # Check recent experiment history
        recent_experiments = [
            exp
            for exp in self.experiment_history[-20:]  # Last 20 experiments
            if exp.target_service == component
            and exp.metadata.get("template_id") == template.template_id
        ]
...
class RecoveryValidator:
    ...
    async def validate_recovery(
        self, pre_snapshot: SystemMetrics, experiment: ChaosExperiment
    ) -> dict[str, Any]:
    ...
    async def _validate_recovery_phase(
        ...
        if (
            current.response_time_p95
            > pre_snapshot.response_time_p95 * performance_tolerance
        ):
            performance_recovered = False

        if (
            current.error_rate > pre_snapshot.error_rate * 2
        ):  # Allow double error rate during recovery
            performance_recovered = False

        # Service health validation
        service_health = (
            sum(current.service_health_scores.values())
            / len(current.service_health_scores)
            if current.service_health_scores
            else 0
        )
        ...
        metrics_comparison = {
            ...
            "response_time_change": current.response_time_p95
            - pre_snapshot.response_time_p95,
        ...
        return {
            ...
            "fully_recovered": health_recovered
            and performance_recovered
            and service_health > 0.8,
        }
    ...
    async def _analyze_performance_impact(
        ...
        if len(validation_phases) > 1:
            response_times = [
                phase["metrics_comparison"].get("response_time_change", 0)
                for phase in validation_phases
            ]
            recovery_slope = (response_times[-1] - response_times[0]) / len(
                validation_phases
            )
        else:
            recovery_slope = 0
        ...
        return {
            ...
            "time_to_stable_performance": next(
                (
                    phase["recovery_time_seconds"]
                    for phase in validation_phases
                    if phase["performance_recovered"]
                ),
                validation_phases[-1]["recovery_time_seconds"]
                if validation_phases
                else 0,
            ),
        }
    ...
    async def _calculate_resilience_score(
        self, validation_phases: list[dict[str, Any]]
    ) -> float:
        ...
        stable_phases = sum(
            1 for phase in validation_phases if phase["performance_recovered"]
        )
    ...
    async def _extract_recovery_lessons(
        self, experiment: ChaosExperiment, validation_phases: list[dict[str, Any]]
    ) -> list[str]:
        ...
        if recovery_time < 60:
            ...
            lessons.append(
                "Recovery time exceeds target - investigate optimization opportunities"
            )

        # Performance impact lessons
        performance_phases = [
            phase for phase in validation_phases if not phase["performance_recovered"]
        ]
        ...
        if final_phase["health_recovered"]:
            ...
            lessons.append(
                "System failed to fully recover - manual intervention may be required"
            )
...
class IntelligentChaosOrchestrator:
        ...
        while self.orchestration_active:
            try:
                ...
                if await self.should_run_chaos_tests(resilience_assessment):
                    ...
                    experiment_results = await self.execute_adaptive_chaos_session(
    ...
    async def assess_system_resilience(self) -> ResilienceAssessment:
        ...
        component_scores = await self._calculate_component_resilience_scores(
            current_metrics, weaknesses
        )
    ...
    async def should_run_chaos_tests(self, assessment: ResilienceAssessment) -> bool:
        ...
        if (
            current_metrics.cpu_percent > 90
            or current_metrics.memory_percent > 90
            or current_metrics.error_rate > 0.1
        ):
        ...
        recent_failures = [
            result
            for result in self.experiment_history[-10:]
    ...
    async def execute_adaptive_chaos_session(
        self, assessment: ResilienceAssessment
    ) -> list[ChaosResult]:
        """Execute an adaptive chaos testing session."""
        # Generate targeted experiments
        experiments = await self.test_generator.generate_targeted_tests(
            assessment.identified_weaknesses
        )
    ...
    async def _calculate_component_resilience_scores(
        ...
        for weakness in weaknesses:
            if weakness.component in base_scores:
                ...
                base_scores[weakness.component] = max(
                    0.0, base_scores[weakness.component] - penalty
                )
    ...
    async def _identify_system_strengths(
        self, metrics: SystemMetrics, weaknesses: list[SystemWeakness]
    ) -> list[str]:
        ...
        healthy_services = sum(
            1 for score in metrics.service_health_scores.values() if score > 0.9
        )
        if healthy_services > 0:
            strengths.append(
                f"{healthy_services} services demonstrate excellent health"
            )
    ...
    async def _identify_improvement_areas(
        ...
        for weakness_type in weakness_types:
            if weakness_type == WeaknessType.RESOURCE_CONTENTION:
                ...
                improvement_areas.append(
                    "Error handling and circuit breaker improvements needed"
                )
            elif weakness_type == WeaknessType.DEPENDENCY_FAILURE:
                improvement_areas.append(
                    "Dependency isolation and fallback mechanisms require attention"
                )
            elif weakness_type == WeaknessType.CAPACITY_LIMIT:
                improvement_areas.append(
                    "Capacity planning and auto-scaling improvements needed"
                )

        return improvement_areas

    async def _recommend_focus_areas(
        self, weaknesses: list[SystemWeakness], component_scores: dict[str, float]
    ) -> list[str]:
        ...
        if business_critical:
            focus_areas.append(
                f"Business critical: {business_critical[0].component} resilience"
            )
    ...
    async def _analyze_blast_radius(
        self,
        pre_snapshot: SystemMetrics,
        monitoring_data: list[SystemMetrics],
        experiment: ChaosExperiment,
    ) -> dict[str, Any]:
        ...
        for component in ["cpu", "memory", "response_time", "error_rate"]:
            baseline = getattr(
                pre_snapshot,
                f"{component}_percent" if component in ["cpu", "memory"] else component,
                0,
            )
            ...
            impact_percentage = (
                ((peak_impact - baseline) / baseline * 100) if baseline > 0 else 0
            )
        ...
        return {
            ...
            "cascading_effects": len(
                [
                    comp
                    for comp, data in impact_analysis.items()
                    if data["impact_percentage"] > 10
                ]
            ),
            "contained_impact": all(
                data["impact_percentage"] < 50 for data in impact_analysis.values()
            ),
        }

    async def _extract_experiment_lessons(
        self,
        experiment: ChaosExperiment,
        execution_result,
        recovery_analysis: dict[str, Any],
        ...
        # Validate hypothesis
        hypothesis_validated = execution_result.success and recovery_analysis.get(
        ...
        if recovery_analysis.get("total_recovery_time", 0) > 180:
            vulnerabilities.append(
                "Slow recovery time indicates optimization opportunity"
            )
        ...
        if execution_result.success:
            improvements.append(
                f"Confirmed resilience to {experiment.failure_type.value}"
            )
        ...
        insights = []
        peak_impact = max(
            (getattr(m, "response_time_p95", 0) for m in monitoring_data), default=0
        )
        if peak_impact > 0:
    ...
    async def _calculate_resilience_impact(
        ...
        similar_experiments = [
            exp
            for exp in self.experiment_history[-20:]
            if exp.experiment_result.experiment_name.startswith(
    ...
    async def _identify_unexpected_effects(
        self,
        experiment: ChaosExperiment,
        monitoring_data: list[SystemMetrics],
        pre_snapshot: SystemMetrics,
        ...
        for metrics in monitoring_data:
            if (
                experiment.target_service != "cpu"
                and metrics.cpu_percent > pre_snapshot.cpu_percent + 30
            ):
                unexpected.append(f"Unexpected CPU spike to {metrics.cpu_percent:.1f}%")

            if (
                experiment.target_service != "memory"
                and metrics.memory_percent > pre_snapshot.memory_percent + 20
            ):
                unexpected.append(
                    f"Unexpected memory increase to {metrics.memory_percent:.1f}%"
                )

            if (
                experiment.failure_type != FailureType.NETWORK_TIMEOUT
    ...
    async def get_orchestration_status(self) -> dict[str, Any]:
        ...
        recent_experiments = (
            self.experiment_history[-20:] if self.experiment_history else []
        )
        ...
        return {
            ...
            "recent_success_rate": recent_successful / len(recent_experiments)
            if recent_experiments
            else 0,
            "resilience_assessments_count": len(self.resilience_history),
            "current_resilience_level": self.resilience_history[
                -1
            ].overall_resilience.value
            if self.resilience_history
            else "unknown",
            "learning_database_size": sum(
                len(learnings) for learnings in self.learning_database.values()
            ),
