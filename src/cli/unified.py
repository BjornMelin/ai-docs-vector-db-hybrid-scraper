...
@cli.command()
@click.option('--profile', type=click.Choice(['unit', 'fast', 'integration', 'full']), default='fast')
@click.option('--coverage/--no-coverage', default=False)
@click.option('--verbose/--quiet', default=False)
@click.option('--parallel', type=int, default=0, help="Number of parallel workers (0 = auto)")
...
def setup():
    ...
    subprocess.run(["uv", "run", "pre-commit", "install"], capture_output=True)
    
    # Validate configuration
    click.echo("✅ Validating configuration...")
    subprocess.run(["python", "scripts/validate_config.py"], capture_output=True)
...
def quality():
    ...
    result = subprocess.run(["mypy", "src/", "--config-file", "pyproject.toml"])
...
def docs(host: str, port: int):
    ...
    subprocess.run(["mkdocs", "serve", "--host", host, "--port", str(port)])
...
def services():
    ...
    subprocess.run(["./scripts/start-services.sh"])
...
def benchmark(profile: str):
    ...
    subprocess.run(["python", "scripts/run_benchmarks.py", "--profile", profile])
...
def validate():
    ...
    # Validate configuration
    result1 = subprocess.run(["python", "scripts/validate_config.py"])
    
    # Validate documentation links
    result2 = subprocess.run(["python", "scripts/validate_docs_links.py"])
