...
class ConfigAuditor:
    ...
    def get_recent_activity(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        while current_date <= end_date:
            audit_file = self.audit_dir / f"audit_{current_date.strftime('%Y%m%d')}.json"
    ...
    def show_audit_summary(self) -> None:
        ...
        # Summary statistics
        total_sessions = len([r for r in recent_activity if r.get("action") == "wizard_completion"])
        profiles_used = set(r.get("profile") for r in recent_activity if r.get("profile"))
        
        console.print(f"• Total wizard sessions: {total_sessions}")
        console.print(f"• Profiles used: {', '.join(sorted(profiles_used))}")
        console.print(f"• Last activity: {recent_activity[-1].get('timestamp', 'Unknown') if recent_activity else 'None'}")
