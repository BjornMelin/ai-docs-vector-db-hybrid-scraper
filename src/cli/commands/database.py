...
def complete_collection_name(ctx: click.Context, param: click.Parameter, incomplete: str) -> List[CompletionItem]:
...
def _display_collections_table(collections: List[Dict[str, Any]], rich_cli):
    ...
    for collection in collections:
        ...
        table.add_row(
            ...
            vector_count,
            str(collection.get('config', {}).get('params', {}).get('vectors', {}).get('size', 'Unknown')),
            status_display,
...
@click.option(
    "--force",
    is_flag=True,
    help="Force creation (delete existing collection)"
)
@click.pass_context
def create_collection(
    ...
    if force:
        if not Confirm.ask(f"Delete existing collection '{collection_name}' if it exists?"):
    ...
    with Progress(
        ...
        task = progress.add_task(f"Creating collection '{collection_name}'...", total=None)
        ...
        try:
            ...
            if force and collection_name in asyncio.run(db_manager.list_collections()):
                progress.update(task, description=f"Deleting existing collection...")
                asyncio.run(db_manager.delete_collection(collection_name))
            
            # Create collection
            progress.update(task, description=f"Creating collection with {dimension}D vectors...")
            success = asyncio.run(db_manager.create_collection(
...
def delete_collection(
    ...
    # Confirmation prompt
    if not yes:
        if not Confirm.ask(f"Delete collection '{collection_name}'? This cannot be undone."):
    ...
    with Progress(
        ...
        task = progress.add_task(f"Deleting collection '{collection_name}'...", total=None)
...
def collection_info(ctx: click.Context, collection_name: str):
    ...
    with Progress(
        ...
        task = progress.add_task(f"Loading collection info...", total=None)
    ...
    info_table.add_row("Indexed Vectors", f"{info_dict.get('indexed_vectors_count', 0):,}")
...
def search_collection(
    ...
    with Progress(
        ...
        try:
            ...
            # Perform search
            results = asyncio.run(db_manager.search_vectors(
                collection_name=collection_name,
