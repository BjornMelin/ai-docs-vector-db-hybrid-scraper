...
class ConfigurationWizard:
    ...
    def welcome(self):
        ...
        welcome_text.append("Let's set up your AI Documentation Scraper!\n\n", style="dim")
        welcome_text.append("This wizard will guide you through configuring:\n", style="")
    ...
    def configure_embeddings(self) -> Dict[str, Any]:
        ...
        provider_table.add_row("1", "OpenAI", "High-quality embeddings, requires API key")
    ...
    def configure_browser(self) -> Dict[str, Any]:
        ...
        headless = Confirm.ask(
            "Run browsers in headless mode? (recommended for servers)",
            default=True
        )
        
        # Anti-detection settings
        use_stealth = Confirm.ask(
    ...
    def configure_performance(self) -> Dict[str, Any]:
        ...
        enable_cache = Confirm.ask(
            "Enable Redis caching? (improves performance)",
            default=True
        )
        
        if enable_cache:
            redis_host = Prompt.ask(
                "Redis host",
                default="localhost"
            )
            redis_port = Prompt.ask(
                "Redis port",
                default="6379"
            )
    ...
    def save_configuration(self, config_data: Dict[str, Any]) -> Path:
        ...
        try:
            ...
            self.console.print(f"✅ Configuration saved to: [green]{config_path}[/green]")
            return config_path
    ...
    def run_setup(self) -> Path:
        ...
        success_text.append("Your AI Documentation Scraper is now configured.\n\n", style="")
        success_text.append("Next steps:\n", style="bold")
        success_text.append(f"1. Test your configuration: ai-docs config validate {config_path}\n", style="cyan")
        success_text.append("2. Check system status: ai-docs status\n", style="cyan")
        success_text.append("3. Create your first collection: ai-docs database create my-docs", style="cyan")
...
@click.pass_context
def setup(ctx: click.Context, output: Optional[Path], config_format: str):
