...
class AgenticSearchRequest(BaseModel):
    ...
    min_quality_score: Optional[float] = Field(None, description="Minimum quality requirement")
...
class AgenticAnalysisRequest(BaseModel):
    ...
    user_context: Optional[Dict[str, Any]] = Field(None, description="User context for analysis")
...
class AgenticSearchResponse(BaseModel):
    ...
    # Agent insights
    orchestration_plan: Dict[str, Any] = Field(default_factory=dict, description="Orchestration decisions")
    tools_used: List[str] = Field(default_factory=list, description="Tools selected and used")
    agent_reasoning: Optional[str] = Field(None, description="Agent decision reasoning")
    ...
    strategy_effectiveness: Optional[float] = Field(None, description="Strategy effectiveness score")
    learned_insights: Optional[Dict[str, Any]] = Field(None, description="Insights for future queries")
...
def register_tools(mcp: FastMCP, client_manager: ClientManager) -> None:
    ...
    async def agentic_search(request: AgenticSearchRequest) -> AgenticSearchResponse:
        ...
        try:
            ...
            if not orchestration_result["success"]:
                raise RuntimeError(f"Orchestration failed: {orchestration_result.get('error')}")
            ...
            return AgenticSearchResponse(
                ...
                agent_reasoning=f"Search failed due to: {str(e)}"
            )
            
    @mcp.tool()
    async def agentic_analysis(request: AgenticAnalysisRequest) -> AgenticAnalysisResponse:
        ...
        try:
            ...
            if not execution_result["success"]:
                raise RuntimeError(f"Analysis execution failed: {execution_result.get('error')}")
            ...
            if "classify_content_results" in results:
                ...
                if classification.get("technical", 0) > 0.8:
                    recommendations.append("Content is highly technical - consider adding explanatory notes")
            ...
            response = AgenticAnalysisResponse(
                ...
                processing_time_ms=execution_result["metadata"]["total_execution_time_ms"]
            )
    ...
    async def reset_agent_learning() -> Dict[str, str]:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Failed to reset agent learning: {str(e)}"
            }
        ...
        try:
            ...
            optimization_results = {
                ...
                "expected_improvements": {
                    "latency_reduction_pct": 15.0 if optimization_target == "speed" else 5.0,
                    "quality_improvement_pct": 8.0 if optimization_target == "quality" else 2.0,
                    "cost_reduction_pct": 10.0 if optimization_target == "cost" else 3.0
                },
                "new_configuration": {
                    "orchestrator_temperature": 0.05 if optimization_target == "speed" else 0.1,
                    "tool_selection_threshold": 0.7 if optimization_target == "speed" else 0.8,
                    "cache_ttl_seconds": 3600 if optimization_target == "speed" else 1800
                },
